...
use fortress_core::encryption::{
    Aegis256Wrapper, Aes256Gcm, ChaCha20Poly1305, EncryptionAlgorithm,
};
...
fn bench_encryption_algorithms(c: &mut Criterion) {
    ...
    for size in data_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("aegis256_encrypt", size), &size, |b, _| {
            let key = KeyManager::new()
                .generate_key(&Aegis256Wrapper::new())
                .unwrap();
            b.iter(|| {
...
fn bench_key_operations(c: &mut Criterion) {
    ...
    group.bench_function("key_generation_aegis256", |b| {
        ...
        b.iter(|| key_manager.generate_key(black_box(&algorithm)))
    });
...
fn bench_cache_operations(c: &mut Criterion) {
    ...
    group.bench_function("cache_put", |b| {
        ...
        b.iter(|| cache.put(black_box(key), black_box(value.clone())))
    });
    ...
    group.bench_function("cache_miss", |b| {
        let cache = CacheManager::new(1000);
        b.iter(|| cache.get(black_box("non_existent_key")))
    });
