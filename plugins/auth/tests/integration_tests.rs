...
async fn test_jwt_plugin_authentication() {
    ...
    registry
        .initialize()
        .await
        .expect("Failed to initialize plugin registry");
...
async fn test_jwt_plugin_invalid_token() {
    ...
    assert!(
        !auth_result.error_message.is_empty(),
...
async fn test_oauth_plugin_authorization_flow() {
    ...
    assert!(
        auth_result.success,
        "Authorization initiation should succeed"
    );

    // Check for authorization URL in response
    let response_data: Value =
        serde_json::from_str(&auth_result.response_data).unwrap_or(json!({}));
    assert!(
        response_data.get("authorization_url").is_some(),
...
async fn test_oauth_plugin_token_exchange() {
    ...
    assert!(
        !auth_result.user_id.is_empty() || !auth_result.error_message.is_empty(),
...
async fn test_plugin_health_checks() {
    ...
    for plugin_name in plugins {
        ...
        assert!(
            health.is_ok(),
            "Health check should succeed for {}",
            plugin_name
        );
        ...
        assert!(
            !health_status.version.is_empty(),
            "Plugin {} should report version",
            plugin_name
        );
    }
}
...
async fn test_plugin_capabilities() {
    ...
    for plugin_name in plugins {
        ...
        assert!(
            capabilities.is_ok(),
            "Should get capabilities for {}",
            plugin_name
        );

        let caps = capabilities.unwrap();
        assert!(
            caps.can_validate_tokens,
            "Plugin {} should validate tokens",
            plugin_name
        );
    }
}
...
async fn test_plugin_configuration_validation() {
    ...
    for (plugin_name, config) in test_configs {
        ...
        assert!(
            result.is_ok(),
            "Config validation should succeed for {}",
            plugin_name
        );
    }
}
...
mod test_helpers {
    ...
    pub fn create_test_saml_assertion(user_id: &str) -> String {
        ...
        format!(
            "<saml:Assertion><saml:Subject>{}</saml:Subject></saml:Assertion>",
            user_id
        )
    }
}
