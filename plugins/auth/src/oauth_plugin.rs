...
fn get_user_info_from_token(
    ...
    match make_http_request(&url, "GET", None) {
        Ok(response) => {
            serde_json::from_str(&response).map_err(|e| format!("Failed to parse user info: {}", e))
        }
...
fn validate_id_token(id_token: &str) -> Result<OAuthUserInfo, String> {
    ...
    let payload_str =
        String::from_utf8(payload).map_err(|_| "Invalid UTF-8 in ID token".to_string())?;
    ...
    match auth_request.method.as_str() {
        "OAuth" => {
            ...
            if let (Some(code), Some(_state), Some(redirect_uri)) = (
                ...
                match exchange_authorization_code_for_token(
                    code,
                    redirect_uri,
                    ...
                    Ok(token_response) => {
                        // Get user info
                        let user_info =
                            match get_user_info_from_token(&token_response.access_token, config) {
                                Ok(info) => Some(info),
                                Err(e) => {
                        ...
                        let expires_at = token_response
                            .expires_in
                            .map(|seconds| (unsafe { get_timestamp() } as u64) + seconds);
                        ...
                        let result = AuthResult {
                            ...
                            metadata: {
                                ...
                                metadata.insert(
                                    ...
                                    serde_json::Value::String(token_response.token_type),
                                );
                                metadata.insert(
                                    "scope".to_string(),
                ...
                let result = AuthResult {
                    ...
                    expires_at: None,
                    error: Some(
                        "Authorization code, state, and redirect_uri are required".to_string(),
                    ),
                    metadata: HashMap::new(),
                };
...
#[no_mangle]
pub extern "C" fn oauth_logout(token_ptr: *const u8, token_len: usize) -> i32 {
    if !*PLUGIN_INITIALIZED.get().unwrap_or(&false) {
        return 0;
    }
