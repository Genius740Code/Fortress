...
async fn main() -> Result<()> {
    ...
    if let Some(status) = jwt_status {
        match status.status {
            ...
            _ => {
                warn!("JWT plugin deployment status: {:?}", status.status);
            }
        }
    }
    ...
    if let Some(status) = oauth_status {
        match status.status {
            ...
            _ => {
                warn!("OAuth plugin deployment status: {:?}", status.status);
            }
        }
    }
    ...
    if let Some(status) = saml_status {
        match status.status {
            ...
            _ => {
                warn!("SAML plugin deployment status: {:?}", status.status);
            }
        }
    }
    ...
    if let Some(status) = hot_swap_status {
        match status.status {
            ...
            _ => {
                warn!("JWT plugin hot-swap status: {:?}", status.status);
            }
        }
    }
    ...
    info!("\nStep 11: Demonstrating Canary Deployment (OAuth Plugin)");
    ...
    if let Some(status) = canary_status {
        match status.status {
            ...
            _ => {
                warn!("OAuth canary deployment status: {:?}", status.status);
            }
        }
    }
