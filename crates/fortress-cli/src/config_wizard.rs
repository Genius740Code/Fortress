...
impl ConfigurationWizard {
    pub async fn run_interactive_setup() -> Result<FortressConfig, FortressError> {
        ...
        term.clear_screen().map_err(|e| {
            FortressError::configuration_error(
                "clear_screen",
                &format!("{}", e),
                "terminal operation",
            )
        ...
        {
            return Err(FortressError::configuration_error(
                "setup_cancelled",
                "user_cancelled",
                "user confirmation",
            ));
    ...
    async fn configure_encryption() -> Result<EncryptionConfig, FortressError> {
        ...
        let (algorithm, key_rotation_days, field_level_encryption) = if enable_encryption {
            let algo_choices = vec![
                "AEGIS-256 (Recommended - Highest performance)",
                "ChaCha20-Poly1305",
                "AES-256-GCM",
            ];
    ...
    async fn configure_networking() -> Result<NetworkingConfig, FortressError> {
        ...
        let port = Input::<u16>::new()
            ...
            .interact()
            .map_err(|e| {
                FortressError::configuration_error("port", &format!("{}", e), "number 1-65535")
            })?;
    ...
    fn display_configuration_summary(config: &FortressConfig) -> Result<(), FortressError> {
        ...
        if config.database.backend == "sqlite" {
            println!(
                "Database: {} @ {}",
                config.database.backend, config.database.host
            );
        } else {
            println!(
                "Database: {} @ {}:{}/{}",
                config.database.backend, config.database.host, config.database.port, "default"
            ); // database_name field doesn't exist
        }

        if config.encryption.enabled {
            println!(
                "Encryption: {} (rotation: {} days)",
                config.encryption.algorithm, config.encryption.key_rotation_days
            );
        } else {
        ...
        println!(
            "Networking: {}:{}",
            config.networking.bind_address, config.networking.port
        );
    ...
    async fn save_configuration(config: &FortressConfig) -> Result<(), FortressError> {
        ...
        if let Some(parent) = config_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                FortressError::io_error(format!("Failed to create config directory: {}", e))
            })?;
        }
    ...
    pub async fn load_configuration() -> Result<Option<FortressConfig>, FortressError> {
        ...
        let config_content = std::fs::read_to_string(&config_path).map_err(|e| {
            FortressError::configuration_error("file_read", &format!("{}", e), "readable file path")
        })?;

        let config = toml::from_str(&config_content).map_err(|e| {
            FortressError::configuration_error("file_parse", &format!("{}", e), "valid TOML format")
        })?;
