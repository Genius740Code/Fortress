...
#[derive(Serialize, Deserialize, Clone, Debug, async_graphql::Enum, Copy, Eq, PartialEq)]
...
impl EventBus {
    ...
    ) -> Result<usize, broadcast::error::SendError<DataChangeEvent>> {
    ...
    pub fn publish_security_event(
        &self,
        event: SecurityEvent,
    ) -> Result<usize, broadcast::error::SendError<SecurityEvent>> {
    ...
    ) -> Result<usize, broadcast::error::SendError<PerformanceMetrics>> {
        self.performance_metrics_sender.send(metrics)
    }

    pub fn publish_cluster_event(
        &self,
        event: ClusterEvent,
    ) -> Result<usize, broadcast::error::SendError<ClusterEvent>> {
        self.cluster_event_sender.send(event)
    }

    pub fn publish_compliance_event(
        &self,
        event: ComplianceEvent,
    ) -> Result<usize, broadcast::error::SendError<ComplianceEvent>> {
...
impl DataSubscription {
    ...
    async fn data_changes(
        ...
        let stream = BroadcastStream::new(subscription_rx).filter_map(|result| async move {
            match result {
                Ok(event) => Some(event),
    ...
    async fn cluster_events(
        ...
        if !user_context.has_permission("cluster.monitor") {
            return Err(async_graphql::Error::new(
                "Insufficient permissions for cluster monitoring",
            ));
        }
    ...
    async fn compliance_events(
        ...
        if !user_context.has_permission("compliance.monitor") {
            return Err(async_graphql::Error::new(
                "Insufficient permissions for compliance monitoring",
            ));
        }
...
impl MetricsCollector {
    ...
    pub async fn simulate_data_change_event(
        &self,
        table_id: &str,
    ...
    pub async fn simulate_security_event(
        &self,
        event_type: &str,
    ...
    pub async fn simulate_compliance_event(
        &self,
        framework: &str,
...
pub type FortressSchema =
    async_graphql::Schema<async_graphql::EmptyMutation, QueryRoot, DataSubscription>;
