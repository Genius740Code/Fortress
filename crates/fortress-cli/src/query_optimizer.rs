...
pub trait Cache: Send + Sync {
    fn get<'a>(
        &'a self,
        key: &'a str,
    ) -> Pin<Box<dyn Future<Output = Option<QueryResult>> + Send + 'a>>;
    fn set<'a>(
        &'a self,
        key: &'a str,
        value: &'a QueryResult,
...
pub trait ConnectionPool: Send + Sync {
    ...
    ) -> Pin<Box<dyn Future<Output = Result<Vec<QueryResult>>> + Send + 'a>>;
    fn get_connection_stats<'a>(
        &'a self,
    ) -> Pin<Box<dyn Future<Output = ConnectionStats> + Send + 'a>>;
}
...
pub trait QueryPlanner: Send + Sync {
    fn optimize<'a>(
        &'a self,
        query: &'a str,
        params: &'a Option<Vec<serde_json::Value>>,
    ) -> Pin<Box<dyn Future<Output = Result<QueryPlan>> + Send + 'a>>;
    fn explain<'a>(
        &'a self,
        query: &'a str,
    ) -> Pin<Box<dyn Future<Output = Result<ExecutionPlan>> + Send + 'a>>;
    fn validate<'a>(
        &'a self,
        query: &'a str,
    ) -> Pin<Box<dyn Future<Output = Result<Vec<ValidationError>>> + Send + 'a>>;
}
