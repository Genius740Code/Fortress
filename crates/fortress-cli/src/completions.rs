    ...
    Command::new("fortress")
        ...
        .subcommand(
            Command::new("config")
                ...
                .subcommand(
                    Command::new("init").about("Initialize configuration").arg(
                        Arg::new("interactive")
                            .long("interactive")
                            .short('i')
        ...
        .subcommand(
            Command::new("server")
                ...
                .subcommand(
                    Command::new("status").about("Show server status").arg(
                        Arg::new("format")
                            .long("format")
                            .short('f')
                ...
                .subcommand(
                    Command::new("stop").about("Stop Fortress server").arg(
                        Arg::new("graceful")
                            .long("graceful")
                            .short('g')
        ...
        .subcommand(
            Command::new("plugin")
                ...
                .subcommand(
                    Command::new("list").about("List available plugins").arg(
                        Arg::new("format")
                            .long("format")
                            .short('f')
                ...
                .subcommand(
                    Command::new("uninstall").about("Uninstall plugin").arg(
                        Arg::new("name")
                            .long("name")
                            .short('n')
                            .value_name("NAME")
                ...
                .subcommand(
                    Command::new("enable").about("Enable plugin").arg(
                        Arg::new("name")
                            .long("name")
                            .short('n')
                            .value_name("NAME")
                ...
                .subcommand(
                    Command::new("disable").about("Disable plugin").arg(
                        Arg::new("name")
                            .long("name")
                            .short('n')
                            .value_name("NAME")
        ...
        .subcommand(
            Command::new("cluster")
                ...
                .subcommand(
                    Command::new("nodes").about("List cluster nodes").arg(
                        Arg::new("format")
                            .long("format")
                            .short('f')
                            .value_parser(["json", "table"])
...
pub fn install_completions() -> Result<(), FortressError> {
    let home_dir =
        dirs::home_dir().ok_or_else(|| FortressError::io_error("Cannot find home directory"))?;
    ...
    let completions_dir = match std::env::consts::OS {
        ...
        _ => {
            return Err(FortressError::io_error(
                "Unsupported operating system for completions",
            ));
        }
    };
    ...
    let zsh_dir = home_dir
        .join(".local")
        .join("share")
        .join("zsh")
        .join("site-functions");
    std::fs::create_dir_all(&zsh_dir).map_err(|e| {
        FortressError::io_error(format!("Failed to create zsh completions directory: {}", e))
    })?;
    let zsh_file = zsh_dir.join("_fortress");
    let mut zsh_output = std::fs::File::create(&zsh_file).map_err(|e| {
        FortressError::io_error(format!("Failed to create zsh completion file: {}", e))
    })?;
    ...
    let fish_dir = home_dir
        ...
        .join("fish")
        .join("completions");
    std::fs::create_dir_all(&fish_dir).map_err(|e| {
        FortressError::io_error(format!(
            "Failed to create fish completions directory: {}",
            e
        ))
    })?;
    let fish_file = fish_dir.join("fortress.fish");
    let mut fish_output = std::fs::File::create(&fish_file).map_err(|e| {
        FortressError::io_error(format!("Failed to create fish completion file: {}", e))
    })?;
    ...
    match std::env::consts::OS {
        ...
        "windows" => {
            ...
            println!(
                "    Import-Module {}",
                completions_dir.join("fortress.ps1").display()
            );
        }
        _ => {}
    }
...
pub fn print_completion_script(shell: &str) -> Result<(), FortressError> {
    let _ = match shell {
        "bash" => generate_completions(clap_complete::shells::Bash, &mut std::io::stdout()),
        "zsh" => generate_completions(clap_complete::shells::Zsh, &mut std::io::stdout()),
        "fish" => generate_completions(clap_complete::shells::Fish, &mut std::io::stdout()),
        "elvish" => generate_completions(clap_complete::shells::Elvish, &mut std::io::stdout()),
        "powershell" => {
            generate_completions(clap_complete::shells::PowerShell, &mut std::io::stdout())
        }
        _ => {
            return Err(FortressError::io_error(format!(
                "Unsupported shell: {}",
                shell
            )))
        }
...
pub mod dynamic_completions {
    ...
    pub async fn complete_key_ids() -> Result<Vec<String>, FortressError> {
        ...
        // Convert (KeyId, KeyMetadata) tuples to strings
        let key_ids: Vec<String> = keys.iter().map(|(key_id, _)| key_id.to_string()).collect();
    ...
    pub async fn complete_keys_by_algorithm(
        algorithm_hint: &str,
