...
pub async fn handle_status(data_dir: Option<String>) -> Result<()> {
    ...
    if config_path.exists() {
        ...
        if let Ok(config_content) = tokio::fs::read_to_string(&config_path).await {
            if let Ok(config) =
                toml::from_str::<crate::commands::config::ConfigSettings>(&config_content)
            {
    ...
    match check_server_status().await {
        Ok(server_info) => {
            ...
            println!(
                "  Uptime: {}",
                style(format_duration(server_info.uptime)).bold()
            );
            ...
            if let Ok(metrics) = get_server_metrics(&server_info.address).await {
                ...
                println!(
                    "    Requests/sec: {:.2}",
                    style(metrics.requests_per_second).bold()
                );
                println!(
                    "    Avg Response: {}ms",
                    style(metrics.average_response_time_ms).bold()
                );
                println!(
                    "    Active Connections: {}",
                    style(metrics.active_connections).bold()
                );
                println!(
                    "    Memory Usage: {}",
                    style(format_bytes(metrics.memory_usage_bytes)).bold()
                );
                println!(
                    "    CPU Usage: {:.1}%",
                    style(metrics.cpu_usage_percent).bold()
                );
            }
        }
        Err(e) => {
...
async fn get_server_info_from_api() -> Result<ServerInfo> {
    ...
    for port in ports {
        ...
        match timeout(Duration::from_secs(2), client.get(&url).send()).await {
            Ok(Ok(response)) if response.status().is_success() => {
                ...
                let uptime_seconds = server_data
                    .get("uptime_seconds")
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| eyre!("Missing uptime information"))?;

                let version = server_data
                    .get("version")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
    ...
    Err(color_eyre::eyre::eyre!(
        "No running Fortress server found via API"
    ))
}
    ...
    match timeout(Duration::from_secs(2), client.get(&url).send()).await {
        Ok(Ok(response)) if response.status().is_success() => {
            ...
            let metrics_data = metrics
                .get("metrics")
                .ok_or_else(|| color_eyre::eyre::eyre!("Missing metrics data in response"))?;

            Ok(ServerMetrics {
                requests_per_second: metrics_data
                    .get("http_requests_per_second")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.0),
                average_response_time_ms: metrics_data
                    .get("http_request_duration_seconds")
                    .and_then(|v| v.as_f64())
                    .map(|v| (v * 1000.0) as u64)
                    .unwrap_or(0),
                active_connections: metrics_data
                    .get("active_connections")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0) as usize,
                memory_usage_bytes: metrics_data
                    .get("memory_usage_bytes")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0),
                cpu_usage_percent: metrics_data
