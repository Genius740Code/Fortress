...
impl Mutation {
    ...
    async fn create_database(
        ...
        let algorithm = input
            ...
            .unwrap_or(crate::graphql::types::EncryptionAlgorithm::Aegis256);

        // Convert GraphQL enum to fortress_core algorithm for key generation
        let core_algorithm: Box<dyn fortress_core::encryption::EncryptionAlgorithm> =
            match algorithm {
        ...
        let encryption_key = key_manager
            .generate_key(core_algorithm.as_ref())
            .await
            .map_err(|e| {
                async_graphql::Error::new(format!("Encryption key generation failed: {}", e))
            })?;
        ...
        storage
            .put(
                &db_key,
                &serde_json::to_vec(&db_metadata).unwrap_or_default(),
            )
            .await
    ...
    async fn delete_database(&self, ctx: &Context<'_>, name: String) -> Result<ApiResponse<bool>> {
        ...
        let db_exists = storage.exists(&db_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Database existence check failed: {}", e))
        })?;
    ...
    async fn create_table(
        ...
        let table_exists = storage.exists(&table_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Table existence check failed: {}", e))
        })?;
        ...
        let encryption_enabled = input
            .fields
            .iter()
            .any(|f| f.field_type == FieldType::Encrypted);
        ...
        storage
            .put(
                &table_key,
                &serde_json::to_vec(&table_metadata).unwrap_or_default(),
            )
            .await
            .map_err(|e| async_graphql::Error::new(format!("Table creation failed: {}", e)))?;

        // Update database table count
        if let Some(db_data) = storage.get(&db_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to get database for update: {}", e))
        })? {
            if let Ok(mut db_info) = serde_json::from_slice::<serde_json::Value>(&db_data) {
                if let Some(table_count) = db_info.get_mut("table_count") {
                    *table_count = serde_json::Value::Number(serde_json::Number::from(
                    ...
                    storage
                        .put(&db_key, &serde_json::to_vec(&db_info).unwrap())
                        .await
                        .map_err(|e| {
                            async_graphql::Error::new(format!("Database update failed: {}", e))
                        })?;
                }
            }
        }
    ...
    async fn insert_data(
        ...
        if let Some(table_data) = storage
            .get(&table_key)
            .await
                            ...
                            if encrypted {
                                ...
                                if let Some(value) = processed_data.get(field_name) {
                                    if let Some(value_str) = value.as_str() {
                                        ...
                                        match field_encryption_manager
                                            .encrypt_field(&field_id, value_str.as_bytes())
                                            .await
                                        {
                                            Ok(encrypted_field) => {
                                                ...
                                                processed_data[field_name] =
                                                    serde_json::Value::String(
                                                        base64::engine::general_purpose::STANDARD
                                                            .encode(&encrypted_field.ciphertext),
                                                    );
                                            }
                                            Err(_) => {
        ...
        let record_key = format!(
            "db:{}:table:{}:record:{}",
            input.database, input.table, record_id
        );
        storage
            .put(&record_key, &serde_json::to_vec(&record_metadata).unwrap())
            .await
            ...
            if let Ok(mut table_info) = serde_json::from_slice::<serde_json::Value>(&table_data) {
                if let Some(record_count) = table_info.get_mut("record_count") {
                    *record_count = serde_json::Value::Number(serde_json::Number::from(
                    ...
                    storage
                        .put(&table_key, &serde_json::to_vec(&table_info).unwrap())
                        .await
                        .map_err(|e| {
                            async_graphql::Error::new(format!("Failed to update table: {}", e))
                        })?;
                }
            }
        }
    ...
    async fn update_data(
        ...
        let record_key = format!(
            "db:{}:table:{}:record:{}",
            input.database, input.table, input.id
        );
        let record_exists = storage.exists(&record_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to check record existence: {}", e))
        })?;
    ...
    async fn bulk_insert(
        ...
        for (index, item) in data.into_iter().enumerate() {
            ...
            if let Some(table_data) = storage.get(&table_key).await.map_err(|e| {
                async_graphql::Error::new(format!("Table data retrieval failed: {}", e))
            })? {
                if let Ok(table_info) = serde_json::from_slice::<serde_json::Value>(&table_data) {
                    if let Some(fields) = table_info.get("fields").and_then(|v| v.as_array()) {
                        for field in fields {
    ...
    async fn rotate_keys(
    ...
    ) -> Result<ApiResponse<KeyRotationStatus>> {
        let graphql_ctx = from_context(ctx)?;
        ...
        let rotation_key = format!(
            "db:{}:table:{}:rotation:{}",
            input.database, input.table, rotation_id
        );
        ...
        storage
            .put(
                &rotation_key,
                &serde_json::to_vec(&rotation_metadata).unwrap(),
            )
            .await
            .map_err(|e| {
                async_graphql::Error::new(format!("Failed to create rotation status: {}", e))
            })?;
        ...
        tokio::spawn(async move {
            ...
            if let Some(metadata_bytes) = storage_clone.get(&rotation_key).await.unwrap_or_default()
            {
                ...
                {
                    rotation_meta["status"] =
                        serde_json::Value::String("reencrypting_data".to_string());
                    rotation_meta["progress_percentage"] =
            ...
            if let Some(metadata_bytes) = storage_clone.get(&rotation_key).await.unwrap_or_default()
            {
                ...
                {
                    rotation_meta["status"] =
                        serde_json::Value::String("updating_metadata".to_string());
                    rotation_meta["progress_percentage"] =
    ...
    async fn rotate_keys_zero_downtime(
        &self,
        ctx: &Context<'_>,
        database: String,
        table: String,
        algorithm: Option<crate::graphql::types::EncryptionAlgorithm>,
    ) -> Result<ApiResponse<KeyRotationStatus>> {
        let graphql_ctx = from_context(ctx)?;
        ...
        storage
            ...
            .map_err(|e| {
                async_graphql::Error::new(format!(
                    "Failed to create zero-downtime rotation status: {}",
                    e
                ))
            })?;
        ...
        tokio::spawn(async move {
            ...
            if let Some(metadata_bytes) = storage_clone.get(&rotation_key).await.unwrap_or_default()
            {
                ...
                {
                    rotation_meta["status"] =
                        serde_json::Value::String("creating_keys".to_string());
                    rotation_meta["progress_percentage"] =
            ...
            if let Some(metadata_bytes) = storage_clone.get(&rotation_key).await.unwrap_or_default()
            {
                ...
                {
                    rotation_meta["status"] =
                        serde_json::Value::String("batch_reencrypting".to_string());
                    rotation_meta["progress_percentage"] =
            ...
            for batch_progress in [40.0, 50.0, 60.0, 70.0, 80.0] {
                ...
                if let Some(metadata_bytes) =
                    storage_clone.get(&rotation_key).await.unwrap_or_default()
                {
                    if let Ok(mut rotation_meta) =
                        serde_json::from_slice::<serde_json::Value>(&metadata_bytes)
                    {
                        rotation_meta["progress_percentage"] = serde_json::Value::Number(
                            serde_json::Number::from_f64(batch_progress).unwrap(),
                        );
                        let _ = storage_clone
                            .put(&rotation_key, &serde_json::to_vec(&rotation_meta).unwrap())
                            .await;
            ...
            if let Some(metadata_bytes) = storage_clone.get(&rotation_key).await.unwrap_or_default()
            {
                ...
                {
                    rotation_meta["status"] =
                        serde_json::Value::String("switching_keys".to_string());
                    rotation_meta["progress_percentage"] =
                        serde_json::Value::Number(serde_json::Number::from_f64(90.0).unwrap());
                    let _ = storage_clone
                        .put(&rotation_key, &serde_json::to_vec(&rotation_meta).unwrap())
                        .await;
                }
    ...
    async fn configure_aws_dynamic_secrets(
        &self,
        ctx: &Context<'_>,
        input: ConfigureAwsInput,
    ) -> Result<ApiResponse<bool>> {
        let graphql_ctx = from_context(ctx)?;
    ...
    async fn generate_aws_credentials(
        &self,
        ctx: &Context<'_>,
        input: GenerateAwsCredentialInput,
    ) -> Result<ApiResponse<AwsCredential>> {
        let graphql_ctx = from_context(ctx)?;
        ...
        let credential = dynamic_secrets
            .generate_aws_credentials(&input.path, params)
            .await
            .map_err(|e| {
                async_graphql::Error::new(format!("Failed to generate AWS credentials: {}", e))
            })?;
    ...
    async fn generate_database_credentials(
        &self,
        ctx: &Context<'_>,
        input: GenerateDatabaseCredentialInput,
    ) -> Result<ApiResponse<DatabaseCredential>> {
        let graphql_ctx = from_context(ctx)?;
        ...
        let credential = dynamic_secrets
            .generate_database_credentials(&input.path, params)
            .await
            .map_err(|e| {
                async_graphql::Error::new(format!("Failed to generate database credentials: {}", e))
            })?;
        ...
        let masked_connection_string = credential
            .connection_string
            .split(':')
            .enumerate()
            .map(|(i, part)| {
                if i == 3 && part.contains('@') {
                    ...
                    if password_part.len() > 8 {
                        format!("{}****@{}", &password_part[..4], host_part)
                    } else {
                        format!("****@{}", host_part)
                    }
                } else {
    ...
    async fn renew_credential_lease(
        ...
        let lease_info = dynamic_secrets
            .renew(&input.lease_id, input.increment)
            .await
    ...
    async fn revoke_credential(
        ...
        dynamic_secrets.revoke(&lease_id).await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to revoke credential: {}", e))
        })?;
