...
use crate::graphql::{context::from_context, context::AuthenticatedUser, types::*};
...
impl Query {
    ...
    async fn databases(&self, ctx: &Context<'_>) -> Result<Vec<Database>> {
        ...
        let db_data = storage.batch_get(&db_keys).await.map_err(|e| {
            async_graphql::Error::new(format!("Database batch retrieval failed: {}", e))
        })?;
    ...
    async fn tables(&self, ctx: &Context<'_>, database: String) -> Result<Vec<Table>> {
        ...
        for key in table_keys {
            if let Some(data) = storage.get(&key).await.map_err(|e| {
                ...
                if let Ok(table_info) = serde_json::from_slice::<serde_json::Value>(&data) {
                    if let (Some(name), Some(created_at)) = (
                        ...
                        let table = Table {
                            ...
                            fields: if let Some(fields_array) =
                                table_info.get("fields").and_then(|v| v.as_array())
                            {
                            ...
                            primary_key: if let Some(pk_array) =
                                table_info.get("primary_key").and_then(|v| v.as_array())
                            {
                                pk_array
                                    .iter()
                                    .filter_map(|pk| pk.as_str().map(String::from))
                                    .collect::<Vec<_>>()
                            } else {
                                Vec::new()
                            },
                            created_at: created_at.parse().unwrap_or_else(|_| Utc::now()),
                            updated_at: table_info
                                .get("updated_at")
                                .and_then(|v| v.as_str())
                                .and_then(|s| s.parse().ok())
                                .unwrap_or_else(|| Utc::now()),
    ...
    async fn get_record(
        ...
        if let Some(data) = storage.get(&record_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Record details retrieval failed: {}", e))
        })? {
            if let Ok(record_info) = serde_json::from_slice::<serde_json::Value>(&data) {
    ...
    async fn encryption_metadata(
        ...
        if let Some(data) = storage.get(&metadata_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Encryption metadata retrieval failed: {}", e))
        })? {
    ...
    async fn key_rotation_status(
        ...
        table: String,
        rotation_id: String,
    ) -> Result<Option<KeyRotationStatus>> {
        let graphql_ctx = from_context(ctx)?;
        let storage = &graphql_ctx.app_state.storage;
        ...
        if let Some(data) = storage.get(&rotation_key).await.map_err(|e| {
            async_graphql::Error::new(format!("Rotation status retrieval failed: {}", e))
        })? {
            if let Ok(rotation_info) = serde_json::from_slice::<serde_json::Value>(&data) {
                if let Some(status) = rotation_info.get("status").and_then(|v| v.as_str()) {
                    let rotation_status = KeyRotationStatus {
                        ...
                        progress_percentage: rotation_info
                            .get("progress_percentage")
                            .and_then(|v| v.as_f64())
                            .unwrap_or(0.0),
                        started_at: rotation_info
                            .get("started_at")
                            .and_then(|v| v.as_str())
                            .and_then(|s| s.parse().ok()),
                        completed_at: rotation_info
                            .get("completed_at")
                            .and_then(|v| v.as_str())
                            .and_then(|s| s.parse().ok()),
                        error_message: rotation_info
                            .get("error_message")
                            .and_then(|v| v.as_str())
                            .map(String::from),
                        records_processed: rotation_info
                            .get("records_processed")
                            .and_then(|v| v.as_i64())
                            .unwrap_or(0) as i32,
                        total_records: rotation_info
                            .get("total_records")
                            .and_then(|v| v.as_i64())
                            .unwrap_or(0) as i32,
                    };
    ...
    async fn dynamic_secrets_status(&self, ctx: &Context<'_>) -> Result<DynamicSecretsStatus> {
        ...
        let engine_status = dynamic_secrets.status().await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to get dynamic secrets status: {}", e))
        })?;
        ...
        Ok(DynamicSecretsStatus {
            ...
            supported_databases,
            default_ttl: engine_status
                .config
                .get("default_ttl")
                .and_then(|v| v.as_u64())
                .unwrap_or(3600),
            max_ttl: engine_status
                .config
                .get("max_ttl")
                .and_then(|v| v.as_u64())
                .unwrap_or(86400),
            auto_cleanup: engine_status
                .config
                .get("auto_cleanup")
                .and_then(|v| v.as_bool())
                .unwrap_or(true),
    ...
    async fn list_dynamic_credentials(
        &self,
        ctx: &Context<'_>,
        path: Option<String>,
    ) -> Result<Vec<String>> {
        ...
        dynamic_secrets.list(&search_path).await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to list dynamic credentials: {}", e))
        })
    }
