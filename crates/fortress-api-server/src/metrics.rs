...
impl MetricsCollector {
    ...
    pub async fn get_metrics(&self) -> MetricsResponse {
        ...
        metrics_map.insert(
            ...
            json!(metrics.total_connections),
        );

        // Data metrics
        metrics_map.insert(
            "data_stored_bytes".to_string(),
            json!(metrics.data_stored_bytes),
        );
        metrics_map.insert(
            "data_retrieved_bytes".to_string(),
            json!(metrics.data_retrieved_bytes),
        );

        // Operation metrics
        metrics_map.insert(
            "encryption_operations".to_string(),
            json!(metrics.encryption_operations),
        );
        metrics_map.insert(
            "decryption_operations".to_string(),
            json!(metrics.decryption_operations),
        );
        ...
        metrics_map.insert(
            "rate_limit_violations".to_string(),
            json!(metrics.rate_limit_violations),
        );
    ...
    pub async fn get_prometheus_metrics(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        prometheus_metrics.push(
            "# HELP fortress_http_status_codes HTTP status codes\n\
             # TYPE fortress_http_status_codes counter"
                .to_string(),
        );
        ...
        prometheus_metrics.push(
            "# HELP fortress_errors_total Total errors by type\n\
             # TYPE fortress_errors_total counter"
                .to_string(),
        );
...
mod tests {
    ...
    async fn test_http_request_recording() {
        ...
        collector
            ...
            .await;
        collector
            .record_http_request("POST", "/data", 201, Duration::from_millis(150))
            .await;
