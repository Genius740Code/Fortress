...
use fortress_core::encryption::{Aegis256, Aes256Gcm, ChaCha20Poly1305, EncryptionAlgorithm};
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    black_box(
                        algorithm
                            .encrypt(black_box(&data), black_box(key.as_bytes()))
                            .unwrap(),
                    )
                });
            },
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    black_box(
                        algorithm
                            .decrypt(black_box(&ciphertext), black_box(key.as_bytes()))
                            .unwrap(),
                    )
                });
            },
...
fn bench_key_generation(c: &mut Criterion) {
    ...
    group.bench_function("secure_key_generate_32", |b| {
        b.iter(|| black_box(SecureKey::generate(32)));
    });

    group.bench_function("secure_key_generate_64", |b| {
        b.iter(|| black_box(SecureKey::generate(64)));
    });
...
fn bench_encrypted_data_serialization(c: &mut Criterion) {
    ...
    group.bench_function("to_base64", |b| {
        b.iter(|| black_box(encrypted_data.to_base64().unwrap()));
    });
    ...
    group.bench_function("from_base64", |b| {
        b.iter(|| {
            black_box(
                fortress_core::encryption::EncryptedData::from_base64(black_box(&base64)).unwrap(),
            )
        });
    });
