...
pub trait TenantManager: Send + Sync {
    ...
    ) -> impl std::future::Future<Output = Result<Tenant>> + Send;

    /// Get tenant by ID
    fn get_tenant(
        &self,
        tenant_id: &TenantId,
    ) -> impl std::future::Future<Output = Result<Option<Tenant>>> + Send;
    ...
    fn get_tenant_stats(
        &self,
        tenant_id: &TenantId,
    ) -> impl std::future::Future<Output = Result<TenantStats>> + Send;
}
...
impl ResourceIsolationManager {
    ...
    pub async fn can_create_database(
        &self,
        tenant_id: &TenantId,
        tenant_limits: &TenantResourceLimits,
    ) -> Result<bool> {
    ...
    pub async fn can_allocate_storage(
        &self,
        tenant_id: &TenantId,
    ...
    pub async fn can_create_connection(
        &self,
        tenant_id: &TenantId,
        tenant_limits: &TenantResourceLimits,
    ) -> Result<bool> {
    ...
    pub async fn record_database_creation(&self, tenant_id: &TenantId) -> Result<()> {
        ...
        let usage = usage_map
            .entry(*tenant_id)
            .or_insert_with(TenantResourceUsage::default);
    ...
    pub async fn get_tenant_usage(
        &self,
        tenant_id: &TenantId,
    ) -> Result<Option<TenantResourceUsage>> {
...
mod tests {
    ...
    async fn test_resource_isolation_database_limits() {
        ...
        assert!(isolation
            .can_create_database(&tenant.id, &tenant_limits)
            .await
            .unwrap());

        // Record database creation
        isolation
            .record_database_creation(&tenant.id)
            .await
            .unwrap();
        isolation
            .record_database_creation(&tenant.id)
    ...
    async fn test_resource_isolation_storage_limits() {
        ...
        assert!(isolation
            .can_allocate_storage(&tenant.id, 400, &tenant_limits)
            .await
            .unwrap());

        // Record storage allocation
        isolation
            .record_storage_allocation(&tenant.id, 400)
            .await
            .unwrap();

        // Should not be able to allocate more than the limit
        assert!(!isolation
            .can_allocate_storage(&tenant.id, 200, &tenant_limits)
            .await
            .unwrap());

        // Should be able to allocate within remaining space
        assert!(isolation
            .can_allocate_storage(&tenant.id, 100, &tenant_limits)
            .await
            .unwrap());
    }
    ...
    async fn test_resource_isolation_connection_limits() {
        ...
        assert!(isolation
            .can_create_connection(&tenant.id, &tenant_limits)
            .await
            .unwrap());

        // Record connection creation
        isolation
            .record_connection_creation(&tenant.id)
            .await
            .unwrap();
        isolation
            .record_connection_creation(&tenant.id)
        ...
        isolation
            .record_connection_termination(&tenant.id)
            .await
            .unwrap();
    ...
    async fn test_tenant_stats_integration() {
        ...
        isolation
            ...
            .unwrap();
        isolation
            .record_storage_allocation(&tenant.id, 1024)
            .await
            .unwrap();
        isolation
            .record_connection_creation(&tenant.id)
        ...
        isolation
            .update_memory_usage(&tenant.id, 45.2)
            .await
            .unwrap();
    ...
    async fn test_global_resource_usage() {
        ...
        isolation
            .record_database_creation(&tenant1_id)
            .await
            .unwrap();
        isolation
            .record_database_creation(&tenant2_id)
            .await
            .unwrap();
        isolation
            .record_storage_allocation(&tenant1_id, 500)
            .await
            .unwrap();
        isolation
            .record_storage_allocation(&tenant2_id, 300)
            .await
            .unwrap();
        isolation
            .record_connection_creation(&tenant1_id)
            .await
            .unwrap();
        isolation
            .record_connection_creation(&tenant2_id)
            .await
            .unwrap();
