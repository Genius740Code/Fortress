...
use crate::plugin::{Plugin, PluginContext, PluginInput, PluginMetadata, PluginCapability};
...
async fn test_wasm_runtime_basic() {
    ...
    // Test health check
    let health = plugin.health_check().await.expect("Failed to get health status");
...
async fn test_wasm_config_validation() {
    ...
    let invalid_config = std::collections::HashMap::from([
        ("invalid_function".to_string(), serde_json::Value::String("test".to_string())),
    ]);
