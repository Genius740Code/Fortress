...
impl KeyCache {
    ...
    pub async fn store(
        ...
        let cache_metadata = metadata.unwrap_or_else(|| {
            KeyMetadata::new(
                key_id.clone(),
    ...
    async fn start_background_cleanup(&self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            let mut interval = tokio::time::interval(std::time::Duration::from_secs(
                config.background_cleanup_interval_seconds,
            ));
            ...
            loop {
                ...
                if !expired_keys.is_empty() {
                    ...
                    stats_write.current_memory_bytes =
                        cache_write.values().map(|entry| entry.size_bytes).sum();
    ...
    pub async fn get_performance_recommendations(&self) -> Vec<String> {
        ...
        let memory_usage_ratio =
            stats.current_memory_bytes as f64 / self.config.max_memory_bytes as f64;
        ...
        if stats.size_evictions > stats.hits / 10 {
            recommendations.push(
                "High number of size evictions detected. Consider increasing cache limits."
                    .to_string(),
            );
        }
...
impl std::fmt::Debug for KeyCache {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KeyCache")
            ...
            .field(
                "current_keys",
                &self.cache.try_read().map(|g| g.len()).unwrap_or(0),
            )
            .finish()
    }
}
...
mod tests {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                if i % 2 == 0 {
                    ...
                    let (new_key, new_metadata) =
                        create_test_key_data(&format!("concurrent_new_{}", i), i);
    ...
    async fn test_error_handling() -> Result<()> {
        ...
        let result = cache
            .put("empty_key_test".to_string(), empty_key, metadata)
            .await;
    ...
    async fn test_large_key_handling() -> Result<()> {
        ...
        let result = cache
            .put(String::from("large_key_test"), large_key, metadata)
            .await;
