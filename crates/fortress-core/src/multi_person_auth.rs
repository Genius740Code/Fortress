...
impl MultiPersonAuthManager {
    ...
    pub fn add_control_group_member(
        ...
        if group.members.iter().any(|m| m.user_id == user_id) {
            return Err(FortressError::validation(
                "User is already a member of this control group",
                None,
                None,
            ));
        }
    ...
    pub fn create_approval_request(
        ...
        if !group.authorized_operations.contains(&operation_type) {
            return Err(FortressError::validation(
                "Control group is not authorized for this operation type",
                None,
                None,
            ));
        }
        ...
        let active_members = group.members.iter().filter(|m| m.active).count();
        ...
        self.approval_requests
            .insert(request_id.clone(), approval_request);
        ...
        self.control_groups
            ...
            .map(|group| {
                group
                    .members
                    .iter()
                    .any(|m| &m.user_id == user_id && m.active)
            })
            .unwrap_or(false)
    }
