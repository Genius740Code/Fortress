...
impl MemoryMonitor {
    ...
    async fn check_alerts(&self, usage_percentage: f64) -> Result<Option<MemoryAlert>> {
        ...
        if usage_percentage >= thresholds.emergency_threshold {
            return Ok(Some(MemoryAlert::Emergency {
                usage_percentage,
                message: format!(
                    "Memory usage at {:.1}% exceeds emergency threshold of {:.1}%",
            ...
            return Ok(Some(MemoryAlert::Critical {
                usage_percentage,
                message: format!(
                    "Memory usage at {:.1}% exceeds critical threshold of {:.1}%",
            ...
            return Ok(Some(MemoryAlert::Warning {
                usage_percentage,
                message: format!(
                    "Memory usage at {:.1}% exceeds warning threshold of {:.1}%",
    ...
    pub async fn get_usage_history(
        &self,
        limit: Option<usize>,
    ) -> Result<Vec<MemoryUsageSnapshot>> {
        let history = self.usage_history.read().await;
        match limit {
            Some(limit) => {
    ...
    pub async fn get_memory_pressure(&self) -> Result<MemoryPressure> {
        let usage_percentage = self.current_usage.load(Ordering::Relaxed) as f64
            / (self.max_memory_mb * 1024 * 1024) as f64;
