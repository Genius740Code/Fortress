...
async fn test_complete_mpa_workflow() {
    ...
    let pending_requests = mpa_service
        .get_pending_requests_for_user(approver1_id.clone())
        .await
        .unwrap();
    ...
    let pending_requests = mpa_service
        .get_pending_requests_for_user(approver2_id.clone())
        .await
        .unwrap();
...
async fn test_request_cancellation() {
    ...
    mpa_service
        .cancel_approval_request(&request_id, requester_id.clone())
        .await
        .unwrap();
...
async fn test_control_group_management() {
    ...
    let groups = mpa_service
        .get_control_groups_for_user(approver1_id.clone())
        .await
        .unwrap();
    ...
    let approver_member = group
        .members
        .iter()
        .find(|m| m.user_id == approver1_id)
        .unwrap();
...
async fn test_custom_operation_types() {
    ...
    match request.operation_type {
        ...
        _ => assert!(
            false,
            "Expected custom operation type, got {:?}",
            request.operation_type
        ),
    }
