...
use crate::storage::{FileSystemStorage, InMemoryStorage, StorageBackend};
use base64::Engine;
...
impl KvEngine {
    ...
    pub fn with_config(config: KvConfig) -> Result<Self> {
        ...
        let master_key = if let Some(key_str) = &config.master_key {
            base64::engine::general_purpose::STANDARD
                .decode(key_str)
                .map_err(|e| {
                    FortressError::secrets(format!(
                        "Failed to decode master key from base64: {}",
                        e
                    ))
                })?
        } else {
        ...
        let storage: Arc<dyn StorageBackend> = match config.storage_backend.as_deref() {
            ...
            Some("file") => FileSystemStorage::new("data/secrets")
                .map(|fs| Arc::new(fs) as Arc<dyn StorageBackend>)
                .map_err(|e| {
                    FortressError::secrets(format!("Failed to create file storage: {}", e))
                })?,
            #[cfg(feature = "cloud-storage")]
            Some("s3") => {
            ...
            Some("s3") => {
                return Err(FortressError::secrets(
                    "S3 storage requested but cloud-storage feature not enabled".to_string(),
                ));
            }
            Some(backend) => {
    ...
    async fn barrier_encrypt_and_serialize(&self, data: &serde_json::Value) -> Result<Vec<u8>> {
        ...
        if !config.encryption_at_rest {
            ...
            return serde_json::to_vec(data).map_err(|e| {
                FortressError::secrets(format!("Failed to serialize secret data: {}", e))
            });
        }
    ...
    async fn barrier_decrypt_and_deserialize(
        &self,
        encrypted_data: &[u8],
        ...
        if !config.encryption_at_rest {
            ...
            return serde_json::from_slice(encrypted_data).map_err(|e| {
                FortressError::secrets(format!("Failed to deserialize secret data: {}", e))
            });
        }
        ...
        let decrypted_json: serde_json::Value =
            serde_json::from_slice(&decrypted_bytes).map_err(|e| {
                FortressError::secrets(format!("Failed to deserialize decrypted data: {}", e))
            })?;
    ...
    async fn barrier_store_secret(&self, path: &str, entry: &KvSecretEntry) -> Result<()> {
        ...
        let serialized_entry = serde_json::to_vec(entry).map_err(|e| {
            FortressError::secrets(format!("Failed to serialize secret entry: {}", e))
        })?;
        ...
        let encrypted_entry = self
            .encryption
            .encrypt(&serialized_entry, &master_key)
            .map_err(|e| {
                FortressError::secrets(format!("Failed to encrypt secret entry: {}", e))
            })?;

        // Store encrypted data in backend
        let storage_key = format!("kv/secrets/{}", path);
        self.storage
            .put(&storage_key, &encrypted_entry)
            .await
            .map_err(|e| {
                FortressError::secrets(format!("Failed to store encrypted secret: {}", e))
            })?;
    ...
    async fn barrier_load_secret(&self, path: &str) -> Result<Option<KvSecretEntry>> {
        ...
        if let Some(encrypted_data) = maybe_cached_data {
            ...
            let decrypted_data = self
                ...
                .decrypt(&encrypted_data, &master_key)
                .map_err(|e| {
                    FortressError::secrets(format!("Failed to decrypt cached secret: {}", e))
                })?;

            let entry: KvSecretEntry = serde_json::from_slice(&decrypted_data).map_err(|e| {
                FortressError::secrets(format!("Failed to deserialize cached secret: {}", e))
            })?;
        ...
        if let Some(encrypted_data) = self.storage.get(&storage_key).await.map_err(|e| {
            FortressError::secrets(format!("Failed to load secret from storage: {}", e))
        })? {
            ...
            let decrypted_data = self
                ...
                .decrypt(&encrypted_data, &master_key)
                .map_err(|e| {
                    FortressError::secrets(format!("Failed to decrypt secret data: {}", e))
                })?;

            // Deserialize the decrypted entry
            let entry: KvSecretEntry = serde_json::from_slice(&decrypted_data).map_err(|e| {
                FortressError::secrets(format!("Failed to deserialize secret entry: {}", e))
            })?;
    ...
    pub async fn initialize_from_storage(&self) -> Result<()> {
        ...
        let keys = self.storage.list_prefix(prefix).await.map_err(|e| {
            FortressError::secrets(format!("Failed to list secrets from storage: {}", e))
        })?;
        ...
        if loaded_count > 0 {
            log::info!(
                "Loaded {} secret metadata from encrypted storage",
                loaded_count
            );
        }
    ...
    async fn decrypt_secret_data(
        ...
        let mut encrypted_bytes = base64::engine::general_purpose::STANDARD
            ...
            .map_err(|e| {
                FortressError::secrets(format!("Failed to decode encrypted data: {}", e))
            })?;
    ...
    pub async fn destroy(&self, path: &str) -> Result<()> {
        ...
        let lease_ids: Vec<String> = leases
            .keys()
        ...
        let storage_key = format!("kv/secrets/{}", path);
        self.storage.delete(&storage_key).await.map_err(|e| {
            FortressError::secrets(format!("Failed to delete secret from storage: {}", e))
        })?;
...
impl SecretsEngine for KvEngine {
    ...
    async fn write(&self, path: &str, data: &serde_json::Value) -> Result<Secret> {
        ...
        let outcome = if store_result.is_ok() {
            "success"
        } else {
            "failure"
        };
        if let Err(e) = self
            .audit_logger
            .log_secret_write(principal, path, version, outcome)
            .await
        {
    ...
    async fn read(&self, path: &str) -> Result<Option<Secret>> {
        ...
        if let Err(e) = self
            .audit_logger
            .log_access(principal, path, "read", outcome)
            .await
        {
            log::warn!("Failed to log audit entry: {}", e);
        }
    ...
    async fn delete(&self, path: &str) -> Result<()> {
        ...
        let outcome = if storage_result.is_ok() {
            "success"
        } else {
            "failure"
        };
        if let Err(e) = self
            .audit_logger
            .log_secret_delete(principal, path, outcome)
            .await
        {
            log::warn!("Failed to log audit entry: {}", e);
        }
    ...
    async fn list(&self, path: &str) -> Result<Vec<String>> {
        ...
        if let Err(e) = self
            .audit_logger
            .log_access(principal, path, "list", outcome)
            .await
        {
            log::warn!("Failed to log audit entry: {}", e);
        }
...
mod tests {
    ...
    async fn test_persistence_across_restarts() {
        ...
        engine1
            .write("persistent/secret", &secret_data)
            .await
            .unwrap();
    ...
    async fn test_encryption_decryption_roundtrip() {
        ...
        let decrypted = engine
            .decrypt_secret_data(&encrypted, &nonce)
            .await
            .unwrap();
    ...
    async fn test_kv_encryption_basic() {
        ...
        assert!(
            retrieved_secret.is_ok(),
            "Failed to read secret: {:?}",
            retrieved_secret.err()
        );
    ...
    async fn test_kv_no_encryption() {
        ...
        assert!(
            retrieved_secret.unwrap().is_some(),
            "Expected to find secret but got None"
        );
