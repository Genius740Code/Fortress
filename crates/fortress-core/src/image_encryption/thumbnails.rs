...
use crate::image_encryption::{encryptor::ImageEncryptorFactory, ImageFormat};
...
impl ThumbnailGenerator {
    ...
    pub async fn generate_from_decrypted(
        ...
        metadata.insert(
            ...
            format!("{:?}", encrypted_image.format_info.format),
        );

        metadata.insert(
            "original_size".to_string(),
            encrypted_image.original_size.to_string(),
        );

        metadata.insert(
            "thumbnail_size".to_string(),
            format!("{}x{}", thumbnail_dimensions.0, thumbnail_dimensions.1),
        );
        ...
        metadata.insert(
            ...
            format!(
                "{:.3}",
                thumbnail_data.len() as f64 / image_data.len() as f64
            ),
        );
    ...
    async fn generate_thumbnail_data(
        &self,
        _image_data: &[u8],
        options: &ThumbnailOptions,
    ) -> Result<Vec<u8>> {
    ...
    async fn create_placeholder_thumbnail(&self, options: &ThumbnailOptions) -> Result<Vec<u8>> {
        ...
        match options.format {
            ThumbnailFormat::Jpeg => {
                ...
                self.create_jpeg_placeholder(width, height, options.quality.unwrap_or(75))
                    .await
            }
            ...
            ThumbnailFormat::WebP => {
                ...
                self.create_webp_placeholder(width, height, options.quality.unwrap_or(75))
                    .await
            }
        }
    }
    ...
    async fn create_jpeg_placeholder(
        &self,
        width: u32,
        height: u32,
        _quality: u8,
        ...
        for _ in 0..image_data_size.min(1000) {
    ...
    async fn create_webp_placeholder(
        &self,
        width: u32,
        height: u32,
        _quality: u8,
    ...
    pub async fn generate_multiple_thumbnails(
        ...
        for size in sizes {
            ...
            let thumbnail = self
                .generate_from_encrypted(encrypted_image, thumbnail_key, image_key, options)
                .await?;
    ...
    pub fn estimate_thumbnail_size(
        &self,
    ...
    pub fn verify_thumbnail_integrity(
        &self,
        encrypted_thumbnail: &EncryptedThumbnail,
    ) -> Result<bool> {
...
mod tests {
    ...
    async fn test_placeholder_thumbnail_generation() {
        ...
        let thumbnail_data = generator
            .create_placeholder_thumbnail(&options)
            .await
            .unwrap();
    ...
    fn test_thumbnail_integrity_verification() {
        ...
        assert!(generator
            .verify_thumbnail_integrity(&valid_thumbnail)
            .unwrap());
        ...
        assert!(!generator
            .verify_thumbnail_integrity(&invalid_thumbnail)
            .unwrap());
