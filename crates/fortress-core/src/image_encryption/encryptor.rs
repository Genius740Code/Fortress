...
use crate::image_encryption::{
    ...
    metadata::{DefaultMetadataExtractor, MetadataExtractor, MetadataProcessor},
    EncryptedImage, EncryptionMode, EncryptionOptions, EncryptionStats, ImageEncryptionError,
    ImageFormat, ImageFormatDetector, ImageFormatInfo, ImageMetadata,
};
...
impl ImageEncryptor {
    ...
    pub async fn encrypt(
        ...
        if !format.validate_data(&image_data)? {
            return Err(ImageEncryptionError::CorruptedData(
                "Invalid image data for detected format".to_string(),
            )
            .into());
        }
        ...
        .with_metadata(
            ...
            format!("{:?}", options.encryption_mode),
        );
        ...
        additional_info.insert(
            "encryption_time_ms".to_string(),
            stats.encryption_time_ms.to_string(),
        );
        ...
        additional_info.insert(
            ...
            stats.encrypted_size.to_string(),
        );
    ...
    pub async fn decrypt(
        ...
        {
            return Err(ImageEncryptionError::CorruptedData(
                "Decrypted data validation failed".to_string(),
            )
            .into());
        }
    ...
    pub async fn decrypt_metadata(
        ...
        if let Some(encrypted_metadata) = &encrypted_image.metadata {
            let metadata = self
                .metadata_processor
                .decrypt_metadata(encrypted_metadata, key)?;
    ...
    fn reconstruct_image_from_data_only(
        ...
        if data.len() <= header_size {
            return Err(ImageEncryptionError::CorruptedData(
                "Encrypted data too small for reconstruction".to_string(),
            )
            .into());
        }
    ...
    fn jpeg_format_aware_processing(
        &self,
        data: &[u8],
        ...
        while pos < data.len() {
            ...
            if data[pos] != 0xFF {
                return Err(
                    ImageEncryptionError::CorruptedData("Invalid JPEG marker".to_string()).into(),
                );
            }
            ...
            match marker {
                ...
                0xDA => {
                    ...
                    let length =
                        u16::from_be_bytes([data[scan_start], data[scan_start + 1]]) as usize;
    ...
    fn reverse_jpeg_format_aware_processing(
        &self,
        data: &[u8],
        _options: &EncryptionOptions,
    ) -> Result<Vec<u8>> {
    ...
    fn png_format_aware_processing(
        &self,
        data: &[u8],
        _options: &EncryptionOptions,
    ) -> Result<Vec<u8>> {
    ...
    fn reverse_png_format_aware_processing(
        &self,
        data: &[u8],
        _options: &EncryptionOptions,
    ) -> Result<Vec<u8>> {
    ...
    fn tiff_format_aware_processing(
        &self,
        data: &[u8],
        _options: &EncryptionOptions,
    ) -> Result<Vec<u8>> {
    ...
    fn reverse_tiff_format_aware_processing(
        &self,
        data: &[u8],
        _options: &EncryptionOptions,
    ) -> Result<Vec<u8>> {
        ...
        if options.algorithm != self.encryption_algorithm.name() {
            return Err(FortressError::encryption(
                format!(
                    ...
                    self.encryption_algorithm.name(),
                    options.algorithm
                ),
    ...
    pub fn get_encryption_stats(&self, encrypted_image: &EncryptedImage) -> EncryptionStats {
        EncryptionStats {
            ...
            encryption_time_ms: encrypted_image
                .additional_info
