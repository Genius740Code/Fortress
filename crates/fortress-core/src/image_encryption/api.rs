...
use crate::image_encryption::{EncryptedImage, EncryptionOptions, ImageFormatInfo};

use crate::image_encryption::{
    encryptor::ImageEncryptorFactory,
    streaming::{StreamingImageDecryptor, StreamingResult, StreamingState},
    thumbnails::ThumbnailOptions,
    AccessPermissions, ChunkConfig, EncryptedThumbnail, ImageEncryptor, ImageFilter, ImageInfo,
    ImageMetadata, ImageSearchResult, StreamingImageEncryptor, ThumbnailGenerator,
};
...
impl ImageEncryptionService {
    ...
    pub fn new(
        ...
        let thumbnail_generator = Arc::new(ThumbnailGenerator::new(
            ImageEncryptorFactory::create_encryptor(algorithm_name)?
    ...
    pub async fn encrypt_image(
        ...
        let key = if let Some(key_id) = &request.key_id {
            ...
            self.key_manager
                .generate_key(&request.options.algorithm)
                .await?
        };

        // Encrypt the image

        let encrypted_image = self
            .encryptor
        ...
        let thumbnails = if !request.thumbnail_options.is_empty() {
            self.generate_thumbnails_for_image(&encrypted_image, &key, &request.thumbnail_options)
                .await?
        } else {
            Vec::new()
        };
    ...
    pub async fn decrypt_image(
        ...
        let metadata = if request.include_metadata {
            self.encryptor
                .decrypt_metadata(&encrypted_image, &key)
                .await?
        } else {
            None
        };
        ...
        tokio::spawn(async move {
            ...
            match result {
                Ok(streaming_result) => {
                    ...
                    if let Err(e) = image_store
                        .store_streaming_result(&session_id_clone, &streaming_result)
                        .await
                    {
    ...
    pub async fn get_streaming_status(
        ...
        let status = self
            .streaming_encryptor
            .get_session_status(&request.session_id)
            .await?;
    ...
    pub async fn delete_image(&self, request: DeleteImageRequest) -> Result<DeleteImageResponse> {
        ...
        let deleted_thumbnails = self
            .image_store
            .delete_thumbnails(&request.image_id)
            .await?;
    ...
    async fn generate_thumbnails_for_image(
        ...
        let image_key = self
            .key_manager
            .generate_key(&encrypted_image.encryption_options.algorithm)
            .await?;

        for options in thumbnail_options {
            let thumbnail = self
                .thumbnail_generator
                .generate_from_encrypted(
                    encrypted_image,
                    thumbnail_key,
                    &image_key,
                    options.clone(),
    ...
    async fn store_thumbnails(
        ...
        for thumbnail in thumbnails {
            let thumbnail_id = self
                .image_store
                .store_thumbnail(image_id, &thumbnail)
                .await?;
