...
impl ImageFormat {
    ...
    pub fn validate_data(&self, data: &[u8]) -> Result<bool> {
        ...
        let is_valid = match self {
            ...
            Self::Dicom => data.len() >= 132 && &data[128..132] == b"DICM",
    ...
    pub fn supports_thumbnails(&self) -> bool {
        match self {
            ...
            | Self::Dng
            | Self::Fits
            | Self::Nifti => true, // With special handling
            Self::Unknown => false,
        }
    }
}
...
impl ImageFormatDetector {
    ...
    fn is_cr2(data: &[u8]) -> bool {
        ...
        data.len() > 16 && data[8] == 0x43 && data[9] == 0x52 && data[10] == 0x32
        // "CR2"
    }
    ...
    fn is_nifti(data: &[u8]) -> bool {
        data.len() >= 348
         ...
         (data[344] == 0x6E && data[345] == 0x69 && data[346] == 0x32 && data[347] == 0x00))
        // "ni2\0"
    }
}
            ...
            match chunk_type {
                ...
                b"zTXt" => {
                    ...
                    if let Some(null_pos) = chunk_data.iter().position(|&b| b == 0) {
                        if null_pos > 0 {
                            let keyword = String::from_utf8_lossy(&chunk_data[..null_pos]);
                            metadata
                                .insert(format!("png_ztxt_{}", keyword), "compressed".to_string());
                        }
                    }
                }
                b"tIME" => {
                ...
                b"pHYs" => {
                    ...
                    if chunk_length >= 9 {
                        ...
                        let pixels_per_unit_y = u32::from_be_bytes([
                            chunk_data[4],
                            chunk_data[5],
                            chunk_data[6],
                            chunk_data[7],
                        ...
                        metadata.insert(
                            "png_pixels_per_unit_x".to_string(),
                            pixels_per_unit_x.to_string(),
                        );
                        metadata.insert(
                            "png_pixels_per_unit_y".to_string(),
                            pixels_per_unit_y.to_string(),
                        );
                        metadata.insert(
                            "png_unit".to_string(),
                            if unit == 1 {
                ...
                b"sRGB" => {
                    ...
                    if chunk_length >= 1 {
                        ...
                        metadata.insert(
                            "png_rendering_intent".to_string(),
...
mod tests {
    ...
    fn test_format_detection() {
        ...
        assert_eq!(
            ImageFormatDetector::detect(&jpeg_data).unwrap(),
            ImageFormat::Jpeg
        );
        ...
        assert_eq!(
            ImageFormatDetector::detect(&png_data).unwrap(),
            ImageFormat::Png
        );
        ...
        assert_eq!(
            ImageFormatDetector::detect(&bmp_data).unwrap(),
            ImageFormat::Bmp
        );
    }
