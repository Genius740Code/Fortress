...
impl MetadataProcessor {
    ...
    pub fn encrypt_metadata(
        ...
        searchable_fields.insert(
            "format".to_string(),
            format!("{:?}", metadata.technical_info.format),
        );
        ...
        searchable_fields.insert(
            ...
            format!("{:?}", metadata.security_info.classification),
        );
        ...
        if !metadata.basic_info.keywords.is_empty() {
            searchable_fields.insert(
                "keywords".to_string(),
                metadata.basic_info.keywords.join(","),
            );
        }
    ...
    pub fn decrypt_metadata(
        ...
        let calculated_checksum =
            self.calculate_checksum(&encrypted_metadata.encrypted_metadata.ciphertext);
        ...
        let metadata: ImageMetadata = serde_json::from_slice(&decrypted_bytes).map_err(|e| {
            FortressError::encryption(
    ...
    fn update_field_recursive(
        ...
        match parts.as_slice() {
            ["basic_info", field] => {
                self.update_basic_info_field(&mut metadata.basic_info, *field, new_value)
            }

            ["technical_info", field] => {
                self.update_technical_info_field(&mut metadata.technical_info, *field, new_value)
            }

            ["security_info", field] => {
                self.update_security_info_field(&mut metadata.security_info, *field, new_value)
            }

            ["business_info", field] => {
                self.update_business_info_field(&mut metadata.business_info, *field, new_value)
            }

            ["custom_fields", field_name] => {
                metadata
                    .custom_fields
                    .insert(field_name.to_string(), new_value);
    ...
    fn update_basic_info_field(
        &self,
        basic_info: &mut BasicImageInfo,
        field: &str,
        new_value: EncryptedValue,
    ) -> Result<()> {
        match field {
    ...
    fn update_technical_info_field(
        &self,
        technical_info: &mut TechnicalInfo,
        field: &str,
        new_value: EncryptedValue,
    ) -> Result<()> {
        match field {
            ...
            "resolution_x" => {
                if new_value.value_type == ValueType::Integer {
                    ...
                    technical_info.resolution = Some((300, current_resolution.1));
                // Default fallback
                } else {
                    return Err(FortressError::encryption(
            ...
            "resolution_y" => {
                if new_value.value_type == ValueType::Integer {
                    ...
                    technical_info.resolution = Some((current_resolution.0, 300));
                // Default fallback
                } else {
                    return Err(FortressError::encryption(
    ...
    fn update_security_info_field(
        &self,
        security_info: &mut SecurityMetadata,
        field: &str,
        new_value: EncryptedValue,
    ) -> Result<()> {
        match field {
    ...
    fn update_business_info_field(
        &self,
        business_info: &mut BusinessInfo,
        field: &str,
        new_value: EncryptedValue,
    ) -> Result<()> {
        match field {
            ...
            "approval_status" => {
                ...
                match status_str.to_lowercase().as_str() {
                    ...
                    _ => return Err(FortressError::encryption(
                        "Invalid approval status, expected: pending, approved, rejected, expired"
                            .to_string(),
                        "metadata_processor".to_string(),
                        crate::error::EncryptionErrorCode::EncryptionFailed,
                    )),
            ...
            "license_start_date" => {
                ...
                if let Some(ref mut license) = business_info.license {
                    if let Ok(date) = chrono::DateTime::parse_from_rfc3339(&date_str) {
                        ...
                        return Err(FortressError::encryption(
                            "Invalid date format for license_start_date, expected RFC3339"
                                .to_string(),
                            "metadata_processor".to_string(),
                            crate::error::EncryptionErrorCode::EncryptionFailed,
                        ));
            ...
            "license_end_date" => {
                ...
                if let Some(ref mut license) = business_info.license {
                    if let Ok(date) = chrono::DateTime::parse_from_rfc3339(&date_str) {
                        ...
                        return Err(FortressError::encryption(
                            "Invalid date format for license_end_date, expected RFC3339"
                                .to_string(),
                            "metadata_processor".to_string(),
                            crate::error::EncryptionErrorCode::EncryptionFailed,
                        ));
...
mod tests {
    ...
    fn test_metadata_encryption() {
        ...
        assert_eq!(
            decrypted.technical_info.format,
            metadata.technical_info.format
        );
    }
