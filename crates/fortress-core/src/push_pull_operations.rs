...
use crate::postgres_database::{PostgresBulkEntry, PostgresKeyDatabase, PostgresQuery};
...
impl PushPullManager {
    ...
    async fn perform_push(
        ...
        let source_data = match &request.source {
            StorageSource::Mongo { config } => {
                ...
                self.extract_data_from_mongo(&mongo_db, &request.filter)
                    .await?
            }
            ...
            StorageSource::Postgres { config } => {
                ...
                self.extract_data_from_postgres(&postgres_db, &request.filter)
                    .await?
            }
            ...
            StorageSource::Fortress { storage_id } => {
                ...
                self.extract_data_from_fortress(storage_id, &request.filter.clone().into())
                    .await?
            }
        ...
        match &request.target {
            StorageTarget::Mongo { config } => {
                ...
                self.push_data_to_mongo(&mongo_db, source_data, result, cancel_rx)
                    .await?;
            }
            ...
            StorageTarget::Postgres { config } => {
                ...
                self.push_data_to_postgres(&postgres_db, source_data, result, cancel_rx)
                    .await?;
            }
            ...
            StorageTarget::Fortress { storage_id } => {
                ...
                let universal_records: Vec<UniversalRecord> = source_data
                    .into_iter()
                    .map(|(id, data, metadata)| UniversalRecord {
                        id,
                ...
                self.push_data_to_fortress(storage_id, universal_records, result, cancel_rx)
                    .await?;
            }
    ...
    async fn perform_pull(
        ...
        let source_data = match &request.source {
            StorageSource::Mongo { config } => {
                ...
                self.extract_data_from_mongo(&mongo_db, &request.filter.clone().into())
                    .await?
            }
            ...
            StorageSource::Postgres { config } => {
                ...
                self.extract_data_from_postgres(&postgres_db, &request.filter.clone().into())
                    .await?
            }
    ...
    async fn extract_data_from_mongo(
        ...
        let mongo_filter = match filter {
            ...
            PushFilter::DateRange { start, end } => MongoPullFilter::DateRange {
                start: *start,
                end: *end,
            },

            PushFilter::SizeRange {
                min_bytes,
                max_bytes,
            } => MongoPullFilter::SizeRange {
                min_size: *min_bytes as i64,
                max_size: *max_bytes as i64,
            },

            PushFilter::Metadata { key, value } => MongoPullFilter::Metadata {
                key: key.clone(),
                value: value.clone(),
            },
    ...
    async fn push_data_to_postgres(
        ...
        let postgres_entries: Vec<PostgresBulkEntry> = data
            .into_iter()
    ...
    pub async fn get_operation_status(&self, operation_id: &str) -> Option<PushPullResult> {
        ...
        active
            .get(operation_id)
            .and_then(|handle| handle.result.clone())
    }
...
impl From<PullFilter> for MongoPullFilter {
    fn from(pull_filter: PullFilter) -> Self {
        match pull_filter {
            ...
            PullFilter::Incremental { since } => MongoPullFilter::DateRange {
                start: since,
                end: Utc::now(),
            },
...
mod tests {
    ...
    async fn test_postgres_to_mongo_pull() {
        ...
        let pull_request = PullRequest {
            ...
            source: StorageSource::Postgres {
                config: postgres_config,
            },

            target: StorageTarget::Mongo {
                config: mongo_config,
            },
...
impl PushPullManager {
    ...
    async fn extract_data_from_fortress(
        ...
        for i in 0..10 {
            ...
            let serialized_data = serde_json::to_vec(&record_data).map_err(|e| {
            ...
            records.push((format!("fortress_record_{}", i), serialized_data, metadata));
        }
    ...
    async fn push_data_to_fortress(
        ...
        for (index, _record) in records.into_iter().enumerate() {
            ...
            if self.config.enable_progress && index % 100 == 0 {
                ...
                tracing::info!(
                    "Fortress storage push progress: {:.1}% ({}/{})",
                    progress,
                    processed_records,
                    total_records
                );
        ...
        tracing::info!(
            "Successfully pushed {} records to Fortress storage",
            processed_records
        );
