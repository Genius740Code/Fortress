...
impl TeeIntegrationTests {
    ...
    pub async fn run_all_tests(&self) -> TestResults {
        ...
        results.add_result(
            "tee_manager_aws_nitro".to_string(),
            self.test_tee_manager_aws_nitro().await,
        );

        // Test 2: TEE Manager with Intel SGX
        results.add_result(
            "tee_manager_intel_sgx".to_string(),
            self.test_tee_manager_intel_sgx().await,
        );

        // Test 3: Secure Communication Protocol
        results.add_result(
            "secure_communication".to_string(),
            self.test_secure_communication().await,
        );

        // Test 4: Attestation Verification
        results.add_result(
            "attestation_verification".to_string(),
            self.test_attestation_verification().await,
        );

        // Test 5: TEE-Aware Key Management
        results.add_result(
            "tee_key_management".to_string(),
            self.test_tee_key_management().await,
        );

        // Test 6: End-to-End Workflow
        results.add_result(
            "end_to_end_workflow".to_string(),
        ...
        results.add_result(
            "performance_scalability".to_string(),
    ...
    async fn test_secure_communication(&self) -> TestResult {
        ...
        details.insert(
            ...
            process_result.is_ok().to_string(),
        );
    ...
    async fn test_attestation_verification(&self) -> TestResult {
        ...
        let nitro_result = verifier
            .verify_nitro_attestation(&nitro_document, &self.config.security_policy)
            .await;
        ...
        let sgx_result = verifier
            .verify_sgx_attestation(&sgx_quote, &self.config.security_policy, Some("test-nonce"))
            .await;

        let mut details = HashMap::new();
        details.insert(
            "nitro_attestation_attempted".to_string(),
            "true".to_string(),
        );
        details.insert("sgx_attestation_attempted".to_string(), "true".to_string());
        details.insert(
            "nitro_verification_passed".to_string(),
            nitro_result.is_ok().to_string(),
        );
        details.insert(
            "sgx_verification_passed".to_string(),
            sgx_result.is_ok().to_string(),
        );
    ...
    async fn test_tee_key_management(&self) -> TestResult {
        ...
        let enclave_creation_result = key_manager
            .create_key_enclave(
                TeeType::AwsNitro,
    ...
    async fn test_end_to_end_workflow(&self) -> TestResult {
        ...
        details.insert(
            "nitro_provider_registered".to_string(),
            nitro_registration.is_ok().to_string(),
        );
        details.insert(
            "sgx_provider_registered".to_string(),
            sgx_registration.is_ok().to_string(),
        );
        ...
        details.insert(
            "nitro_capabilities_available".to_string(),
            nitro_caps.is_some().to_string(),
        );
        details.insert(
            "sgx_capabilities_available".to_string(),
            sgx_caps.is_some().to_string(),
        );

        // Test channel listing
        let active_channels = protocol_handler.list_active_channels().await;
        details.insert(
            "active_channels_count".to_string(),
            format!("{}", active_channels.len()),
        );
    ...
    async fn test_error_handling(&self) -> TestResult {
        ...
        let mut details = HashMap::new();
        details.insert(
            "invalid_enclave_handled".to_string(),
            invalid_status.is_err().to_string(),
        );
        details.insert(
            "no_active_channels".to_string(),
            format!("{}", channels.is_empty()),
        );
        ...
        let verification_result = verifier
            .verify_nitro_attestation(&invalid_document, &self.config.security_policy)
            .await;

        details.insert(
            "invalid_document_handled".to_string(),
            verification_result.is_err().to_string(),
        );
    ...
    async fn test_performance_scalability(&self) -> TestResult {
        ...
        let mut details = HashMap::new();
        details.insert(
            "channels_created".to_string(),
            format!("{}", channels.len()),
        );
        details.insert(
            "messages_created".to_string(),
            format!("{}", message_results.len()),
        );
        details.insert(
            "successful_messages".to_string(),
            format!("{}", message_results.iter().filter(|r| r.is_ok()).count()),
        );
        details.insert(
            "channels_cleaned_up".to_string(),
            format!("{}", cleanup_count),
        );

        let success_rate = message_results.iter().filter(|r| r.is_ok()).count() as f64
            / message_results.len() as f64;
