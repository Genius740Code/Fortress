                ...
                if let Some(ext) = path.extension() {
                    if ext == "rs" {
                        ...
                        let content = std::fs::read_to_string(path)
                            .map_err(|e| FortressError::GeneralError(format!("Failed to read file {}: {}", path.display(), e)))?;

                        for pattern in patterns {
                            if content.contains(pattern) {
                                println!("  🚨 Found vulnerable pattern '{}' in file: {}", pattern, path.display());
            ...
            if path.is_file() {
                if let Some(ext) = path.extension() {
                    if ext == "rs" {
                        ...
                        for pattern in patterns {
                            if content.contains(pattern) {
                                println!("  ✅ Found expected pattern '{}' in file: {}", pattern, path.display());
...
impl SecurityRegressionTests {
    ...
    pub fn test_no_weak_random_generation() -> Result<()> {
        ...
        assert!(codebase_safe, "Codebase should not use weak random number generation (e.g., rand::thread_rng())");
    ...
    pub fn test_input_validation_present() -> Result<()> {
        ...
        assert!(validation_present, "Codebase should have proper input validation (missing patterns: {:?})", validation_patterns);
    ...
    pub fn test_csrf_protection_present() -> Result<()> {
        ...
        assert!(csrf_present, "Codebase should have CSRF protection (missing patterns: {:?})", csrf_patterns);
    ...
    pub fn test_rate_limiting_present() -> Result<()> {
        ...
        let scanner = CodebaseScanner::new("../../");
        let rate_limiting_present = scanner.scan_for_presence(&rate_limit_patterns, EXCLUDE_PATHS)?;

        assert!(rate_limiting_present, "Codebase should have rate limiting (missing patterns: {:?})", rate_limit_patterns);
    ...
    pub fn test_security_headers_present() -> Result<()> {
        ...
        let scanner = CodebaseScanner::new("../../");
        let security_headers_present = scanner.scan_for_presence(&security_header_patterns, EXCLUDE_PATHS)?;

        assert!(security_headers_present, "Codebase should have security headers (missing patterns: {:?})", security_header_patterns);
    ...
    pub fn test_constant_time_comparisons() -> Result<()> {
        ...
        let scanner = CodebaseScanner::new("../../");
        let constant_time_present = scanner.scan_for_presence(&constant_time_patterns, EXCLUDE_PATHS)?;

        assert!(constant_time_present, "Codebase should use constant-time comparisons (missing patterns: {:?})", constant_time_patterns);
    ...
    pub fn test_proper_error_handling() -> Result<()> {
        ...
        let scanner = CodebaseScanner::new("../../");
        let proper_error_handling = scanner.scan_for_presence(&error_handling_patterns, EXCLUDE_PATHS)?;

        assert!(proper_error_handling, "Codebase should have proper error handling (missing patterns: {:?})", error_handling_patterns);
    ...
    pub fn test_no_unwrap_calls() -> Result<()> {
        ...
        assert!(no_unwrap_in_production, "Production code should not contain unwrap() or expect() calls");
    ...
    pub fn test_secure_session_management() -> Result<()> {
        ...
        assert!(secure_sessions, "Codebase should have secure session management (missing patterns: {:?})", session_patterns);
    ...
    pub fn test_cryptographic_best_practices() -> Result<()> {
        ...
        let scanner = CodebaseScanner::new("../../");
        let good_practices_present = scanner.scan_for_presence(&crypto_good_patterns, EXCLUDE_PATHS)?;
        ...
        assert!(good_practices_present, "Codebase should follow cryptographic best practices (missing patterns: {:?})", crypto_good_patterns);
        assert!(no_anti_patterns, "Codebase should not contain cryptographic anti-patterns");
    ...
    pub fn test_no_sensitive_logging() -> Result<()> {
        ...
        assert!(no_sensitive_logging, "Code should not log sensitive information (found patterns: {:?})", sensitive_patterns);
