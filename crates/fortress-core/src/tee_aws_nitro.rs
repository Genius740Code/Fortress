...
impl AwsNitroProvider {
    ...
    async fn execute_nitro_cli(&self, args: &[&str]) -> Result<String> {
        ...
        let output = tokio::task::spawn_blocking(move || {
            Command::new("nitro-cli").args(&args_owned).output()
        })
        .await
        .map_err(|e| {
            FortressError::tee(
    ...
    async fn get_enclave_description(&self, eif_path: &str) -> Result<NitroEnclaveDescription> {
        let output = self
            .execute_nitro_cli(&["describe-enclave", "--eif-path", eif_path])
            .await?;
    ...
    fn verify_measurements(
        ...
        if let Some(ref allowed_pcrs) = policy.allowed_pcr_values {
            if let (Some(ref pcr0), Some(allowed_pcr0)) =
                (&measurements.pcr0, allowed_pcrs.get("PCR0"))
            {
            ...
            if let (Some(ref pcr1), Some(allowed_pcr1)) =
                (&measurements.pcr1, allowed_pcrs.get("PCR1"))
            {
            ...
            if let (Some(ref pcr2), Some(allowed_pcr2)) =
                (&measurements.pcr2, allowed_pcrs.get("PCR2"))
            {
            ...
            if let (Some(ref pcr3), Some(allowed_pcr3)) =
                (&measurements.pcr3, allowed_pcrs.get("PCR3"))
            {
    ...
    async fn generate_attestation_document(&self, enclave_id: &str) -> Result<Vec<u8>> {
        let _output = self
            .execute_nitro_cli(&["run-enclave", "--enclave-id", enclave_id, "--debug-mode"])
            .await?;
...
impl TeeProvider for AwsNitroProvider {
    ...
    async fn create_enclave(&self, config: &EnclaveConfig) -> Result<String> {
        ...
        let enclave_id = response.enclave_id.ok_or_else(|| {
            FortressError::tee(
    ...
    async fn start_enclave(&self, enclave_id: &str) -> Result<()> {
        let mut state = self.state.write().await;
        let enclave_info = state.active_enclaves.get_mut(enclave_id).ok_or_else(|| {
            FortressError::tee(
    ...
    async fn stop_enclave(&self, enclave_id: &str) -> Result<()> {
        let _output = self
            .execute_nitro_cli(&["terminate-enclave", "--enclave-id", enclave_id])
            .await?;
    ...
    async fn attest_enclave(&self, enclave_id: &str) -> Result<AttestationResult> {
        ...
        let attestation: AttestationDocument =
            serde_json::from_slice(&attestation_doc).map_err(|e| {
                FortressError::tee(
    ...
    async fn send_message(&self, enclave_id: &str, message: &[u8]) -> Result<Vec<u8>> {
        ...
        let _enclave_cid = enclave_info.enclave_cid.ok_or_else(|| {
            FortressError::tee(
