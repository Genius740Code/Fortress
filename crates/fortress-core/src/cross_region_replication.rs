...
use crate::backup::{BackupManager, CrossRegionConfig, ReplicationResult, ReplicationStrategy};
...
impl CrossRegionReplicationManager {
    ...
    pub fn new(backup_manager: Arc<dyn BackupManager>, max_bandwidth_bps: u64) -> Self {
        Self {
            backup_manager,
    ...
    async fn start_replication_task(&self, config: CrossRegionConfig) -> Result<()> {
        ...
        let task = tokio::spawn(async move {
            ...
            let mut interval =
                tokio::time::interval(tokio::time::Duration::from_secs(config.frequency_seconds));
                ...
                if let Some(current_config) = configs_guard.get(&replication_id) {
                    ...
                    let should_replicate = match current_config.strategy {
                        ...
                        ReplicationStrategy::FullOnly => {
                            ...
                            Self::should_replicate_full_backups(&backup_manager, &current_config)
                                .await
                        }
    ...
    async fn replicate_to_region(
        ...
        let backups_to_replicate = match config.strategy {
            ...
            ReplicationStrategy::FullOnly => {
                ...
                backup_manager.list_backups().await.map(|backups| {
                    backups
                        .into_iter()
                        .filter(|backup| {
                            matches!(backup.strategy, crate::backup::BackupStrategy::Full)
                        })
                        .take(1)
                        .collect()
                })
        ...
        for backup in backups {
            ...
            let already_replicated = config.replication_history.iter().any(|result| {
                result.backup_id == backup.backup_id && result.target_region == region
            });
            ...
            match Self::replicate_single_backup(backup_manager, target_storage, &backup.backup_id)
                .await
            {
                Ok((items, size)) => {
    ...
    async fn replicate_single_backup(
        ...
        let manifest_data = serde_json::to_vec(&metadata).map_err(|e| {
            FortressError::storage(
    ...
    pub async fn update_replication_config(&self, config: CrossRegionConfig) -> Result<()> {
        ...
        self.remove_replication_config(&config.replication_id)
            .await?;
    ...
    pub async fn get_replication_config(
        &self,
        replication_id: &str,
    ) -> Result<Option<CrossRegionConfig>> {
        let configs = self.configs.read().await;
    ...
    pub async fn get_replication_stats(&self) -> Result<ReplicationStats> {
        ...
        if completed_replications > 0 {
            stats.average_replication_time_seconds =
                total_duration as f64 / completed_replications as f64;
        }
    ...
    pub async fn create_default_replications(&self) -> Result<()> {
        let target_regions: Vec<String> =
            self.target_storages.read().await.keys().cloned().collect();
...
mod tests {
    ...
    async fn test_cross_region_replication_creation() {
        ...
        replication_manager
            .add_target_region("us-east-1".to_string(), target_storage)
            .await
            .unwrap();
    ...
    async fn test_add_replication_config() {
        ...
        assert!(replication_manager
            .add_replication_config(config)
            .await
            .is_ok());

        let configs = replication_manager
            .list_replication_configs()
            .await
            .unwrap();
