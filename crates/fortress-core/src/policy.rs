...
impl PolicyEngine {
    ...
    pub async fn assign_role(&self, user_id: &str, role_name: &str) -> Result<()> {
        ...
        if !roles.contains_key(role_name) {
            return Err(FortressError::PolicyError(format!(
                "Role '{}' not found",
                role_name
            )));
        }
            ...
            if let Some(role) = roles.get(role_name) {
                for permission_entry in &role.permissions {
                    if permission_entry.permission == permission
                        ...
                        && self
                            .evaluate_conditions(&permission_entry.conditions, user_id)
                            .await?
                    {
    ...
    fn matches_resource(
        ...
        match (permission_resource, requested_resource) {
            ...
            (
                Resource::Field(perm_db, perm_table, perm_field),
                Resource::Field(req_db, req_table, req_field),
    ...
    async fn evaluate_condition(&self, condition: &Condition, user_id: &str) -> Result<bool> {
        match condition {
            ...
            Condition::Attribute(attr_cond) => {
                self.evaluate_attribute_condition(attr_cond, user_id).await
            }

            Condition::Custom(custom_cond) => {
                self.evaluate_custom_condition(custom_cond, user_id).await
            }
        }
    }
    ...
    async fn evaluate_ip_condition(&self, condition: &IpCondition, user_id: &str) -> Result<bool> {
        ...
        let user_ip = user_ip_context.get(user_id).ok_or_else(|| {
            FortressError::PolicyError(format!("No IP address found for user '{}'", user_id))
        })?;

        // Parse and validate IP address

        let ip_addr = IpAddr::from_str(user_ip).map_err(|_| {
            FortressError::PolicyError(format!(
                "Invalid IP address format for user '{}': {}",
                user_id, user_ip
            ))
        })?;
    ...
    async fn evaluate_attribute_condition(
        &self,
        ...
        let attributes = user_attributes.get(user_id).ok_or_else(|| {
            FortressError::PolicyError(format!("No attributes found for user '{}'", user_id))
        })?;

        let user_value = attributes.get(&condition.attribute).ok_or_else(|| {
            FortressError::PolicyError(format!(
                "Attribute '{}' not found for user '{}'",
        ...
        let result = match condition.operator {
            ...
            AttributeOperator::StartsWith => {
                sanitized_user_value.starts_with(sanitized_condition_value)
            }

            AttributeOperator::EndsWith => {
                sanitized_user_value.ends_with(sanitized_condition_value)
            }
            ...
            AttributeOperator::GreaterThan => {
                ...
                match (
                    sanitized_user_value.parse::<f64>(),
                    sanitized_condition_value.parse::<f64>(),
                ) {
    ...
    async fn evaluate_device_trust_score(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let min_score = condition
            .parameters
            .get("min_score")
        ...
        tracing::debug!(
            "Device trust score evaluation for {}: {} >= {} = {}",
            user_id,
            device_score,
            min_score,
            result
    ...
    async fn evaluate_geo_location(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let allowed_countries = condition
            .parameters
        ...
        let blocked_countries = condition
            .parameters
        ...
        tracing::debug!(
            "Geo location evaluation for {}: country={}, result={}",
            user_id,
            user_country,
            result
        );
    ...
    async fn evaluate_session_age(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let max_age_minutes = condition
            .parameters
            .get("max_age_minutes")
        ...
        tracing::debug!(
            "Session age evaluation for {}: {}min <= {}min = {}",
            user_id,
            session_age_minutes,
            max_age_minutes,
            result
    ...
    async fn evaluate_risk_score(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let max_risk_score = condition
            .parameters
            .get("max_risk_score")
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(70.0);
    ...
    async fn evaluate_business_hours(&self, condition: &CustomCondition) -> Result<bool> {
        ...
        let start_hour = condition
            .parameters
            .get("start_hour")
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(9); // 9 AM default

        let end_hour = condition
            .parameters
            .get("end_hour")
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(17); // 5 PM default

        let work_days = condition
            .parameters
            .get("work_days")
            .map(|days| {
                days.split(',')
    ...
    async fn evaluate_compliance_check(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let compliance_type = condition
            .parameters
            .get("compliance_type")
        ...
        let compliance_status = self
            .get_simulated_compliance_status(user_id, &compliance_type)
            .await?;

        let result = compliance_status;

        tracing::debug!(
            "Compliance check evaluation for {}: type={}, status={}",
            user_id,
            compliance_type,
            result
        );
    ...
    async fn evaluate_resource_quota(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        ...
        let current_usage = self
            .get_simulated_resource_usage(user_id, &resource_type)
            .await?;

        let result = current_usage <= max_usage;

        tracing::debug!(
            "Resource quota evaluation for {}: type={}, {} <= {} = {}",
            user_id,
            resource_type,
            current_usage,
            max_usage,
    ...
    async fn evaluate_mfa_status(
        &self,
        condition: &CustomCondition,
        user_id: &str,
    ) -> Result<bool> {
        let require_mfa = condition
            .parameters
            .get("require_mfa")
        ...
        tracing::debug!(
            "MFA status evaluation for {}: required={}, enabled={}, result={}",
            user_id,
            require_mfa,
            mfa_enabled,
            result
    ...
    async fn evaluate_script_condition(
        ...
        let script = condition.parameters.get("script").ok_or_else(|| {
            FortressError::PolicyError("Script parameter missing for custom condition".to_string())
        })?;
        ...
        tracing::warn!(
            "Script-based custom conditions are not fully implemented for security reasons"
        );

        // For now, return true as a safe default

        tracing::debug!(
            "Script condition evaluation for {}: script_length={}, result=true",
            user_id,
            script.len()
        );
    ...
    async fn get_simulated_compliance_status(
        &self,
        _user_id: &str,
        _compliance_type: &str,
    ) -> Result<bool> {
    ...
    async fn get_simulated_resource_usage(
        &self,
        user_id: &str,
        _resource_type: &str,
    ) -> Result<u64> {
...
mod tests {
    ...
    async fn test_resource_matching() {
        ...
        let can_read_table = engine
            .check_permission(
                "user1",
                Permission::Read,
                Resource::Table("users".to_string(), "profiles".to_string()),
            )
            .await
            .unwrap();
    ...
    async fn test_time_conditions() {
        ...
        let role = PolicyRole::new("time_restricted").with_permission_conditions(
            Permission::Read,
            Resource::Database("users".to_string()),
        ...
        engine
            .assign_role("user1", "time_restricted")
            .await
            .unwrap();
    ...
    async fn test_ip_conditions() {
        ...
        let role = PolicyRole::new("ip_restricted").with_permission_conditions(
            Permission::Read,
            Resource::Database("users".to_string()),
    ...
    async fn test_attribute_conditions() {
        ...
        let role = PolicyRole::new("attr_restricted").with_permission_conditions(
            Permission::Read,
            Resource::Database("users".to_string()),
        ...
        engine
            .assign_role("user1", "attr_restricted")
            .await
            .unwrap();
        ...
        engine
            .set_user_attribute("user1", "department", "sales")
            .await
            .unwrap();

        let cannot_read = engine
            .check_permission(
                "user1",
                Permission::Read,
                Resource::Database("users".to_string()),
    ...
    async fn test_user_attribute_management() {
        ...
        engine
            .set_user_attribute("user1", "clearance", "critical")
            .await
            .unwrap();
