...
impl SecureProtocolHandler {
    ...
    pub async fn create_message(
        ...
        let channel_state = channels.get(channel_id).ok_or_else(|| {
            FortressError::tee(
        ...
        let encrypted_payload = self
            .encrypt_payload(&channel_state.encryption_key, data, &header)
            .await?;
    ...
    pub async fn process_message(
        ...
        let channels = self.channels.read().await;
        let channel_state = channels.get(&message.header.channel_id).ok_or_else(|| {
            FortressError::tee(
        ...
        self.verify_message_signature(&channel_state.auth_key, &message)
            .await?;
    ...
    async fn encrypt_payload(
        ...
        let aad = serde_json::to_vec(header).map_err(|e| {
            FortressError::tee(
        ...
        let ciphertext = cipher.encrypt(nonce, aad.as_ref()).map_err(|e| {
            FortressError::tee(
    ...
    async fn decrypt_payload(
        ...
        let aad_result = if let Some(ref aad_str) = payload.aad {
            general_purpose::STANDARD.decode(aad_str).map_err(|e| {
                FortressError::tee(
        ...
        let plaintext = cipher.decrypt(nonce, aad_vec.as_ref()).map_err(|e| {
            FortressError::tee(
    ...
    pub async fn send_heartbeat(&self, channel_id: &str) -> Result<SecureMessage> {
        self.create_message(channel_id, SecureMessageType::Heartbeat, b"heartbeat")
            .await
    }
    ...
    pub async fn initiate_key_rotation(&self, channel_id: &str) -> Result<SecureMessage> {
        ...
        self.create_message(channel_id, SecureMessageType::KeyRotation, &exchange_bytes)
            .await
    }
...
mod tests {
    ...
    async fn test_message_creation() {
        ...
        let message = handler
            .create_message("test-channel", SecureMessageType::Heartbeat, b"test data")
            .await;
