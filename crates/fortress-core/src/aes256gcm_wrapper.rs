...
impl EncryptionAlgorithm for Aes256GcmWrapper {
    fn encrypt(&self, plaintext: &[u8], key: &[u8]) -> Result<Vec<u8>> {
        ...
        let nonce_bytes: [u8; 12] = nonce.try_into().map_err(|_e| {
            FortressError::encryption(
    ...
    fn decrypt(&self, ciphertext: &[u8], key: &[u8]) -> Result<Vec<u8>> {
        ...
        let plaintext = cipher.decrypt(nonce, actual_ciphertext).map_err(|_e| {
            FortressError::encryption(
...
mod tests {
    ...
    async fn test_invalid_key_sizes() {
        ...
        for key in short_keys {
            ...
            assert!(
                result.is_err(),
                "Should reject short key of length {}",
                key.len()
            );
        }
        ...
        for key in long_keys {
            ...
            assert!(
                result.is_err(),
                "Should reject long key of length {}",
                key.len()
            );
        }
    }
    ...
    async fn test_different_keys() {
        ...
        let keys = vec![[0u8; 32], [1u8; 32], [255u8; 32], {
            let mut key = [0u8; 32];
            for i in 0..32 {
                key[i] = (i * 7) as u8;
            }
    ...
    async fn test_random_nonce() {
        ...
        for ciphertext in &ciphertexts {
            ...
            assert_eq!(
                decrypted, plaintext,
                "All ciphertexts should decrypt to original"
            );
        }
    }
