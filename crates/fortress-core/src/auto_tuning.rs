...
use crate::performance_monitor::{
    AdvancedPerformanceMonitor as PerformanceMonitor,
    ImplementationComplexity as PmImplementationComplexity,
    RecommendationPriority as PmRecommendationPriority, RecommendationType as PmRecommendationType,
};
...
impl AutomaticPerformanceTuner {
    ...
    async fn perform_tuning_cycle(
        ...
        let recommendations: Vec<TuningRecommendation> = pm_recommendations
            .into_iter()
            .map(|pm_rec| {
                // Map the recommendation type

                let recommendation_type = match pm_rec.recommendation_type {
                    PmRecommendationType::IncreaseCacheSize => {
                        RecommendationType::IncreaseCacheSize
                    }

                    PmRecommendationType::AdjustConnectionPool => {
                        RecommendationType::AdjustConnectionPool
                    }
        ...
        let active_strategies: Vec<&TuningStrategy> =
            strategies_guard.iter().filter(|s| s.enabled).collect();
    ...
    async fn check_for_rollbacks(
        ...
        for change in changes.iter_mut() {
            if change.status == ChangeStatus::Observing {
                ...
                let degradation = Self::calculate_performance_degradation(
    ...
    async fn rollback_parameter_change(change: &mut AppliedChange) -> Result<()> {
        ...
        println!(
            "Rolling back parameter {} from {:?} to {:?}",
            change.parameter_name, change.new_value, change.previous_value
        );
    ...
    async fn apply_strategy(
        ...
        for recommendation in relevant_recommendations {
            ...
            let parameter_name =
                Self::parameter_from_recommendation(&recommendation.recommendation_type);
            ...
            if let Some(parameter) = params_guard.get(&parameter_name) {
                ...
                if let Some(new_value) = Self::calculate_optimal_value(parameter, &state, config)? {
                    ...
                    let change_percentage =
                        Self::calculate_change_percentage(&parameter.current_value, &new_value);

                    if change_percentage >= 5.0
                        && change_percentage <= (config.max_change_percentage * 100.0)
                    {
                        // Apply the change
    ...
    fn calculate_optimal_value(
        ...
        match parameter.data_type {
            ParameterType::Integer => {
                ...
                let min_bound = parameter
                    .min_value
                    .as_ref()
                    .and_then(|v| v.as_i64())
                    .unwrap_or(i64::MIN);

                let max_bound = parameter
                    .max_value
                    .as_ref()
                    .and_then(|v| v.as_i64())
                    .unwrap_or(i64::MAX);
    ...
    pub async fn get_tuning_status(&self) -> Result<TuningStatus> {
        ...
        let recent_changes: Vec<&AppliedChange> = applied_changes
            .iter()
            .filter(|change| change.applied_at > chrono::Utc::now() - chrono::Duration::hours(24))
            .collect();
