...
use crate::security::memory_safety::{ConstantTimeOps, SecureKey};
...
pub trait ZeroKnowledgeProof {
    ...
    ) -> Result<Self::Proof, FortressError>;
...
impl ZeroKnowledgeProof for SchnorrProof {
    ...
    fn verify(statement: &Self::Statement, proof: &Self::Proof) -> Result<bool, FortressError> {
        ...
        let commitment_matches =
            ConstantTimeOps::compare_bytes_secure(&proof.commitment, &expected_commitment);

        // Verify statement matches
        let statement_matches = ConstantTimeOps::compare_bytes_secure(statement, &proof.statement);
...
impl AnonymousAuth {
    ...
    pub fn register_user(
        ...
        user_id: UserId,
        valid_days: i64,
    ) -> Result<Credential, FortressError> {
        use rand::rngs::OsRng;
        use rand::RngCore;
    ...
    pub fn authenticate_anonymously(
        &self,
        user_id: &UserId,
        challenge: &[u8],
    ) -> Result<AnonymousProof, FortressError> {
        let credential = self
            .user_credentials
            .get(user_id)
    ...
    pub fn verify_anonymous(
        ...
        let challenge_valid =
            ConstantTimeOps::compare_bytes_secure(&proof.challenge_hash, &expected_challenge_hash);
    ...
    fn verify_credential_proof(
        ...
        Ok(ConstantTimeOps::compare_bytes_secure(
            proof,
            &expected_proof,
        ))
    }
...
impl GdprComplianceProof {
    ...
    pub fn create_compliance_proof(
        ...
        let retention_proof =
            Self::prove_retention_compliance(data_subject, retention_period_days)?;
