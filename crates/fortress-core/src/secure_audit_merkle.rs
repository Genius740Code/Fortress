...
impl AuditMerkleTree {
    ...
    pub fn generate_proof(&self, leaf_hash: &str) -> Result<MerkleProof> {
        let root_hash = self.root_hash.as_ref()
            .ok_or_else(|| FortressError::audit("Merkle tree is empty".to_string(), None, AuditErrorCode::LogRetrievalFailed))?;
...
impl SecureAuditLogger {
    ...
    async fn verify_entry_signature(&self, entry: &SecureAuditEntry) -> Result<bool> {
        ...
        let entry_data = serde_json::to_string(&entry_for_verification)
            .map_err(|e| FortressError::audit(format!("Failed to serialize audit entry: {}", e), None, AuditErrorCode::LogRetrievalFailed))?;
        ...
        if !entry.signature.is_empty() {
            ...
            if let Ok(signature_bytes) = base64::engine::general_purpose::STANDARD.decode(sig_str) {
                if signature_bytes.len() == 64 {
                    ...
                    let public_key_bytes = base64::engine::general_purpose::STANDARD.decode(&entry.key_fingerprint)
                        .map_err(|e| FortressError::audit(format!("Failed to decode public key: {}", e), None, AuditErrorCode::VerificationFailed))?;
                    
                    if public_key_bytes.len() != 32 {
                        return Err(FortressError::audit("Invalid public key length".to_string(), None, AuditErrorCode::VerificationFailed));
                    }
                    ...
                    let public_key = match VerifyingKey::from_bytes(&key_array) {
                        Ok(key) => key,
                        Err(e) => return Err(FortressError::audit(format!("Failed to create public key: {:?}", e), None, AuditErrorCode::VerificationFailed)),
                    };
                    
                    return Ok(public_key.verify(&entry_data.as_bytes(), &signature).is_ok());
                }
            }
        }
        ...
        if !entry.signature.is_empty() {
            ...
            if let Ok(signature_bytes) = base64::engine::general_purpose::STANDARD.decode(sig_str) {
                if signature_bytes.len() == 64 {
                    ...
                    if public_key.verify(&entry_data.as_bytes(), &signature).is_ok() {
                        // Signature is valid, proceed with writing
                    } else {
                        return Err(FortressError::audit("Invalid signature".to_string(), None, AuditErrorCode::VerificationFailed));
                    }
                }
            }
        }
    ...
    async fn create_anchor_proof(&self, root_hash: &str) -> Result<String> {
        ...
        let proof = format!("anchor_proof_{}_{}", 
            root_hash,
            Utc::now().timestamp_nanos_opt().unwrap_or(0));
        Ok(proof)
    }
    ...
    pub async fn verify_integrity(&self, file_path: &str) -> Result<IntegrityVerificationReport> {
        ...
        for entry in &sorted_entries {
            ...
            if config.enable_digital_signatures {
                if !self.verify_entry_signature(entry).await? {
                    tampered_entries.push(TamperedEntry {
                        ...
                        description: format!("Invalid signature for entry {}", entry.sequence_number),
                    });
                    entry_valid = false;
            ...
            if let Some(prev_entry) = sorted_entries.iter().find(|e| e.sequence_number == entry.sequence_number - 1) {
                if entry.timestamp <= prev_entry.timestamp {
                    tampered_entries.push(TamperedEntry {
                        sequence_number: entry.sequence_number,
                        entry_id: entry.entry_id.clone(),
                        tampering_type: TamperingType::TimestampInconsistency,
                        description: format!("Timestamp inconsistency at entry {}", entry.sequence_number),
                    });
                    entry_valid = false;
                }
            }
        ...
        
        {
            ...
            stats.avg_verification_time_ms = 
                (stats.avg_verification_time_ms * (stats.total_checks - 1) as f64 + duration_ms) / stats.total_checks as f64;
        }
