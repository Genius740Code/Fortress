...
use crate::encryption::PerformanceProfile;
    ...
    async fn test_special_characters_in_key_id() -> Result<()> {
        ...
        for (i, special_id) in special_ids.iter().enumerate() {
            ...
            // Retrieve key
            let result = db.retrieve_key(&key_id).await?;
            assert!(result.is_some(), "Failed to retrieve key with ID: {}", special_id);
            
            // Check existence
            assert!(db.key_exists(&key_id).await?, "Key should exist with ID: {}", special_id);
        }
    ...
    async fn test_performance_with_many_keys() -> Result<()> {
        ...
        assert!(store_time.as_millis() < 5000, "Store operation took too long: {:?}", store_time);
        assert!(retrieve_time.as_millis() < 5000, "Retrieve operations took too long: {:?}", retrieve_time);
        assert!(list_time.as_millis() < 1000, "List operation took too long: {:?}", list_time);
