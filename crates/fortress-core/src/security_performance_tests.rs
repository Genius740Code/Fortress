...
use crate::security_fixes::{CsrfProtection, InputValidator, SecureSessionGenerator};
...
impl SecurityPerformanceTests {
    ...
    pub async fn test_authentication_performance() -> Result<()> {
        ...
        assert!(
            single_auth_time < Duration::from_millis(10),
            "Single authentication should complete in < 10ms, took {:?}",
            single_auth_time
        );
        ...
        assert!(
            ...
            "Average authentication time should be < 5ms, took {:?}",
            avg_time
        );
        assert!(
            max_time < Duration::from_millis(20),
            "Max authentication time should be < 20ms, took {:?}",
            max_time
        );
        assert!(
            concurrent_total_time < Duration::from_millis(100),
            "100 concurrent authentications should complete in < 100ms, took {:?}",
    ...
    pub async fn test_session_generation_performance() -> Result<()> {
        ...
        assert!(
            single_session_time < Duration::from_millis(5),
            "Single session generation should complete in < 5ms, took {:?}",
            single_session_time
        );
        ...
        assert!(
            ...
            "1000 session generations should complete in < 100ms, took {:?}",
            batch_time
        );
        assert!(
            avg_time < Duration::from_micros(100),
            "Average session generation time should be < 100µs, took {:?}",
            avg_time
        );
        ...
        assert!(
            uniqueness_time < Duration::from_millis(10),
            "Uniqueness verification should complete in < 10ms, took {:?}",
            uniqueness_time
        );
    ...
    pub async fn test_csrf_protection_performance() -> Result<()> {
        ...
        assert!(
            ...
            "1000 token generations should complete in < 50ms, took {:?}",
            generation_time
        );
        assert!(
            avg_generation_time < Duration::from_micros(50),
            "Average token generation time should be < 50µs, took {:?}",
            avg_generation_time
        );
        ...
        assert!(
            ...
            "1000 token validations should complete in < 20ms, took {:?}",
            validation_time
        );
        assert!(
            avg_validation_time < Duration::from_micros(20),
            "Average token validation time should be < 20µs, took {:?}",
            avg_validation_time
        );
        ...
        assert_eq!(
            validations, 100,
            "All concurrent CSRF operations should validate"
        );
        assert!(
            concurrent_time < Duration::from_millis(50),
            "100 concurrent CSRF operations should complete in < 50ms, took {:?}",
            concurrent_time
        );
    ...
    pub async fn test_input_validation_performance() -> Result<()> {
        ...
        assert!(
            email_validation_time < Duration::from_millis(10),
            "3500 email validations should complete in < 10ms, took {:?}",
        ...
        assert!(
            url_validation_time < Duration::from_millis(10),
            "2000 URL validations should complete in < 10ms, took {:?}",
            url_validation_time
        );

        // Test 3: Filename validation performance
        let start_time = Instant::now();
        let valid_filenames = vec!["document.pdf", "image_123.jpg", "file.txt", "data.csv"];

        let invalid_filenames = vec!["../etc/passwd", "file\\name", "file|pipe", "file<name>", ""];
        ...
        assert_eq!(
            filename_validations, 4500,
            "All filename validations should complete"
        );
        assert!(
            filename_validation_time < Duration::from_millis(10),
            "4500 filename validations should complete in < 10ms, took {:?}",
            filename_validation_time
        );
    ...
    pub async fn test_rate_limiting_performance() -> Result<()> {
        ...
        assert!(
            ...
            "Average rate limiting lookup should be < 2ms, took {:?}",
            avg_time
        );
        assert!(
            max_time < Duration::from_millis(10),
            "Max rate limiting lookup should be < 10ms, took {:?}",
            max_time
        );
        assert!(
            total_time < Duration::from_millis(200),
            "1000 rate limiting lookups should complete in < 200ms, took {:?}",
            total_time
        );
    ...
    pub async fn test_memory_usage() -> Result<()> {
        ...
        assert!(
            session_memory_usage < 1_000_000,
            "10000 sessions should use < 1MB memory, used ~{} bytes",
        ...
        assert!(
            csrf_memory_usage < 100_000,
            "1000 CSRF tokens should use < 100KB memory, used ~{} bytes",
            csrf_memory_usage
        );
...
mod tests {
    ...
    fn test_performance_report_generation() {
        ...
        assert!(
            report.contains("Security Performance Report"),
