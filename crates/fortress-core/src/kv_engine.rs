...
impl PostgresKvEngine {
    ...
    async fn update_stats(&self, operation_duration: Duration) {
        ...
        stats.avg_latency_us =
            (stats.avg_latency_us * 0.9) + (operation_duration.as_micros() as f64 * 0.1);
    ...
    fn get_expiration_time(&self, ttl: Option<Duration>) -> DateTime<Utc> {
        let effective_ttl = ttl
            .or(self.default_ttl)
            .unwrap_or(Duration::from_secs(86400 * 365)); // 1 year default
        Utc::now()
...
impl KvEngine for PostgresKvEngine {
    ...
    async fn set_with_ttl(&self, key: &str, value: &[u8], ttl: Duration) -> Result<()> {
        ...
        metadata.insert(
            ...
            self.get_expiration_time(Some(ttl)).to_rfc3339(),
        );
    ...
    async fn batch_set(&self, pairs: &[(String, Vec<u8>)]) -> Result<()> {
        ...
        let entries: Vec<PostgresBulkEntry> = pairs
            .iter()
            .map(|(key, data)| PostgresBulkEntry {
                key: key.clone(),
                data: data.clone(),
                metadata: HashMap::new(),
                content_type: "application/octet-stream".to_string(),
...
impl MemoryKvEngine {
    ...
    async fn cleanup_expired(&self) {
        ...
        data.retain(|_, (_, expires_at)| expires_at.map_or(true, |exp| exp > now));
    }
}
...
impl KvEngine for MemoryKvEngine {
    ...
    async fn batch_get(&self, keys: &[String]) -> Result<Vec<(String, Option<Vec<u8>>)>> {
        ...
        let data = self.data.read().await;
        let results: Vec<(String, Option<Vec<u8>>)> = keys
            .iter()
...
mod tests {
    ...
    async fn test_memory_kv_engine_ttl() {
        ...
        // Set with TTL
        engine
            .set_with_ttl("ttl_key", b"ttl_value", Duration::from_millis(100))
            .await
            .unwrap();
