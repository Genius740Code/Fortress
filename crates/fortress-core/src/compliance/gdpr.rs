...
impl GdprComplianceManager {
    ...
    pub async fn get_upcoming_deadlines(
        &self,
    ) -> Result<Vec<crate::compliance::framework::ComplianceDeadline>> {
    ...
    pub async fn monitor_consent_expiry(&self) -> Result<Vec<ConsentExpiryAlert>> {
            ...
            if let Some(expires_at) = consent.expires_at {
                ...
                if days_until_expiry >= 0 && days_until_expiry <= 30 {
                    ...
                    let recommended_action = match days_until_expiry {
                        0..=7 => format!(
                            "URGENT: Consent expires in {} days. Immediate renewal required.",
                            days_until_expiry
                        ),
                        8..=14 => format!(
                            "Consent expires in {} days. Initiate renewal process.",
                            days_until_expiry
                        ),
                        15..=30 => format!(
                            "Consent expires in {} days. Plan renewal notification.",
                            days_until_expiry
                        ),
        ...
        for alert in &alerts {
            ...
            if !exists {
                ...
                log::warn!(
                    "Consent expiry alert created: {} for subject {}, expires in {} days",
                    alert.consent_id,
                    alert.subject_id,
        ...
        log::info!(
            "Consent expiry monitoring completed. {} alerts generated",
            alerts.len()
        );
        Ok(alerts)
    }
    ...
    async fn send_renewal_notifications(&self, alerts: &[&ConsentExpiryAlert]) -> Result<()> {
        log::info!(
            "Sending renewal notifications for {} critical alerts",
            alerts.len()
        );
        ...
        for alert in alerts {
            ...
            let event = ComplianceEvent {
                ...
                description: format!(
                    "Renewal notification sent for consent {} expiring in {} days",
                ...
                metadata: {
                    ...
                    meta.insert(
                        ...
                        alert.days_until_expiry.to_string(),
                    );
                    meta.insert(
                        "notification_type".to_string(),
                        "renewal_reminder".to_string(),
                    );
                    meta
                },
            };
            ...
            let mut stored_alerts = self.expiry_alerts.write().await;
            if let Some(stored_alert) = stored_alerts.iter_mut().find(|a| {
                a.consent_id == alert.consent_id && a.days_until_expiry == alert.days_until_expiry
            }) {
    ...
    pub async fn enforce_purpose_based_access(
        &self,
        user_id: &str,
        purpose: &str,
        data_id: &str,
    ) -> Result<PurposeAccessResult> {
        log::info!(
            "Enforcing purpose-based access: user={}, purpose={}, data={}",
            user_id,
            purpose,
            data_id
        );
        ...
        for consent in consent_records.values() {
            ...
            let purpose_matches = consent
                .purposes
                .iter()
                .any(|p| p.to_lowercase() == purpose.to_lowercase());

            if is_valid && purpose_matches {
                applicable_consents.push(consent.id);
                granted = true;
                reason = format!(
                    "Access granted based on consent {} for purpose '{}'",
                    consent.id, purpose
                );
        ...
        if !granted {
            if applicable_consents.is_empty() {
                reason = format!(
                    "Access denied: No valid consent found for purpose '{}'",
                    purpose
                );
            } else {
                reason = format!(
                    "Access denied: Consent exists but purpose '{}' not authorized",
                    purpose
                );
            }
        }
        ...
        let event = ComplianceEvent {
            ...
            description: format!(
                "Purpose-based access {} for user {} on data {} with purpose '{}'",
            ...
            outcome: if granted {
                ComplianceEventOutcome::Success
            } else {
                ComplianceEventOutcome::Blocked
            },
            metadata: {
                let mut meta = HashMap::new();
                ...
                meta.insert(
                    "applicable_consents".to_string(),
                    format!("{:?}", applicable_consents),
                );
        ...
        if granted {
            log::info!(
                "Purpose-based access GRANTED: user={}, purpose={}, data={}, consents={:?}",
                user_id,
                purpose,
                data_id,
                applicable_consents
            ...
            log::warn!(
                "Purpose-based access DENIED: user={}, purpose={}, data={}, reason={}",
                user_id,
                purpose,
                data_id,
                reason
    ...
    pub async fn process_consent_withdrawal(
        &self,
        consent_id: &str,
    ) -> Result<WithdrawalProcessingResult> {
        log::info!(
            "Processing consent withdrawal for consent ID: {}",
            consent_id
        );

        let consent_uuid = Uuid::parse_str(consent_id).map_err(|e| {
            crate::error::FortressError::compliance(format!("Invalid consent ID: {}", e))
        })?;
        ...
        let consent = match found_consent {
            ...
            None => {
                ...
                result
                    .errors
                    .push(format!("Consent record {} not found", consent_id));
        ...
        let event = ComplianceEvent {
            ...
            severity: EventSeverity::Warning,
            description: format!(
                "Consent withdrawal initiated for consent {} belonging to subject {}",
        ...
        match self
            .execute_withdrawal_workflow(&consent, &result.subject_id)
            .await
        {
            Ok(records_affected) => {
                ...
                log::info!(
                    "Consent withdrawal completed successfully: {} records deleted",
                    records_affected
                );
            }
            Err(e) => {
                ...
                result
                    .errors
                    .push(format!("Withdrawal workflow failed: {}", e));
                result.completed_at = Some(Utc::now());
        ...
        let completion_event = ComplianceEvent {
            ...
            description: format!(
                "Consent withdrawal {} for consent {}: {} records affected",
                match result.status {
            ...
            metadata: {
                ...
                meta.insert(
                    "records_affected".to_string(),
                    result.data_records_affected.to_string(),
                );
    ...
    async fn execute_withdrawal_workflow(
        &self,
        consent: &ConsentRecord,
        subject_id: &str,
    ) -> Result<u64> {
        log::info!(
            "Executing withdrawal workflow for consent {} subject {}",
            consent.id,
            subject_id
        );
        ...
        log::info!(
            "Withdrawal workflow completed: {} records processed for deletion",
            records_affected
        );
        Ok(records_affected)
    }
