...
use crate::mpc::{
    ComputationConfig, ComputationResult, ComputationStatus, MpcManager, MpcMessage, MpcNetwork,
    MpcParty, MpcProtocol, PartyId, SessionId, ShamirSecretSharing,
};
...
impl DefaultMpcManager {
    ...
    fn validate_config(&self, config: &ComputationConfig) -> Result<()> {
        ...
        match &config.sharing_scheme {
            crate::mpc::SecretSharingScheme::Shamir {
                threshold,
                total_shares,
            } => {
            ...
            crate::mpc::SecretSharingScheme::Replicated {
                replicas,
                threshold,
            } => {
...
impl MpcManager for DefaultMpcManager {
    ...
    async fn join_session(&self, session_id: &SessionId, party: Box<dyn MpcParty>) -> Result<()> {
        ...
        let session_state = sessions.get_mut(session_id).ok_or_else(|| {
            FortressError::encryption(
        ...
        if !matches!(
            ...
            ComputationStatus::Recruiting | ComputationStatus::Preparing
        ) {
            return Err(FortressError::encryption(
    ...
    async fn get_session_status(&self, session_id: &SessionId) -> Result<ComputationStatus> {
        ...
        let session_state = sessions.get(session_id).ok_or_else(|| {
            FortressError::encryption(
                format!("Session '{}' not found", session_id),
                "mpc_manager".to_string(),
                EncryptionErrorCode::AlgorithmNotSupported,
...
impl MpcManagerBuilder {
    ...
    pub fn build(self) -> Result<DefaultMpcManager> {
        let network = self
            .network
            .unwrap_or_else(|| Arc::new(InMemoryMpcNetwork::new()));
        ...
        let protocols = self.protocols.unwrap_or_else(|| {
            ...
            p.insert(
                "shamir".to_string(),
                Box::new(ShamirSecretSharing::new().unwrap()) as Box<dyn MpcProtocol>,
            );
...
mod tests {
    ...
    async fn test_session_joining() {
        ...
        let party1 = Arc::new(InMemoryMpcParty::new(
            "party1".to_string(),
            PartyRole::Initiator,
        ));

        let party2 = Arc::new(InMemoryMpcParty::new(
            "party2".to_string(),
        ...
        manager
            .join_session(&session_id, Box::new(party1.as_ref().clone()))
            .await
            .unwrap();
        ...
        manager
            .join_session(&session_id, Box::new(party2.as_ref().clone()))
            .await
            .unwrap();
    ...
    async fn test_session_listing() {
        ...
        let config2 =
            ComputationConfig::new("shamir", SecretSharingScheme::Additive { num_parties: 2 });
    ...
    fn test_mpc_manager_builder() {
        let builder = MpcManagerBuilder::new().with_default_protocol("shamir");
