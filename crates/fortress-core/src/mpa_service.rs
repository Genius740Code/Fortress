...
use crate::multi_person_auth::{
    ApprovalRequest, ApprovalRequestId, ControlGroup, ControlGroupId, ControlGroupRole, Decision,
    MultiPersonAuthManager, MultiPersonOperationType,
};
...
impl MpaService {
    ...
    pub async fn create_control_group(
        ...
        let mut manager = self
            .manager
            .write()
            .map_err(|_| FortressError::internal("Failed to acquire MPA lock", "MPA_LOCK_ERROR"))?;
        ...
        match adding_user_role {
            ...
            _ => {
                return Err(FortressError::authentication(
                    "Only administrators and owners can add members to control groups",
                    None,
                ));
            }
        }
    ...
    pub async fn create_approval_request(
        ...
        if operation_description.len() > 1000 {
            return Err(FortressError::validation(
                "Operation description too long (max 1000 characters)",
                None,
                None,
            ));
        }
        ...
        if !operation_context.trim().is_empty() {
            serde_json::from_str::<serde_json::Value>(&operation_context).map_err(|_| {
                FortressError::validation("Operation context must be valid JSON", None, None)
            })?;
        }
    ...
    pub async fn submit_approval_decision(
        ...
        if let Some(ua) = &user_agent {
            if ua.len() > 500 {
                return Err(FortressError::validation(
                    "User agent too long (max 500 characters)",
                    None,
                    None,
                ));
            }
        ...
        manager.submit_approval_decision(
            request_id, user_id, decision, comment, source_ip, user_agent,
        )
    }
...
mod tests {
    ...
    async fn test_create_control_group_service() {
        ...
        let groups = service
            .list_control_groups("admin".to_string())
            .await
            .unwrap();
    ...
    async fn test_get_pending_requests() {
        ...
        let pending = service
            .get_pending_requests_for_user("user1".to_string())
            .await
            .unwrap();
        ...
        let pending = service
            .get_pending_requests_for_user("requester".to_string())
            .await
            .unwrap();
