...
use crate::error::{FortressError, QueryErrorCode, Result};
...
impl Row {
    ...
    {
        T::try_from(self.values.get(index).ok_or_else(|| {
            FortressError::query_execution(
...
impl TryFromQueryParameter for uuid::Uuid {
    fn try_from(param: &QueryParameter) -> Result<Self> {
        match param {
            ...
            QueryParameter::String(s) => uuid::Uuid::parse_str(s).map_err(|_| {
                FortressError::query_execution(
...
impl TryFromQueryParameter for chrono::DateTime<chrono::Utc> {
    fn try_from(param: &QueryParameter) -> Result<Self> {
        match param {
            ...
            QueryParameter::String(s) => s.parse::<chrono::DateTime<chrono::Utc>>().map_err(|_| {
                FortressError::query_execution(
...
impl InMemoryQueryEngine {
    ...
    pub async fn insert(&self, table_name: &str, row: Row) -> Result<()> {
        ...
        let rows = data.get_mut(table_name).ok_or_else(|| {
            FortressError::query_execution(
    ...
    async fn execute(&self, query: &str, _parameters: &[QueryParameter]) -> Result<QueryResult> {
        ...
        if query.trim().to_uppercase().starts_with("SELECT") {
            ...
            if parts.len() >= 4
                ...
                && parts[2].to_uppercase() == "FROM"
            {
                let table_name = parts[3];

                let tables = self.tables.read().await;

                let schema = tables.get(table_name).ok_or_else(|| {
                    FortressError::query_execution(
                        format!("Table not found: {}", table_name),
