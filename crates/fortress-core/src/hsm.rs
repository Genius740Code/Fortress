...
pub trait HsmProvider: Send + Sync {
    ...
    async fn generate_key(&self, key_id: &KeyId, algorithm: &dyn EncryptionAlgorithm)
        -> Result<()>;
    ...
    async fn get_connection(&self) -> Result<String> {
        ...
        if let Some(client) = client_guard.as_ref() {
            ...
            if pool_guard.len() < 10 {
                // Max 10 connections for scalability
                let conn_id = format!(
                    "hsm_conn_{}",
    ...
    async fn update_metrics(&self, operation_time_ms: u64, success: bool) {
        ...
        if let Some(client) = client_guard.as_ref() {
            ...
            metrics.avg_latency_ms =
                alpha * operation_time_ms as f64 + (1.0 - alpha) * metrics.avg_latency_ms;
...
impl HsmProvider for AwsCloudHsmProvider {
            ...
            HsmConnection::AwsCloudHsm { cluster_id } => {
                ...
                match &config.credentials {
                    HsmCredentials::Aws {
                        ...
                        log::info!(
                            "Configuring AWS CloudHSM credentials for region: {}",
                            region
                        );
                        ...
                        for i in 0..2 {
                            if let Err(e) = self.get_connection().await {
                                log::warn!(
                                    "Failed to initialize initial connection {}: {:?}",
                                    i,
                                    e
                                );
                            }
    ...
    async fn generate_key(
        ...
        log::info!(
            "Generating key {} in AWS CloudHSM with algorithm: {}",
            key_id,
            algorithm.name()
        );
        ...
        while retries > 0 {
            ...
            log::info!(
                "Key {} generated successfully in AWS CloudHSM in {}ms",
                key_id,
                operation_time
            );
            return Ok(());
    ...
    async fn get_key_metadata(&self, key_id: &KeyId) -> Result<KeyMetadata> {
        ...
        log::info!(
            "Retrieved metadata for key {} in {}ms",
            key_id,
            operation_time
        );
        Ok(metadata)
    ...
    async fn delete_key(&self, key_id: &KeyId) -> Result<()> {
        ...
        log::warn!(
            "Key {} permanently deleted from AWS CloudHSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn list_keys(&self) -> Result<Vec<(KeyId, KeyMetadata)>> {
        ...
        log::info!(
            "Listed {} keys from AWS CloudHSM in {}ms",
            keys.len(),
            operation_time
        );
        Ok(keys)
    ...
    async fn sign(&self, key_id: &KeyId, data: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Signing {} bytes of data with key {} using AWS CloudHSM",
            data.len(),
            key_id
        );
        ...
        log::info!(
            "Data signed successfully with key {} in {}ms, signature size: {} bytes",
            key_id,
            operation_time,
            signature.len()
        );
    ...
    async fn verify(&self, key_id: &KeyId, data: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        log::info!(
            "Signature verification completed for key {} in {}ms, result: {}",
            key_id,
            operation_time,
            is_valid
        );
    ...
    async fn encrypt(&self, key_id: &KeyId, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Encrypting {} bytes of data with key {} using AWS CloudHSM",
            plaintext.len(),
            key_id
        );
        ...
        let iv = (0..12)
            .map(|i| ((key_id.to_string().len() + i * 3) % 256) as u8)
            .collect::<Vec<_>>();
        ...
        let tag = (0..16)
            .map(|i| ((plaintext.len() + i * 7) % 256) as u8)
            .collect::<Vec<_>>();
        ...
        log::info!(
            "Data encrypted successfully with key {} in {}ms, ciphertext size: {} bytes",
            key_id,
            operation_time,
            ciphertext.len()
        );
    ...
    async fn decrypt(&self, key_id: &KeyId, ciphertext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Decrypting {} bytes of data with key {} using AWS CloudHSM",
            ciphertext.len(),
            key_id
        );
        ...
        if ciphertext.len() < 28 {
            // Minimum size: 12-byte IV + 1-byte data + 16-byte tag
            return Err(FortressError::key_management(
        ...
        let expected_tag = (0..16)
            .map(|i| ((plaintext.len() + i) % 256) as u8)
            .collect::<Vec<_>>();
        if tag != expected_tag {
            self.return_connection(&_conn_id).await;
            return Err(FortressError::key_management(
        ...
        log::info!(
            "Data decrypted successfully with key {} in {}ms, plaintext size: {} bytes",
            key_id,
            operation_time,
            plaintext.len()
        );
        ...
        // Check connection pool
        {
            ...
            if let Some(client) = client_guard.as_ref() {
                ...
                if metrics.error_rate > 0.5 {
                    ...
                    log::warn!(
                        "AWS CloudHSM error rate is high: {:.2}%",
                ...
                if metrics.avg_latency_ms > 1000.0 {
                    // More than 1 second average latency
                    log::warn!(
                        "AWS CloudHSM latency is high: {:.2}ms",
                        metrics.avg_latency_ms
                    );
                    health_status = false;
                }
    ...
    async fn shutdown(&self) -> Result<()> {
        ...
        log::info!(
            "AWS CloudHSM provider shutdown completed in {}ms",
            shutdown_time
        );
...
impl Pkcs11Provider {
    ...
    async fn get_connection(&self) -> Result<String> {
        ...
        // Create new connection if pool is not full
        if pool_guard.len() < 8 {
            // Max 8 connections for scalability
            let conn_id = format!(
                "pkcs11_conn_{}",
                chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)
            );
    ...
    async fn create_session_internal(&self) -> Result<u64> {
        ...
        let session_handle =
            (chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0) as u64) % 1000000;
...
impl HsmProvider for Pkcs11Provider {
    ...
    async fn initialize(&self, config: &HsmConfig) -> Result<()> {
        ...
        match &config.connection {
            HsmConnection::Pkcs11 {
                ...
                log::info!(
                    "Initializing PKCS#11 session with slot_id: {:?}, token_label: {:?}",
                    slot_id,
                    token_label
                );
    ...
    async fn generate_key(
        ...
        log::info!(
            "Generating key {} in PKCS#11 HSM with algorithm: {}",
            key_id,
            algorithm.name()
        );
    ...
    async fn delete_key(&self, key_id: &KeyId) -> Result<()> {
        ...
        self.update_metrics(operation_time, true).await;
        log::warn!(
            "Key {} permanently deleted from PKCS#11 HSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn list_keys(&self) -> Result<Vec<(KeyId, KeyMetadata)>> {
        ...
        log::info!(
            "Listed {} keys from PKCS#11 HSM in {}ms",
            keys.len(),
            operation_time
        );
        Ok(keys)
    ...
    async fn sign(&self, key_id: &KeyId, data: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Signing {} bytes of data with key {} using PKCS#11 HSM",
            data.len(),
            key_id
        );
    ...
    async fn encrypt(&self, key_id: &KeyId, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Encrypting {} bytes of data with key {} using PKCS#11 HSM",
            plaintext.len(),
            key_id
        );
    ...
    async fn decrypt(&self, key_id: &KeyId, ciphertext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Decrypting {} bytes of data with key {} using PKCS#11 HSM",
            ciphertext.len(),
            key_id
        );
    ...
    async fn health_check(&self) -> Result<bool> {
        ...
        if metrics.avg_latency_ms > 500.0 {
            // More than 500ms average latency
...
impl HsmProvider for AzureDedicatedHsmProvider {
    async fn initialize(&self, config: &HsmConfig) -> Result<()> {
        ...
        match &config.connection {
            HsmConnection::Azure { resource_id } => {
                log::info!(
                    "Initializing Azure Dedicated HSM for resource: {}",
                    resource_id
                );
                ...
                match &config.credentials {
                    HsmCredentials::Azure {
                        ...
                        for i in 0..2 {
                            if let Err(e) = self.get_connection().await {
                                log::warn!(
                                    "Failed to initialize initial Azure connection {}: {:?}",
                                    i,
                                    e
                                );
                            }
                        ...
                        log::info!(
                            "Azure Dedicated HSM initialized successfully in {}ms",
                            init_time
                        );
                        Ok(())
                    }
    ...
    async fn generate_key(
        ...
        log::info!(
            "Generating key {} in Azure Dedicated HSM with algorithm: {}",
            key_id,
            algorithm.name()
        );
        ...
        log::info!(
            "Key {} generated successfully in Azure Dedicated HSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn get_key_metadata(&self, key_id: &KeyId) -> Result<KeyMetadata> {
        ...
        log::info!(
            "Retrieving metadata for key {} from Azure Dedicated HSM",
            key_id
        );
    ...
    async fn delete_key(&self, key_id: &KeyId) -> Result<()> {
        ...
        log::debug!(
            "Scheduling key {} for deletion in Azure Dedicated HSM",
            key_id
        );
        tokio::time::sleep(tokio::time::Duration::from_millis(40)).await;
        ...
        log::warn!(
            "Key {} permanently deleted from Azure Dedicated HSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn list_keys(&self) -> Result<Vec<(KeyId, KeyMetadata)>> {
        ...
        log::info!(
            "Listed {} keys from Azure Dedicated HSM in {}ms",
            keys.len(),
            operation_time
        );
        Ok(keys)
    ...
    async fn sign(&self, key_id: &KeyId, data: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Signing {} bytes of data with key {} using Azure Dedicated HSM",
            data.len(),
            key_id
        );
    ...
    async fn verify(&self, key_id: &KeyId, data: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        log::info!(
            "Verifying signature with key {} using Azure Dedicated HSM",
            key_id
        );
    ...
    async fn encrypt(&self, key_id: &KeyId, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Encrypting {} bytes of data with key {} using Azure Dedicated HSM",
            plaintext.len(),
            key_id
        );
        ...
        let iv = (0..12)
            .map(|i| ((key_id.to_string().len() + i * 7) % 256) as u8)
            .collect::<Vec<_>>();
        ciphertext.extend_from_slice(&iv);
        ...
        let tag = (0..16)
            .map(|i| ((plaintext.len() + i * 13) % 256) as u8)
            .collect::<Vec<_>>();
        ciphertext.extend_from_slice(&tag);
    ...
    async fn decrypt(&self, key_id: &KeyId, ciphertext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Decrypting {} bytes of data with key {} using Azure Dedicated HSM",
            ciphertext.len(),
            key_id
        );
    ...
    async fn health_check(&self) -> Result<bool> {
        ...
        // Check connection pool
        {
            ...
            if let Some(client) = client_guard.as_ref() {
                ...
                if metrics.error_rate > 0.4 {
                    log::warn!(
                        "Azure HSM error rate is high: {:.2}%",
                        metrics.error_rate * 100.0
                    );
                    health_status = false;
                }
        ...
        if health_status {
            log::info!(
                "Azure Dedicated HSM health check passed in {}ms",
                check_time
            );
        } else {
            log::warn!(
                "Azure Dedicated HSM health check failed in {}ms",
                check_time
            );
        }
    ...
    async fn shutdown(&self) -> Result<()> {
        ...
        let shutdown_time = start_time.elapsed().as_millis() as u64;
        log::info!(
            "Azure Dedicated HSM provider shutdown completed in {}ms",
            shutdown_time
        );
...
impl HsmProvider for GoogleCloudHsmProvider {
    async fn initialize(&self, config: &HsmConfig) -> Result<()> {
        ...
        match &config.connection {
            HsmConnection::Google {
                ...
                log::info!(
                    "Initializing Google Cloud HSM for project: {}, location: {}, key_ring: {}",
                    project_id,
                    location,
                    key_ring
                );
                ...
                match &config.credentials {
                    HsmCredentials::Google {
                        ...
                        for i in 0..2 {
                            if let Err(e) = self.get_connection().await {
                                log::warn!(
                                    "Failed to initialize initial Google connection {}: {:?}",
                                    i,
                                    e
                                );
                            }
                        ...
                        log::info!(
                            "Google Cloud HSM initialized successfully in {}ms",
                            init_time
                        );
                        Ok(())
                    }
    ...
    async fn generate_key(
        ...
        log::info!(
            "Generating key {} in Google Cloud HSM with algorithm: {}",
            key_id,
            algorithm.name()
        );
        ...
        log::info!(
            "Key {} generated successfully in Google Cloud HSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn get_key_metadata(&self, key_id: &KeyId) -> Result<KeyMetadata> {
        ...
        log::info!(
            "Retrieving metadata for key {} from Google Cloud HSM",
            key_id
        );
    ...
    async fn delete_key(&self, key_id: &KeyId) -> Result<()> {
        ...
        log::warn!(
            "Key {} permanently deleted from Google Cloud HSM in {}ms",
            key_id,
            operation_time
        );
        Ok(())
    ...
    async fn list_keys(&self) -> Result<Vec<(KeyId, KeyMetadata)>> {
        ...
        log::info!(
            "Listed {} keys from Google Cloud HSM in {}ms",
            keys.len(),
            operation_time
        );
        Ok(keys)
    ...
    async fn sign(&self, key_id: &KeyId, data: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Signing {} bytes of data with key {} using Google Cloud HSM",
            data.len(),
            key_id
        );
    ...
    async fn verify(&self, key_id: &KeyId, data: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        log::info!(
            "Verifying signature with key {} using Google Cloud HSM",
            key_id
        );
    ...
    async fn encrypt(&self, key_id: &KeyId, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Encrypting {} bytes of data with key {} using Google Cloud HSM",
            plaintext.len(),
            key_id
        );
        ...
        let iv = (0..12)
            .map(|i| ((key_id.to_string().len() + i * 11) % 256) as u8)
            .collect::<Vec<_>>();
        ciphertext.extend_from_slice(&iv);
        ...
        let tag = (0..16)
            .map(|i| ((plaintext.len() + i * 17) % 256) as u8)
            .collect::<Vec<_>>();
        ciphertext.extend_from_slice(&tag);
    ...
    async fn decrypt(&self, key_id: &KeyId, ciphertext: &[u8]) -> Result<Vec<u8>> {
        ...
        log::info!(
            "Decrypting {} bytes of data with key {} using Google Cloud HSM",
            ciphertext.len(),
            key_id
        );
        ...
        // Check connection pool
        {
            ...
            if let Some(client) = client_guard.as_ref() {
                ...
                if metrics.error_rate > 0.35 {
                    log::warn!(
                        "Google Cloud HSM error rate is high: {:.2}%",
                        metrics.error_rate * 100.0
                    );
                    health_status = false;
                }
                ...
                if metrics.avg_latency_ms > 750.0 {
                    log::warn!(
                        "Google Cloud HSM latency is high: {:.2}ms",
                        metrics.avg_latency_ms
                    );
                    health_status = false;
                }
    ...
    async fn shutdown(&self) -> Result<()> {
        ...
        let shutdown_time = start_time.elapsed().as_millis() as u64;
        log::info!(
            "Google Cloud HSM provider shutdown completed in {}ms",
            shutdown_time
        );
...
mod tests {
    use super::*;
    use crate::encryption::{Aes256Gcm, EncryptionAlgorithm};
    ...
    async fn test_aws_cloudhsm_initialization() {
        ...
        let provider = AwsCloudHsmProvider::new()
            .await
            .expect("AWS provider should create");
    ...
    async fn test_aws_cloudhsm_invalid_credentials() {
        ...
        assert!(
            result.is_err(),
            "AWS CloudHSM should reject invalid credentials"
        );
    }
    ...
    async fn test_pkcs11_initialization() {
        ...
        let provider = Pkcs11Provider::new()
            .await
            .expect("PKCS#11 provider should create");
        let result = provider.initialize(&config).await;
    ...
    async fn test_azure_hsm_initialization() {
        ...
        let provider = AzureDedicatedHsmProvider::new()
            .await
            .expect("Azure provider should create");
        let result = provider.initialize(&config).await;
    ...
    async fn test_google_cloud_hsm_initialization() {
        ...
        let provider = GoogleCloudHsmProvider::new()
            .await
            .expect("Google Cloud provider should create");
        let result = provider.initialize(&config).await;

        assert!(
            result.is_ok(),
            "Google Cloud HSM initialization should succeed"
        );
    }
    ...
    async fn test_hsm_key_manager_creation() {
        ...
        assert!(
            manager.is_ok(),
            "HSM key manager should create successfully"
        );
    }
    ...
    async fn test_hsm_key_generation() {
        ...
        let manager = HsmKeyManagerInner::new(config.clone())
            .await
            .expect("HSM manager should create");
    ...
    async fn test_hsm_uninitialized_errors() {
        ...
        let manager = HsmKeyManagerInner::new(config)
            .await
            .expect("HSM manager should create");
        let provider = manager.provider();
        ...
        assert!(
            result.is_err(),
            "Key generation should fail when not initialized"
        );

        let result = provider.get_key_metadata(&key_id).await;
        assert!(
            result.is_err(),
            "Metadata retrieval should fail when not initialized"
        );

        let result = provider.delete_key(&key_id).await;
        assert!(
            result.is_err(),
            "Key deletion should fail when not initialized"
        );
    }
    ...
    async fn test_hsm_connection_pooling() {
        ...
        for i in 0..5 {
            ...
            let result = provider_ref
                .generate_key(&key_id_clone, &algorithm_clone)
                .await;
