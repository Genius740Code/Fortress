...

        {
            ...
            match Client::connect(&conn_string, NoTls).await {
                ...
                Ok(mut client) => {
                    ...
                    if let Err(e) = client.execute(&create_user_query, &[&username, &password]).await {
                    ...
                    for permission in permissions {
                        ...
                        let (grant_query, params) = match permission.to_uppercase().as_str() {
                            ...
                            "SELECT" => ("GRANT SELECT ON ALL TABLES IN SCHEMA public TO $1", vec![username.clone()]),

                            "INSERT" => ("GRANT INSERT ON ALL TABLES IN SCHEMA public TO $1", vec![username.clone()]),

                            "UPDATE" => ("GRANT UPDATE ON ALL TABLES IN SCHEMA public TO $1", vec![username.clone()]),

                            "DELETE" => ("GRANT DELETE ON ALL TABLES IN SCHEMA public TO $1", vec![username.clone()]),

                            "ALL" => ("GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO $1", vec![username.clone()]),
                ...
                Ok(pool) => {
                    ...
                    match pool.get_conn().await {
                        ...
                        Ok(mut conn) => {
                            ...
                            if let Err(e) = conn.exec_drop(create_user_query, (&username, &password)).await {
                        ...
                        for permission in permissions {
                            let (grant_query, params) = match permission.to_uppercase().as_str() {
                                "SELECT" => ("GRANT SELECT ON ?.* TO ?@'%'", vec![database.to_string(), username.to_string()]),
                                "INSERT" => ("GRANT INSERT ON ?.* TO ?@'%'", vec![database.to_string(), username.to_string()]),
                                "UPDATE" => ("GRANT UPDATE ON ?.* TO ?@'%'", vec![database.to_string(), username.to_string()]),
                                "DELETE" => ("GRANT DELETE ON ?.* TO ?@'%'", vec![database.to_string(), username.to_string()]),
                                "ALL" => ("GRANT ALL PRIVILEGES ON ?.* TO ?@'%'", vec![database.to_string(), username.to_string()]),
                                _ => {
