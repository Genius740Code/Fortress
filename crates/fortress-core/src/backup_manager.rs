...
use crate::backup::{
    BackupConfig, BackupItem, BackupManager, BackupManifest, BackupMetadata, BackupStorageStats,
    BackupStrategy, RestoreOperationStatus, RestoreStatus, RetentionPolicy, VerificationLevel,
...
impl DefaultBackupManager {
    ...
    async fn backup_single_key(
        ...
        let data = source_storage.get(key).await?.ok_or_else(|| {
        ...
        self.process_backup_item(key.to_string(), data, backup_id, config)
            .await
    }
    ...
    async fn get_point_in_time_backup(
        &self,
    ...
    async fn restore_to_point_in_time(
        ...
        let base_backup = full_backups.first().ok_or_else(|| {
            FortressError::storage(
        ...
        let restore_status = self
            .restore_backup(&base_backup.backup_id, target_storage, config)
            .await?;
        ...
        for inc_backup in incremental_backups {
            let inc_restore_status = self
                .restore_backup(&inc_backup.backup_id, target_storage, config)
                .await?;
    ...
    async fn verify_backup_comprehensive(&self, backup_id: &str) -> Result<VerificationResult> {
        ...
        let manifest_data = serde_json::to_vec(&manifest).map_err(|e| {
            FortressError::storage(
    ...
    async fn restore_backup_item(
        ...
        // Store to target
        target_storage
            .put(&item.original_key, &restored_data)
            .await?;
        ...
        Box::pin(async move {
            ...
            let keys_to_backup = match &config.default_strategy {
                ...
                | BackupStrategy::Differential { base_backup_id } => {
                    ...
                    self.get_changed_keys_since_backup(source_storage, &base_metadata)
                        .await?
                }
            };
            ...
            if config.parallel_settings.enabled && keys_to_backup.len() > 1 {
                let stream = futures::stream::iter(keys_to_backup)
                    .map(|key| {
                        ...
                        async move {
                            let permit = manager.semaphore.acquire().await.map_err(|e| {
                                FortressError::storage(
                            ...
                            let result = manager
                                .backup_single_key(source_storage, &key, &backup_id, &config)
                                .await;
                            drop(permit);
                            result
                        }
            ...
            manifest_with_checksum.metadata.manifest_checksum =
                utils::calculate_checksum(&manifest_data);
        ...
        Box::pin(async move {
            ...
            for key in backup_keys {
                if key.ends_with("/manifest.json") {
                    let backup_id = key.strip_suffix("/manifest.json").unwrap_or("");
        ...
        Box::pin(async move {
            ...
            match level {
                ...
                VerificationLevel::Comprehensive => {
                    ...
                    if !self
                        .verify_backup(backup_id, VerificationLevel::Full)
                        .await?
                    {
                        return Ok(false);
                    }
                    ...
                    let restore_status = self
                        .restore_backup(backup_id, &*test_storage, &config)
                        .await?;
    ...
    fn cleanup_old_backups<'a>(
        ...
        Box::pin(async move {
            ...
            if full_backups.len() > policy.max_full_backups as usize {
                let to_delete =
                    &full_backups[..full_backups.len() - policy.max_full_backups as usize];
            ...
            if incremental_backups.len() > policy.max_incremental_backups as usize {
                let to_delete = &incremental_backups
                    [..incremental_backups.len() - policy.max_incremental_backups as usize];
                for backup in to_delete {
                    if backup.created_at < cutoff_date {
                        self.delete_backup(&backup.backup_id).await?;
                        deleted_count += 1;
            ...
            for backup in &backups {
                ...
                match backup.strategy {
                    ...
                    BackupStrategy::Incremental { .. } => {
                        stats.incremental_backup_storage += backup.total_size
                    }
                    BackupStrategy::Differential { .. } => {
                        stats.differential_backup_storage += backup.total_size
                    }
                }
            }
...
mod tests {
    ...
    async fn test_backup_manager_creation() {
        ...
        let manager = DefaultBackupManager::new(storage.clone(), None, config);
    ...
    async fn test_backup_creation_full() {
        ...
        let config = BackupConfig::default();
        let manager = DefaultBackupManager::new(backup_storage.clone(), None, config).unwrap();

        let backup_metadata = manager
            .create_backup(&*source_storage, &manager.get_config().await)
            .await
            .unwrap();
    ...
    async fn test_backup_verification() {
        ...
        let is_valid = manager
            .verify_backup(&backup_metadata.backup_id, VerificationLevel::Basic)
            .await
            .unwrap();
        assert!(is_valid);
        ...
        let is_valid = manager
            .verify_backup(&backup_metadata.backup_id, VerificationLevel::Full)
            .await
            .unwrap();
        assert!(is_valid);
    }
    ...
    async fn test_backup_list_and_delete() {
        ...
        let metadata = manager
            .get_backup_metadata(&backup_metadata.backup_id)
            .await
            .unwrap();
        ...
        // Delete backup
        manager
            .delete_backup(&backup_metadata.backup_id)
            .await
            .unwrap();
