...
use crate::auth_plugin_manager::{
    DeploymentStrategy, PluginDeployment, PluginManagerStats, PluginRegistryEntry,
    PluginReloadRequest,
};

#[cfg(test)]
use crate::auth_service::AuthServiceConfig;
use crate::auth_service::{AuthServiceStats, PluginAuthService, ServiceContext};

use crate::auth_plugin::{AuthMethod, AuthPluginMetadata, AuthRequest};
...
pub fn create_auth_api_routes(
    ...
    let list_plugins = warp::path("plugins")
        ...
        .and_then(|manager: Arc<AuthApiManager>| async move {
            match manager.list_plugins().await {
                Ok(plugins) => {
                    Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(plugins)))
                }
    ...
    let get_plugin = warp::path!("plugins")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |plugin_name: String, manager: Arc<AuthApiManager>| async move {
                match manager.get_plugin_metadata(&plugin_name).await {
                    Ok(metadata) => {
                        Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(metadata)))
                    }
    ...
    let deploy_plugin = warp::path("plugins")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |deployment: PluginDeployment, manager: Arc<AuthApiManager>| async move {
                match manager.deploy_plugin(deployment).await {
                    Ok(()) => Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(
    ...
    let reload_plugin = warp::path("plugins")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |request: ReloadPluginRequest, manager: Arc<AuthApiManager>| async move {
                match manager
                    .reload_plugin(&request.plugin_name, request.force)
                    .await
                {
                    Ok(()) => Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(
    ...
    let set_default_method = warp::path("methods")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |method: AuthMethod, manager: Arc<AuthApiManager>| async move {
    ...
    let get_default_method = warp::path("methods")
        ...
        .and_then(|manager: Arc<AuthApiManager>| async move {
            match manager.get_default_method().await {
                Ok(method) => {
                    Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(method)))
                }
    ...
    let health_check = warp::path("health")
        ...
        .and_then(|manager: Arc<AuthApiManager>| async move {
            match manager.health_check().await {
                Ok(health) => {
                    Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(health)))
                }
    ...
    let get_available_methods = warp::path("methods")
        ...
        .and_then(|manager: Arc<AuthApiManager>| async move {
            match manager.get_available_methods().await {
                Ok(methods) => {
                    Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(methods)))
                }
    ...
    let authenticate = warp::path("auth")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |request: AuthRequest, manager: Arc<AuthApiManager>| async move {
                ...
                match manager.auth_service.authenticate(request, context).await {
                    Ok(result) => {
                        Ok::<_, warp::Rejection>(warp::reply::json(&ApiResponse::success(result)))
                    }
    ...
    let validate_token = warp::path("auth")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |request: TokenValidationRequest, manager: Arc<AuthApiManager>| async move {
                let context = ServiceContext {
                    ip_address: None,
                ...
                match manager
                    ...
                    .await
                {
                    Ok(user_info) => Ok::<_, warp::Rejection>(warp::reply::json(
    ...
    let refresh_token = warp::path("auth")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |request: TokenRefreshRequest, manager: Arc<AuthApiManager>| async move {
                let context = ServiceContext {
                    ip_address: None,
    ...
    let logout = warp::path("auth")
        ...
        .and(with_auth_api_manager(api_manager_clone.clone()))
        .and_then(
            |request: LogoutRequest, manager: Arc<AuthApiManager>| async move {
                let context = ServiceContext {
                    ip_address: None,
...
pub fn get_api_documentation() -> ApiDocumentation {
    ApiDocumentation {
        ...
        description: "REST API for managing authentication plugins and performing authentication"
            .to_string(),
...
mod tests {
    ...
    async fn test_api_manager_creation() {
        let auth_service = Arc::new(
            PluginAuthService::new(AuthServiceConfig::default())
                .await
                .unwrap(),
        );
