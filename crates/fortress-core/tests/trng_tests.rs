...
fn test_entropy_sources() {
    ...
    for source in &sources {
        ...
        assert!(
            result.is_ok(),
            "Failed to collect entropy from {:?}",
            source
        );
...
fn test_random_byte_generation() {
    ...
    for &size in &sizes {
        ...
        if size >= 16 {
            assert_ne!(
                bytes1, bytes2,
                "Generated identical byte sequences of size {}",
                size
            );
        }
    ...
    for &size in &sizes {
        ...
        if size >= 8 {
            ...
            assert_ne!(
                buf,
                vec![0u8; size],
                "Buffer of size {} should not be all zeros",
                size
            );
        }
    }
...
fn test_performance_characteristics() {
    ...
    for _ in 0..100 {
        // Reduced count to avoid entropy depletion
        let _bytes = trng.generate_bytes(32).unwrap();
    }
