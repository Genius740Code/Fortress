...
async fn test_token_bucket_rate_limiting() {
    ...
    let result = rate_limiter.check_rate_limit(client_id).await;
    assert!(result.is_ok(), "Request should be allowed after token refill");
}
...
async fn test_sliding_window_rate_limiting() {
    ...
    assert!(allowed_count > 0, "Some requests should be allowed as window slides");
}
...
async fn test_concurrent_requests() {
    ...
    assert_eq!(allowed_count + blocked_count, 20, "Total requests should match");
}
