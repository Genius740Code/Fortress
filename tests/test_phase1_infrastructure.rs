...
use fortress_core::config::{
    AuditConfig, BackupConfig, Config, StorageConfig, StreamingConfig, TransactionConfig,
};
use fortress_core::error::{
    AuditErrorCode, BackupErrorCode, FortressError, StreamingErrorCode, TransactionErrorCode,
};
use fortress_core::storage::{
    AuditEvent, AuditEventOutcome, AuditEventType, BackupId, StreamId, TransactionId,
};
...
async fn test_storage_backend_extensions() {
    ...
    match tx_result.unwrap_err() {
        ...
        other => assert!(false, "Expected Storage error, got {:?}", other),
    }
...
fn test_configuration_extensions() {
    ...
    assert_eq!(
        audit_config.log_level,
        fortress_core::config::AuditLogLevel::Standard
    );
    assert!(audit_config
        .audited_events
