...
use fortress_core::auth::{AuthToken, TokenClaims, User};
use fortress_core::auth_plugin::{AuthPlugin, AuthPluginConfig, AuthPluginManager, PluginMetadata};
...
mod tests {
    ...
    async fn test_end_to_end_authentication_workflow() {
        ...
        plugin_manager
            .register_plugin("primary".to_string(), Box::new(primary_plugin))
            .await
            .expect("Primary plugin registration should succeed");

        plugin_manager
            .register_plugin("secondary".to_string(), Box::new(secondary_plugin))
            .await
            .expect("Secondary plugin registration should succeed");

        plugin_manager
            .register_plugin("audit".to_string(), Box::new(audit_plugin))
            .await
        ...
        let primary_result = auth_service
            .authenticate_with_plugin("primary", &user.username, "password123", &context)
            .await
            .expect("Primary authentication should succeed");

        assert!(
            primary_result.success,
            "Primary authentication should succeed"
        );
        assert!(!primary_result.user_id.is_empty(), "Should return user ID");
        assert!(
            primary_result.token.is_some(),
            "Should return authentication token"
        );

        // Step 2: Secondary authentication (2FA)
        let secondary_result = auth_service
            .authenticate_with_plugin(
                "secondary",
                &user.username,
                "123456", // 2FA code
                &context,
            ...
            .await
            .expect("Secondary authentication should succeed");

        assert!(
            secondary_result.success,
            "Secondary authentication should succeed"
        );

        // Step 3: Token validation
        if let Some(token) = primary_result.token {
            let validation_result = auth_service
                .validate_token_with_plugin("primary", &token, &context)
                .await
                .expect("Token validation should succeed");

            assert!(validation_result.is_valid, "Token should be valid");
            assert_eq!(
                validation_result.user_id, primary_result.user_id,
                "Validation should return same user ID"
            );
        }

        // Step 4: Audit logging
        let audit_result = auth_service
            .log_authentication_event("audit", &user.username, "login_success", &context)
            .await
        ...
        let session_result = auth_service
            .create_session(&primary_result.user_id, &context)
            .await
            .expect("Session creation should succeed");

        assert!(
            !session_result.session_id.is_empty(),
            "Should create session"
        );
        assert!(
            session_result.expires_at > Utc::now().timestamp() as u64,
            "Session should have future expiration"
        );

        // Verify end-to-end workflow metrics
        let workflow_metrics = auth_service
            .get_workflow_metrics()
            .await
            .expect("Workflow metrics should be available");

        assert!(
            workflow_metrics.total_workflows > 0,
            "Should have completed workflows"
        );
        assert!(
            workflow_metrics.success_rate > 0.0,
    ...
    async fn test_multi_plugin_authentication_orchestration() {
        ...
        plugin_manager
            .register_plugin("orchestrator".to_string(), Box::new(orchestrator))
            .await
            .expect("Orchestrator plugin registration should succeed");

        plugin_manager
            .register_plugin("validator".to_string(), Box::new(validator))
            .await
            .expect("Validator plugin registration should succeed");

        plugin_manager
            .register_plugin("enhancer".to_string(), Box::new(enhancer))
            .await
        ...
        let orchestration_result = auth_service
            .setup_authentication_workflow(
                "orchestrator",
                vec!["validator".to_string(), "enhancer".to_string()],
                &context,
            )
            .await
            .expect("Orchestration setup should succeed");

        assert!(
            orchestration_result.success,
            "Orchestration setup should succeed"
        );

        // Step 2: Multi-step authentication
        let multi_step_result = auth_service
            .authenticate_with_workflow(username, "password123", &context)
            .await
            .expect("Multi-step authentication should succeed");

        assert!(
            multi_step_result.success,
            "Multi-step authentication should succeed"
        );

        // Step 3: Verify all plugins participated
        let plugin_participation = auth_service
            .get_plugin_participation()
            .await
            .expect("Plugin participation should be available");

        assert!(
            plugin_participation.contains_key("validator"),
            "Validator plugin should have participated"
        );
        assert!(
            plugin_participation.contains_key("enhancer"),
            "Enhancer plugin should have participated"
        );

        // Step 4: Test plugin chaining
        let chain_result = auth_service
            .chain_authentication(
                username,
                vec![
            ...
            .await
            .expect("Chained authentication should succeed");

        assert!(
            chain_result.success,
            "Chained authentication should succeed"
        );
        assert!(
            chain_result
                .enhanced_metadata
                .contains_key("validation_passed"),
            "Should have validation metadata"
        );
        assert!(
            chain_result
                .enhanced_metadata
                .contains_key("enhancement_applied"),
            "Should have enhancement metadata"
        );

        // Step 5: Test plugin fallback
        let fallback_result = auth_service
            ...
            .await
            .expect("Fallback authentication should succeed");

        assert!(
            fallback_result.success,
            "Fallback authentication should succeed"
        );
        assert_eq!(
            fallback_result.used_plugin, "validator",
            "Should have used fallback plugin"
        );
    ...
    async fn test_plugin_integration_security_systems() {
        ...
        plugin_manager
            .register_plugin("security".to_string(), Box::new(security_plugin))
            .await
            .expect("Security plugin registration should succeed");

        plugin_manager
            .register_plugin("threat".to_string(), Box::new(threat_plugin))
            .await
            .expect("Threat plugin registration should succeed");

        plugin_manager
            .register_plugin("compliance".to_string(), Box::new(compliance_plugin))
            .await
        ...
        let security_result = auth_service
            .authenticate_with_security_validation(
                "security",
                username,
                "secure_password",
                &context,
        ...
        assert!(
            ...
            "Security validation should succeed"
        );
        assert!(
        ...
        let threat_result = auth_service
            .detect_threats("threat", &security_result.user_id, &context)
            .await
            .expect("Threat detection should succeed");

        assert!(
            threat_result.threats_detected.len() >= 0,
            "Should detect threats (or none)"
        );
        assert!(
            threat_result.risk_assessment.is_some(),
            "Should provide risk assessment"
        );

        // Step 3: Compliance validation
        let compliance_result = auth_service
            .validate_compliance("compliance", &security_result.user_id, &context)
            .await
        ...
        assert!(
            ...
            "Should list compliance requirements"
        );
        ...
        let security_metrics = auth_service
            .get_security_metrics()
            .await
            .expect("Security metrics should be available");

        assert!(
            security_metrics.total_authentications > 0,
            "Should have authentication metrics"
        );
        assert!(
            security_metrics.average_security_score > 0.0,
            "Should have security score metrics"
        );
        assert!(
            security_metrics.threats_blocked >= 0,
            "Should have threat metrics"
        );
        ...
        println!(
            "  Threats detected: {}",
            threat_result.threats_detected.len()
        );
    ...
    async fn test_performance_testing_under_load() {
        ...
        for i in 0..5 {
            ...
            plugin_manager
                .register_plugin(format!("perf_{}", i), Box::new(perf_plugin))
                .await
                .expect(&format!(
                    "Performance plugin {} registration should succeed",
                    i
                ));
        }

        // Test load balancing
        let load_balance_result = auth_service
            ...
            .await
            .expect("Load balancing configuration should succeed");
        ...
        for i in 0..num_concurrent_requests {
            ...
            let handle = tokio::spawn(async move {
                ...
                let result = auth_service_clone
                    .authenticate_with_load_balancing(&username, "password123", &context)
                    .await;
        ...
        let max_duration = response_times
            .iter()
            .max()
            .unwrap_or(&std::time::Duration::ZERO);
        let min_duration = response_times
            .iter()
            .min()
            .unwrap_or(&std::time::Duration::ZERO);
        ...
        assert!(
            successful_requests > num_concurrent_requests * 80 / 100,
        ...
        for i in 0..5 {
            ...
            let metrics = auth_service
                .get_plugin_performance_metrics(&plugin_name)
                .await
                .expect(&format!(
                    "Plugin {} metrics should be available",
                    plugin_name
                ));

            assert!(
                metrics.total_requests > 0,
                "Plugin {} should have requests",
                plugin_name
            );
            assert!(
                metrics.average_response_time_ms > 0,
                "Plugin {} should have response time",
                plugin_name
            );
        ...
        let system_metrics = auth_service
        ...
        assert!(
            system_metrics.average_response_time_ms > 0,
            "Should have system response time"
        );
    ...
    async fn test_plugin_failure_scenarios_recovery() {
        ...
        plugin_manager
            .register_plugin("flaky".to_string(), Box::new(flaky_plugin))
            .await
            .expect("Flaky plugin registration should succeed");

        plugin_manager
            .register_plugin("recovery".to_string(), Box::new(recovery_plugin))
            .await
        ...
        plugin_manager
            .register_plugin(
                ...
                Box::new(circuit_breaker_plugin),
            )
            .await
        ...
        for i in 0..10 {
            let result = auth_service
                .authenticate_user("flaky", &format!("user_{}", i), "password123", &context)
                .await;
        ...
        println!(
            "Flaky plugin results: {} successes, {} failures",
            flaky_successes, flaky_failures
        );
        ...
        let recovery_first = auth_service
            .authenticate_user("recovery", "recovery_user", "password123", &context)
            .await;

        assert!(recovery_first.is_err(), "First recovery call should fail");

        // Second call should succeed (recovery)
        let recovery_second = auth_service
            .authenticate_user("recovery", "recovery_user", "password123", &context)
            .await;

        assert!(
            recovery_second.is_ok(),
            "Second recovery call should succeed"
        );

        if let Ok(auth_result) = recovery_second {
            assert!(
                auth_result.success,
                "Recovered authentication should succeed"
            );
        }
        ...
        // Normal operation
        let circuit_normal = auth_service
            .authenticate_user("circuit_breaker", "normal_user", "password123", &context)
            .await;

        assert!(
            circuit_normal.is_ok(),
            "Normal circuit breaker operation should succeed"
        );
        ...
        let circuit_broken = auth_service
            .authenticate_user("circuit_breaker", "valid_user", "password123", &context)
            .await;

        assert!(
            circuit_broken.is_err(),
            "Broken circuit should reject requests"
        );

        // Test plugin health monitoring
        let health_status = auth_service
            .get_plugin_health_status()
            .await
            .expect("Plugin health status should be available");

        assert!(
            health_status.contains_key("flaky"),
            "Should have flaky plugin status"
        );
        assert!(
            health_status.contains_key("recovery"),
            "Should have recovery plugin status"
        );
        assert!(
            health_status.contains_key("circuit_breaker"),
            "Should have circuit breaker plugin status"
        );

        // Test auto-recovery
        let recovery_status = auth_service
            .attempt_plugin_recovery("circuit_breaker")
            .await
        ...
        let fallback_config = auth_service
            .configure_fallback_plugins(
                vec![("flaky".to_string(), "recovery".to_string())],
                FallbackStrategy::Failover,
            )
            .await
            .expect("Fallback configuration should succeed");

        assert!(
            fallback_config.success,
            "Fallback configuration should succeed"
        );
        ...
        assert_eq!(
            fallback_result.used_plugin, "recovery",
            "Should use fallback plugin"
        );

        println!("Plugin failure and recovery testing completed");
        println!(
            "  Flaky plugin: {} successes, {} failures",
            flaky_successes, flaky_failures
        );
    ...
    impl AuthPlugin for OrchestrationAuthPlugin {
        async fn authenticate(
            ...
            println!(
                "ORCHESTRATION: Orchestrating authentication for user: {}",
                username
            );
    ...
    impl AuthPlugin for FlakyAuthPlugin {
        async fn authenticate(
            ...
            if (hash as f64 / u64::MAX) < self.failure_rate {
                Err(FortressError::auth(
                    "Random failure".to_string(),
                    AuthErrorCode::TemporaryFailure,
                ))
            } else {
                Ok(AuthResult {
                    success: true,
        ...
        async fn authenticate(
            ...
            if state == 1 {
                ...
                if password == "wrong_password" {
                    ...
                    self.failure_count
                        .store(new_failures, std::sync::atomic::Ordering::SeqCst);
                    ...
                    if new_failures >= 3 {
                        ...
                        return Err(FortressError::auth(
                            ...
                            AuthErrorCode::CircuitBreakerTripped,
                        ));
                    }
