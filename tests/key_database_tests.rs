...
use fortress_core::key_database::{
    KeyDatabase, KeyDatabaseBackend, KeyDatabaseConfig, SqliteKeyDatabase,
};
use std::path::Path;
...
mod tests {
    ...
    fn create_test_metadata(algorithm_name: &str, version: u32) -> KeyMetadata {
        ...
        metadata
            .metadata
            .insert("database".to_string(), "test".to_string());
        metadata
            .metadata
            .insert("purpose".to_string(), "testing".to_string());
        metadata
    }
    ...
    async fn test_database_connection_management() {
        ...
        let db = SqliteKeyDatabase::new(config)
            .await
        ...
        let db2 = SqliteKeyDatabase::new(config2)
            .await
        ...
        db2.store_key(&key_id, &key, &metadata)
            .await
        ...
        assert_eq!(
            retrieved_key.to_vec(),
            key.to_vec(),
            "Encrypted key should match original"
        );
        assert_eq!(
            retrieved_metadata.algorithm, metadata.algorithm,
            "Encrypted metadata should match"
        );
    }
    ...
    async fn test_query_optimization() {
        ...
        for i in 0..num_keys {
            ...
            db.store_key(&key_id, &key, &metadata)
                .await
                .expect(&format!(
                    "Query optimization key {} storage should succeed",
                    i
                ));
        ...
        println!(
            "Retrieved {} keys in {:?}",
            successful_retrievals, retrieval_duration
        );

        assert_eq!(
            successful_retrievals, num_keys,
        ...
        assert_eq!(
            page2_ids.intersection(&page3_ids).count(),
            0,
            "Pages 2 and 3 should not overlap"
        );
        assert_eq!(
            page3_ids.intersection(&page4_ids).count(),
            0,
            "Pages 3 and 4 should not overlap"
        );
        ...
        println!(
            "Checked existence for {} keys in {:?}",
            successful_exists, existence_duration
        );
        ...
        assert!(
            ...
            "Storage should complete in reasonable time"
        );
        assert!(
        ...
        assert!(
            pagination_duration.as_millis() < 1000,
            "Pagination should be fast"
        );
        assert!(
            existence_duration.as_millis() < 2000,
            "Existence checks should be fast"
        );
    }
    ...
    async fn test_data_integrity_validation() {
        ...
        for (i, size) in test_sizes.iter().enumerate() {
            ...
            assert!(
                retrieved.is_some(),
                "Integrity key {} should be retrievable",
                i
            );
            ...
            assert_eq!(
                retrieved_key.to_vec(),
                key.to_vec(),
                "Integrity key {} data should match",
                i
            );
            assert_eq!(
                retrieved_metadata.algorithm, metadata.algorithm,
                "Integrity key {} metadata should match",
                i
            );
            assert_eq!(
                retrieved_metadata.version, metadata.version,
                "Integrity key {} version should match",
                i
            );
        ...
        for (i, key_id) in key_ids.iter().enumerate() {
            ...
            assert!(
                metadata.metadata.contains_key("test"),
                "Should have test tag"
            );
            assert!(
                metadata.metadata.contains_key("database"),
                "Should have database tag"
            );
            assert!(
                metadata.metadata.contains_key("purpose"),
                "Should have purpose metadata field"
            );
        }
        ...
        db.store_key(&test_key_id, &updated_key, &updated_metadata)
            .await
        ...
        assert_eq!(
            upd_key.to_vec(),
            updated_key.to_vec(),
            "Updated key should match"
        );
        assert_eq!(upd_metadata.version, 2, "Updated version should match");
        assert!(
            upd_metadata.metadata.contains_key("updated"),
        ...
        assert_ne!(
            upd_key.to_vec(),
            original_key.to_vec(),
            "Old key data should be replaced"
        );
    }
    ...
    async fn test_backup_restore_operations() {
        ...
        if Path::new(&backup_path).exists() {
            fs::copy(
                &backup_path,
                db_path.strip_prefix("sqlite:").unwrap_or(&db_path),
            )
            .await
    ...
    async fn test_migration_scenarios() {
        ...
        let db_v1 = SqliteKeyDatabase::new(config_v1)
            .await
        ...
        db_v1
            .store_key(&v1_key_id, &v1_key, &v1_metadata)
            .await
            .expect("V1 key storage should succeed");

        // Verify V1 data
        let v1_retrieved = db_v1
            .retrieve_key(&v1_key_id)
            .await
        ...
        assert_eq!(
            v1_key_data.to_vec(),
            v1_key.to_vec(),
            "V1 key data should match"
        );
        assert_eq!(
            v1_meta_data.metadata.get("schema_version"),
            Some(&"1".to_string()),
            "V1 schema version should match"
        );
        ...
        v2_metadata
            .metadata
            .insert("schema_version".to_string(), "2".to_string());
        v2_metadata
            .metadata
            .insert("migration_date".to_string(), Utc::now().to_rfc3339());
        v2_metadata
            .metadata
            .insert("migrated".to_string(), "true".to_string());

        db_v1
            .store_key(&v2_key_id, &v2_key, &v2_metadata)
            .await
            .expect("V2 key storage should succeed");

        // Verify V2 data
        let v2_retrieved = db_v1
            .retrieve_key(&v2_key_id)
            .await
        ...
        assert_eq!(
            v2_key_data.to_vec(),
            v2_key.to_vec(),
            "V2 key data should match"
        );
        assert_eq!(
            v2_meta_data.metadata.get("schema_version"),
            Some(&"2".to_string()),
            "V2 schema version should match"
        );
        assert!(
            v2_meta_data.metadata.contains_key("migration_date"),
            "V2 should have migration date"
        );
        assert!(
            v2_meta_data.metadata.contains_key("migrated"),
            "V2 should have migrated tag"
        );

        // Test backward compatibility
        let v1_retrieved_after_migration = db_v1
            .retrieve_key(&v1_key_id)
            .await
            .expect("V1 retrieval after migration should succeed");
        assert!(
            v1_retrieved_after_migration.is_some(),
            "V1 key should still be retrievable after migration"
        );
        ...
        for i in 0..num_migration_keys {
            ...
            metadata
                .metadata
                .insert("migrated_at".to_string(), Utc::now().to_rfc3339());
        ...
        for (i, key_id) in migration_key_ids.iter().enumerate() {
            ...
            assert!(
                retrieved.is_some(),
                "Migrated key {} should be retrievable",
                i
            );
            ...
            assert_eq!(
                metadata.version, 2,
                "Migrated key {} should have version 2",
                i
            );
            assert_eq!(
                metadata.metadata.get("schema_version"),
                Some(&"2".to_string()),
                "Migrated key {} should have schema version 2",
                i
            );
        }
    }
    ...
    async fn test_database_corruption_recovery() {
        ...
        db.store_key(&critical_key_id, &critical_key, &critical_metadata)
            .await
            .expect("Critical key storage should succeed");

        // Verify critical data exists
        let critical_retrieved = db
            .retrieve_key(&critical_key_id)
            .await
            .expect("Critical key retrieval should succeed");
        assert!(
            critical_retrieved.is_some(),
            "Critical key should be retrievable"
        );
        ...
        for i in 0..10 {
            ...
            db.store_key(&test_key_id, &test_key, &test_metadata)
                .await
            ...
            assert!(
                retrieved.is_some(),
                "Recovery test key {} should be retrievable",
                i
            );

            // Clean up
            db.delete_key(&test_key_id)
                .await
                .expect(&format!("Recovery test key {} deletion should succeed", i));
        }

        // Verify critical data is still intact
        let critical_retrieved_after = db
            .retrieve_key(&critical_key_id)
            .await
            .expect("Critical key retrieval after recovery tests should succeed");
        assert!(
            critical_retrieved_after.is_some(),
            "Critical key should still be retrievable"
        );
        ...
        assert_eq!(
            final_key.to_vec(),
            critical_key.to_vec(),
            "Critical key data should be intact"
        );
        assert_eq!(
            final_metadata.algorithm, critical_metadata.algorithm,
            "Critical metadata should be intact"
        );
    }
    ...
    async fn test_concurrent_database_operations() {
        ...
        assert_eq!(
            successful_reads, num_concurrent_writes,
        ...
        for i in 0..num_mixed_operations {
            ...
            let handle = tokio::spawn(async move {
                ...
                db_clone
                    .store_key(&write_key_id, &write_key, &write_metadata)
                    .await
                    .expect("Mixed write should succeed");

                // Read operation
                let read_result = db_clone
                    .retrieve_key(&write_key_id)
                    .await
                    .expect("Mixed read should succeed");
                assert!(read_result.is_some(), "Mixed read should find key");

                // Metadata operation
                let metadata_result = db_clone
                    .get_key_metadata(&write_key_id)
                    .await
                ...
                // Cleanup
                db_clone
                    .delete_key(&write_key_id)
                    .await
                    .expect("Mixed deletion should succeed");

                // Verify deletion
                let exists_after_delete = db_clone
                    .key_exists(&write_key_id)
                    .await
        ...
        let final_stats = db.get_stats().await.expect("Final stats should succeed");
        assert_eq!(
            final_stats.total_keys, num_concurrent_writes as u64,
            "Should have original concurrent write keys"
        );
    }
    ...
    async fn test_database_performance_under_load() {
        ...
        println!(
            "Batch checked {} keys in {:?}",
            batch_successful, batch_duration
        );
        ...
        for _ in 0..10 {
            let stats = db
                ...
                .expect("Stats retrieval should succeed");
            assert!(
                stats.total_keys >= num_operations,
                "Stats should show correct key count"
            );
        }
        ...
        assert_eq!(
            batch_successful, num_operations,
            "All batch checks should succeed"
        );

        assert!(
            write_duration.as_millis() < 30000,
            "Write performance should be reasonable"
        );
        assert!(
            read_duration.as_millis() < 15000,
            "Read performance should be reasonable"
        );
        assert!(
            batch_duration.as_millis() < 5000,
            "Batch performance should be good"
        );
        assert!(
            stats_duration.as_millis() < 1000,
            "Stats performance should be fast"
        );
        ...
        assert!(
            cleanup_duration.as_millis() < 20000,
            "Cleanup performance should be reasonable"
        );
