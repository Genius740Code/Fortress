...
use fortress_core::auth::{
    AuthPermission, AuthToken, PermissionId, Role, RoleId, TokenClaims, User, UserId,
};
use fortress_core::error::{AuthErrorCode, FortressError, Result};
...
mod tests {
    ...
    fn hash_password(password: &str) -> Result<String> {
        ...
        let password_hash = argon2.hash_password(password, &salt).map_err(|_| {
            FortressError::auth(
    ...
    fn verify_password(password: &str, hash: &str) -> Result<bool> {
        ...
        argon2.verify_password(password, &parsed_hash).map_err(|_| {
            FortressError::auth(
    ...
    async fn test_argon2id_password_hashing_expanded() {
        ...
        assert!(
            verify_password(same_password, &hash1).unwrap(),
            "First hash should verify"
        );
        assert!(
            verify_password(same_password, &hash2).unwrap(),
    ...
    async fn test_multi_factor_authentication() {
        ...
        assert!(
            mfa_verified,
            "MFA should be verified with either TOTP or backup code"
        );
        ...
        assert!(
            mfa_required,
            "User with mfa_enabled role should require MFA"
        );
    ...
    async fn test_token_generation_validation() {
        ...
        assert!(
            ...
            "Token should expire after issuance"
        );
        assert_eq!(
            token.permissions.len(),
            permissions.len(),
            "Token should have correct permissions"
        );
        ...
        let deserialized_token: Result<AuthToken, serde_json::Error> =
        ...
        assert_eq!(
            deserialized.user_id, token.user_id,
            "Deserialized token should match original"
        );
        assert_eq!(
            deserialized.token, token.token,
            "Deserialized token value should match original"
        );
    }
    ...
    async fn test_session_management() {
        ...
        assert_eq!(
            ...
            "Stored session should have correct user"
        );
        assert_eq!(
            last_activity, now,
            "Stored session should have correct activity time"
        );
        ...
        assert_eq!(
            updated_last_activity, updated_activity,
        ...
        assert!(
            !sessions.contains_key(&expired_session_id),
        ...
        assert_eq!(
            sessions_read.len(),
            10,
            "All concurrent sessions should be stored"
        );
    }
    ...
    async fn test_password_policy_enforcement() {
        ...
        let invalid_passwords = vec![
            ...
            (
                "Password123",
                "Password contains forbidden pattern: password",
            ),
            (
                "mypassword",
                "Password contains forbidden pattern: mypassword",
            ),
        ...
        // Test edge cases
        assert!(
            validate_password("Aa1!", &policy).is_ok(),
            "Minimum valid password should pass"
        );
        assert!(
            validate_password(
                "Aa1!",
    ...
    async fn test_account_lockout_mechanisms() {
        ...
        for attempt in 1..=max_attempts {
            let (attempts, last_attempt_time) = failed_attempts
            ...
            *last_attempt_time = Utc::now().timestamp() as u64;
            ...
            assert_eq!(
                ...
                "Account should be locked after {} attempts",
                max_attempts
            );
        }
        ...
        assert_eq!(
            ...
            "Failed attempts should reach maximum"
        );
        ...
        assert!(
            will_be_unlocked,
            "Account should be unlocked after lockout duration"
        );
        ...
        assert!(
            successful_login_after_lockout,
        ...
        let (partial_attempts, partial_last_attempt) =
            failed_attempts.get(partial_user_id).unwrap();
        let is_partially_locked = partial_attempts >= max_attempts;
        assert!(
            !is_partially_locked,
            "Partial attempts should not trigger lockout"
        );
        ...
        let user2_locked = failed_attempts
            .get(user2_id)
            .map(|(attempts, _)| *attempts >= max_attempts)
            .unwrap_or(false);
        assert!(
            !user2_locked,
            "User2 should not be locked by User1's failures"
        );

        // Lock user2
        failed_attempts.insert(user2_id.to_string(), (max_attempts, now));

        let user2_locked_after = failed_attempts
            .get(user2_id)
            .map(|(attempts, _)| *attempts >= max_attempts)
            .unwrap_or(false);
        ...
        let user1_locked_final = failed_attempts
            .get(user1_id)
            .map(|(attempts, _)| *attempts >= max_attempts)
            .unwrap_or(false);
        let user2_locked_final = failed_attempts
            .get(user2_id)
            .map(|(attempts, _)| *attempts >= max_attempts)
            .unwrap_or(false);

        assert!(
            user1_locked_final && user2_locked_final,
            "Both users should be locked"
        );
    }
    ...
    async fn test_brute_force_protection() {
        ...
        for attempt in 1..=10 {
            let (attempts, last_attempt) = ip_attempts
            ...
            *last_attempt = Utc::now().timestamp() as u64;
            ...
            assert!(
                !should_be_blocked,
                "IP should not be blocked after {} attempts (below limit)",
                attempt
            );
        }
        ...
        for (i, username) in usernames.iter().enumerate() {
            ...
            *attempts += 1;
            ...
            assert!(
                !should_be_blocked_globally,
                "Username {} should not be blocked after {} attempts",
                username,
                i + 1
            );
        }
        ...
        assert!(
            is_blocked_globally,
            "Username should be blocked at global threshold"
        );
        ...
        for (max_att, window_sec) in progressive_limits {
            ...
            let now = Utc::now().timestamp() as u64;
            let recent_count = temp_attempts
                .get(test_ip)
                .map(|timestamps| {
                    timestamps
                        .iter()
                        .filter(|&&t| *t >= now - window_sec as u64)
                        .count()
                })
                .unwrap_or(0);

            assert_eq!(
                recent_count, max_att,
                "Should count {} attempts within {} second window",
                max_att, window_sec
            );
        }
        ...
        assert!(
            captcha_required_ips.contains(suspicious_ip),
            "CAPTCHA should be required after threshold"
        );
    ...
    async fn test_authentication_performance() {
        ...
        assert!(
            hashing_duration.as_millis() < 1000,
            "Password hashing should be fast"
        );

        // Test password verification performance
        let hashes: Vec<String> = test_passwords
            .iter()
        ...
        println!(
            "Verified {} passwords in {:?}",
            test_passwords.len(),
            verification_duration
        );

        assert!(
            verification_duration.as_millis() < 500,
            "Password verification should be very fast"
        );
        ...
        assert!(
            token_generation_duration.as_millis() < 100,
        ...
        assert!(
            session_creation_duration.as_millis() < 200,
        ...
        println!(
            "Looked up {} sessions in {:?}",
            successful_lookups, session_lookup_duration
        );
        ...
        assert!(
            session_lookup_duration.as_millis() < 100,
            "Session lookup should be very fast"
        );

        // Performance assertions
        assert!(
            hashing_duration.as_millis() < verification_duration.as_millis(),
            "Hashing should be faster than verification"
        );
        assert!(
            verification_duration.as_millis() < token_generation_duration.as_millis(),
            "Verification should be faster than token generation"
        );
        assert!(
            token_generation_duration.as_millis() < session_creation_duration.as_millis(),
    ...
    async fn test_user_lifecycle_management() {
        ...
        assert_eq!(
            stored_user.username, "lifecycle_user",
            "Stored user should have correct username"
        );
        assert_eq!(
            stored_user.email, "lifecycle@example.com",
        ...
        assert_eq!(
            updated_stored_user.full_name, "Updated Name",
            "User name should be updated"
        );
        assert!(
            updated_stored_user.last_login.is_some(),
            "Last login should be set"
        );
        assert!(
            updated_stored_user.roles.contains(&"admin".to_string()),
            "Admin role should be added"
        );
        ...
        assert!(
            !deactivated_stored_user.active,
    ...
    async fn test_role_based_access_control() {
        ...
        let user_role = create_test_role("user", "Regular User", vec!["read:own", "write:own"]);

        let readonly_role = create_test_role("readonly", "Read-Only User", vec!["read:all"]);
        ...
        assert!(
            admin_permissions.contains(&"read:all".to_string()),
            "Admin role should have read:all permission"
        );
        assert!(
            admin_permissions.contains(&"write:all".to_string()),
            "Admin role should have write:all permission"
        );
        assert!(
            admin_permissions.contains(&"delete:all".to_string()),
            "Admin role should have delete:all permission"
        );
        ...
        assert!(
            user_permissions.contains(&"read:own".to_string()),
            "User should have read:own permission"
        );
        assert!(
            user_permissions.contains(&"read:all".to_string()),
            "User should have read:all permission from readonly role"
        );
        assert!(
            !user_permissions.contains(&"write:all".to_string()),
            "User should not have write:all permission"
        );
        assert!(
            !user_permissions.contains(&"delete:all".to_string()),
            "User should not have delete:all permission"
        );
        ...
        assert!(
            has_permission(&user_permissions, "own", "read"),
            "User should have read:own permission"
        );
        assert!(
            has_permission(&user_permissions, "all", "read"),
            "User should have read:all permission"
        );
        assert!(
            !has_permission(&user_permissions, "all", "write"),
            "User should not have write:all permission"
        );
        assert!(
            !has_permission(&user_permissions, "all", "delete"),
            "User should not have delete:all permission"
        );
        ...
        assert!(
            has_permission(&admin_permissions, "all", "read"),
            "Admin should have read:all permission"
        );
        assert!(
            has_permission(&admin_permissions, "all", "write"),
            "Admin should have write:all permission"
        );
        assert!(
            has_permission(&admin_permissions, "all", "delete"),
            "Admin should have delete:all permission"
        );
        assert!(
            has_permission(&admin_permissions, "users", "manage"),
            "Admin should have manage:users permission"
        );
        ...
        // Verify sorting
        assert_eq!(
            sorted_roles[0], readonly_role.id,
            "Lowest role should be readonly"
        );
        assert_eq!(sorted_roles[1], user_role.id, "Middle role should be user");
        assert_eq!(
            sorted_roles[2], admin_role.id,
            "Highest role should be admin"
        );
        ...
        assert!(
            updated_user_permissions.contains(&"read:own".to_string()),
            "User should still have permissions from active role"
        );
        assert!(
            updated_user_permissions.contains(&"read:all".to_string()),
    ...
    async fn test_permission_inheritance_hierarchy() {
        ...
        let read_secrets =
            create_test_permission("read:secrets", "Read secret data", "secrets", "read");

        let write_secrets =
            create_test_permission("write:secrets", "Write secret data", "secrets", "write");

        let delete_secrets =
            create_test_permission("delete:secrets", "Delete secret data", "secrets", "delete");

        let admin_secrets =
            create_test_permission("admin:secrets", "Administer secrets", "secrets", "admin");

        // Create wildcard permissions
        let read_all = create_test_permission("read:all", "Read all data", "*", "read");

        let write_all = create_test_permission("write:all", "Write all data", "*", "write");
        ...
        assert!(
            check_permission(&user_permissions, "secrets", "read", &permissions),
            "Should have exact read:secrets permission"
        );
        assert!(
            check_permission(&user_permissions, "secrets", "write", &permissions),
            "Should have exact write:secrets permission"
        );
        assert!(
            !check_permission(&user_permissions, "secrets", "delete", &permissions),
        ...
        assert!(
            check_permission(&user_permissions, "users", "read", &permissions),
        ...
        let admin_permissions = HashSet::from([admin_secrets.id.clone(), read_all.id.clone()]);

        assert!(
            check_permission(&admin_permissions, "secrets", "admin", &permissions),
            "Should have exact admin:secrets permission"
        );
        assert!(
            check_permission(&admin_permissions, "secrets", "read", &permissions),
            "Should match read:secrets with read:all wildcard"
        );
        assert!(
            check_permission(&admin_permissions, "secrets", "write", &permissions),
            "Should not match write:secrets without write:all wildcard"
        );

        // Test permission priority (exact > wildcard)
        let priority_permissions = HashSet::from([read_secrets.id.clone(), read_all.id.clone()]);

        // Should prefer exact permission over wildcard
        assert!(
            check_permission(&priority_permissions, "secrets", "read", &permissions),
            "Should use exact read:secrets over read:all wildcard"
        );

        // Test resource-specific wildcard
        let secrets_wildcard =
            create_test_permission("secrets:*", "All secrets operations", "secrets", "*");

        permissions.insert(secrets_wildcard.id.clone(), secrets_wildcard);

        let secrets_wildcard_permissions = HashSet::from([secrets_wildcard.id.clone()]);

        assert!(
            check_permission(
                &secrets_wildcard_permissions,
                "secrets",
                "read",
                &permissions
            ),
        ...
        assert!(
            !check_permission(&secrets_wildcard_permissions, "users", "read", &permissions),
    ...
    async fn test_authentication_token_security() {
        ...
        assert_eq!(
            token.user_id, token2.user_id,
            "Tokens for same user should have same user_id"
        );
        ...
        assert!(
            unique_chars.len() > 10,
            "Token should have good character diversity"
        );
        ...
        assert!(
            short_lived_token.expires_at < long_lived_token.expires_at,
            "Short-lived token should expire sooner"
        );
        ...
        assert!(
            admin_token.permissions.len() > limited_token.permissions.len(),
            "Admin token should have more permissions"
        );
        ...
        let deserialized_token: Result<AuthToken, serde_json::Error> =
            serde_json::from_str(&token_json.unwrap());
        assert!(
            deserialized_token.is_ok(),
            "Token should deserialize securely"
        );

        let deserialized = deserialized_token.unwrap();
        assert_eq!(
            deserialized.user_id, admin_token.user_id,
            "Deserialized token should have correct user_id"
        );
        assert_eq!(
            deserialized.permissions.len(),
            admin_token.permissions.len(),
        ...
        tampered_token
            .permissions
            .push("unauthorized:access".to_string());

        let tampered_json = serde_json::to_string(&tampered_token);
        let deserialized_tampered: Result<AuthToken, serde_json::Error> =
        ...
        if deserialized_tampered.is_ok() {
            ...
            assert!(
                tampered.permissions.len() > token.permissions.len(),
    ...
    async fn test_session_timeout_cleanup() {
        ...
        // Perform cleanup
        sessions.retain(|_, (_, last_activity)| *last_activity > now - session_timeout);

        // Verify cleanup results
        assert_eq!(sessions.len(), 3, "Should have 3 sessions after cleanup");
        assert!(
            !sessions.contains_key(&expired_sessions[0]),
            "Expired session should be removed"
        );
        ...
        assert!(
            updated_activity
                > creation_times[active_sessions
                    .iter()
        ...
        assert!(
            !sessions.contains_key(active_session_id),
        ...
        batch_sessions.retain(|_, (_, last_activity)| *last_activity > now - session_timeout);

        let final_count = batch_sessions.len();
        assert!(
            final_count < initial_count,
            "Batch cleanup should remove expired sessions"
        );
    ...
    async fn test_concurrent_authentication_operations() {
        ...
        assert!(
            ...
            "All sessions should be removed after logout"
        );
        ...
        for (i, user_id) in user_handles.iter().enumerate() {
            ...
            assert_eq!(
                user.full_name,
                format!("Updated User {}", i),
                "User {} should be updated",
                i
            );
            assert_eq!(
                user.last_login,
                Some(Utc::now().timestamp() as u64 + i as u64),
                "User {} should have updated last login",
                i
            );
        }
    }
    ...
    async fn test_authentication_error_handling_logging() {
        ...
        for (error_type, recovery_message) in recovery_scenarios {
            ...
            assert!(
                can_recover || error_type == "system_error",
        ...
        assert!(
            auth_logs.len() >= error_scenarios.len() + audit_events.len(),
        ...
        assert_eq!(
            rotated_logs.len(),
            auth_logs.len() / 2,
            "Logs should be split for rotation"
        );
    }
}
