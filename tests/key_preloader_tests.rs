...
use fortress_core::key_preloader::{KeyPreloadConfig, KeyPreloader, PreloadStrategy};
use std::sync::Arc;
...
mod tests {
    ...
    async fn test_preloading_strategy_validation() {
        ...
        // Test All strategy
        preloader
            .apply_preload_strategy(&PreloadStrategy::All)
            .await
        ...
        preloader
            .apply_preload_strategy(&PreloadStrategy::FrequentlyUsed)
            .await
            .expect("FrequentlyUsed strategy preload should succeed");
        let frequent_keys = preloader.get_preloaded_keys().await;
        assert!(
            frequent_keys.len() <= 10,
            "FrequentlyUsed strategy should respect memory limit"
        );

        // Test ByPurpose strategy
        preloader
            .apply_preload_strategy(&PreloadStrategy::ByPurpose)
            .await
        ...
        for (_key_id, _, metadata) in &purpose_keys {
            assert!(
                config.priority_purposes.contains(&metadata.purpose),
                "Purpose {} should be in priority list",
                metadata.purpose
            );
        }
        ...
        preloader
            .apply_preload_strategy(&PreloadStrategy::ExpiringSoon)
            .await
        ...
        for (_, _, metadata) in &expiring_keys {
            ...
            assert!(
                time_to_expiry <= ChronoDuration::hours(24),
        ...
        preloader
            .apply_preload_strategy(&PreloadStrategy::ByPerformanceProfile)
            .await
        ...
        preloader
            .apply_preload_strategy(&PreloadStrategy::Custom("test".to_string()))
            .await
        ...
        assert!(
            custom_keys.len() <= key_ids.len(),
            "Custom strategy should not exceed total keys"
        );
    }
    ...
    async fn test_performance_impact_measurement() {
        ...
        println!(
            "Cold retrieval of {} keys took {:?}",
            cold_retrievals, cold_duration
        );
        ...
        println!(
            "Hot retrieval of {} keys took {:?}",
            hot_retrievals, hot_duration
        );

        // Performance improvement analysis
        assert_eq!(
            cold_retrievals, hot_retrievals,
            "Cold and hot retrievals should have same count"
        );
        ...
        assert!(
            stats.total_preload_time > ChronoDuration::zero(),
            "Should have recorded preload time"
        );
        assert!(
            stats.total_memory_usage_bytes > 0,
            "Should use memory for preloaded keys"
        );

        println!(
            "Preloader stats: {} keys, {:?} time, {} bytes",
    ...
    async fn test_memory_efficiency_testing() {
        ...
        // Test memory cleanup
        preloader
            .clear_preloaded_keys()
            .await
            .expect("Preload cleanup should succeed");

        let stats_after_cleanup = preloader
            .get_preload_stats()
            .await
            .expect("Stats after cleanup should be available");

        assert_eq!(
            stats_after_cleanup.total_preloaded_keys, 0,
            "All keys should be cleared"
        );
        assert_eq!(
            stats_after_cleanup.total_memory_usage_bytes, 0,
        ...
        let selective_stats = selective_preloader
            .get_preload_stats()
            .await
            .expect("Selective stats should be available");

        assert!(
            selective_keys.len() <= 5,
            "Selective preload should respect key limit"
        );
        assert!(
            selective_stats.total_memory_usage_bytes <= 50 * 1024,
            "Selective preload should respect memory limit"
        );
        ...
        for (_, _, metadata) in &selective_keys {
            assert_eq!(
                ...
                "Only encryption keys should be loaded"
            );
        }
    }
    ...
    async fn test_background_task_management() {
        ...
        let is_running = preloader
            .is_background_preloading()
            .await
        ...
        let bg_stats = preloader
            .get_background_preload_stats()
            .await
            .expect("Background stats should be available");

        assert!(
            bg_stats.total_preload_attempts > 0,
            "Should have performed background preloads"
        );
        assert!(
            bg_stats.last_preload_time > Some(Utc::now() - ChronoDuration::minutes(1)),
            "Should have recent preload activity"
        );

        // Stop background preloading
        preloader
            .stop_background_preloading()
            .await
            .expect("Background preloading should stop");

        // Verify it stopped
        let is_stopped = preloader
            .is_background_preloading()
            .await
        ...
        let bg_stats_multiple = bg_preloader
            .get_background_preload_stats()
            .await
            .expect("Multiple background stats should be available");

        assert!(
            bg_stats_multiple.total_preload_attempts > 1,
    ...
    async fn test_error_handling_retry_logic() {
        ...
        let result = preloader
            .apply_preload_strategy(&PreloadStrategy::Custom("invalid".to_string()))
            .await;
        match result {
            Ok(_) => {
    ...
    async fn test_preloader_configuration_variations() {
        ...
        assert!(
            minimal_keys.len() <= 5,
            "Minimal config should respect key limit"
        );
        ...
        assert_eq!(
            maximal_keys.len(),
            key_ids.len(),
            "Maximal config should load all keys"
        );
        ...
        let disabled_result = disabled_preloader
            .apply_preload_strategy(&PreloadStrategy::All)
            .await;
        ...
        assert!(
            ...
            "Custom priorities should return empty result"
        );
        ...
        assert!(
            custom_profile_keys.is_empty(),
            "Custom profile should return empty result"
        );
    }
    ...
    async fn test_preloader_lifecycle_management() {
        ...
        // Test initial state
        let initial_stats = preloader
            .get_preload_stats()
            .await
            .expect("Initial stats should be available");
        assert_eq!(
            initial_stats.total_preloaded_keys, 0,
            "Initial stats should show 0 keys"
        );
        assert_eq!(
            initial_stats.total_memory_usage_bytes, 0,
        ...
        let after_preload_stats = preloader
            .get_preload_stats()
            .await
            .expect("Stats after preload should be available");
        assert_eq!(
            after_preload_stats.total_preloaded_keys,
            preloaded_keys.len(),
            "Stats should reflect preloaded keys"
        );
        assert!(
            after_preload_stats.total_memory_usage_bytes > 0,
        ...
        assert!(
            bg_stats.total_preloaded_keys > 0,
            "Should have preloaded keys"
        );
        ...
        assert_eq!(
            final_stats.total_preloaded_keys, 0,
            "Final stats should show 0 keys"
        );
        assert_eq!(
            final_stats.total_memory_usage_bytes, 0,
            "Final memory usage should be 0"
        );
        ...
        assert_eq!(
            shutdown_stats.total_preloaded_keys,
            restart_keys.len(),
    ...
    async fn test_preloader_database_integration() {
        ...
        for (key_id, key, metadata) in &preloaded_keys {
            ...
            if let Some((db_key, db_metadata)) = db_result {
                assert_eq!(
                    db_key.to_vec(),
                    key.to_vec(),
                    "Preloaded key should match database key"
                );
                assert_eq!(
                    db_metadata.algorithm, metadata.algorithm,
                    "Preloaded metadata should match database"
                );
            } else {
        ...
        preloader
            .database()
            .store_key(&new_key_id, &new_key, &new_metadata)
            .await
            .expect("New key storage should succeed");
        preloader
            .force_preload_key(&new_key_id)
            .await
        ...
        assert!(
            !has_deleted_key,
            "Sync preload should not include deleted key"
        );
        ...
        assert!(
            final_stats.total_preloaded_keys <= key_ids.len(),
