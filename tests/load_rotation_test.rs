...
async fn test_high_concurrency_load() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(results.error_rate < (100.0 - config.expected_success_rate), 
           "Error rate too high: {:.2}%", results.error_rate);
    assert!(results.avg_rotation_time_ms < config.max_acceptable_latency_ms as f64,
...
async fn test_sustained_load() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(results.error_rate < 1.0, "Error rate too high for sustained load: {:.2}%", results.error_rate);
    assert!(results.memory_stats.memory_growth_rate < 10.0, 
...
async fn test_bulk_operations_load() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for batch_size in &batch_sizes {
        ...
        let rotation_ids = rotation_manager.bulk_rotate_keys(&key_ids, algorithm.as_ref(), security_context.clone()).await?;
        let rotation_time = rotation_start.elapsed();
        
        // Verify all rotations succeeded
        assert_eq!(rotation_ids.len(), key_ids.len(), "Not all keys in batch {} were rotated", batch_size);
        
        // Verify all keys were actually rotated
        for key_id in &key_ids {
            let (_, metadata) = rotation_manager.key_manager.retrieve_key(key_id).await?;
            assert_eq!(metadata.version, 2, "Key {} was not rotated properly", key_id);
        }
        
        let total_time = start_time.elapsed();
        let throughput = batch_size as f64 / total_time.as_secs_f64();
        
        println!("  Batch size {}: {:.2}ms total, {:.2} rps", 
                batch_size, total_time.as_millis(), throughput);
        
        // Validate performance expectations
        assert!(throughput > 10.0, "Throughput too low for batch size {}: {:.2} rps", batch_size, throughput);
        assert!(rotation_time.as_millis() < 10000, "Rotation time too long for batch size {}: {}ms", 
...
async fn test_resource_exhaustion() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..num_rotations {
        ...
        match rotation_manager.rotate_key_optimized(&key_id, algorithm.as_ref(), security_context.clone()).await {
            Ok(_) => {
...
async fn test_mixed_workload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_ids: Vec<String> = (0..100)
        .map(|i| format!("mixed_key_{}", i))
        .collect();
    ...
    assert!(error_rate < 5.0, "Error rate too high for mixed workload: {:.2}%", error_rate);
    assert!(throughput > 100.0, "Throughput too low for mixed workload: {:.2} ops/sec", throughput);
    assert!(total_rotations > 0, "No rotations completed in mixed workload");
    
    Ok(())
}

/// Helper function to run load tests
async fn run_load_test(config: LoadTestConfig) -> Result<LoadTestResults, Box<dyn std::error::Error>> {
    let key_manager = Arc::new(InMemoryKeyManager::new());
    ...
    let min_rotation_time_ms = rotation_times.iter()
        .map(|d| d.as_millis() as u64)
        .min()
        .unwrap_or(0);
    
    let max_rotation_time_ms = rotation_times.iter()
        .map(|d| d.as_millis() as u64)
        .max()
        .unwrap_or(0);
    
    // Calculate percentiles
    let mut sorted_times: Vec<u64> = rotation_times.iter()
        .map(|d| d.as_millis() as u64)
        .collect();
