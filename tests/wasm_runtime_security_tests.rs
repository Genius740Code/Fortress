...
async fn test_wasm_sandbox_isolation() {
    ...
    assert!(init_result.is_ok(), "Plugin initialization should work in sandbox");
...
async fn test_memory_limits_enforcement() {
    ...
    if result.is_ok() {
        ...
        assert!(execution_time < Duration::from_millis(2000), "Should complete quickly with memory limits");
    } else {
...
async fn test_execution_time_limits() {
    ...
    assert!(execution_time < Duration::from_millis(1000), "Should be terminated within time limit");
...
async fn test_host_function_access_control() {
    ...
    let plugin2 = loader2.load_from_bytes(&create_valid_wasm_bytes(), metadata2).unwrap();
    ...
    assert!(result.is_err(), "Should fail due to disallowed host functions");
}
...
async fn test_plugin_isolation_between_instances() {
    ...
    let plugin1 = loader.load_from_bytes(&create_valid_wasm_bytes(), metadata1).unwrap();
    let plugin2 = loader.load_from_bytes(&create_valid_wasm_bytes(), metadata2).unwrap();
    let plugin3 = loader.load_from_bytes(&create_valid_wasm_bytes(), metadata3).unwrap();
    
    // Initialize all plugins with different contexts
    let context1 = PluginContext {
        config: HashMap::from([("instance_id".to_string(), serde_json::Value::String("1".to_string()))]),
    ...
    let context2 = PluginContext {
        config: HashMap::from([("instance_id".to_string(), serde_json::Value::String("2".to_string()))]),
        metadata: plugin2.metadata().clone(),
        encryption_access: false,
        storage_access: false,
    };
    ...
    let context3 = PluginContext {
        config: HashMap::from([("instance_id".to_string(), serde_json::Value::String("3".to_string()))]),
...
async fn test_resource_cleanup_on_error() {
    ...
    assert!(cleanup_result.is_ok(), "Cleanup should succeed even after execution error");
}
...
async fn test_concurrent_plugin_execution() {
    ...
    for i in 1..=10 {
        let metadata = create_test_metadata(&format!("concurrent-{}", i), &format!("Concurrent Plugin {}", i));
        let plugin = loader.load_from_bytes(&create_valid_wasm_bytes(), metadata).unwrap();
    ...
    let init_tasks: Vec<_> = plugins.iter_mut().zip(contexts).map(|(plugin, context)| {
    ...
    let exec_tasks: Vec<_> = plugins.iter_mut().enumerate().map(|(i, plugin)| {
        let input = PluginInput {
    ...
    for (i, result) in exec_results.into_iter().enumerate() {
        assert!(result.is_ok() || result.is_err(), "Plugin {} should complete without crashing", i);
    }
    
    // Cleanup all plugins
    let cleanup_tasks: Vec<_> = plugins.iter_mut().map(|plugin| {
        plugin.cleanup()
    }).collect();
...
async fn test_plugin_configuration_validation() {
    ...
    let valid_config = HashMap::from([
        ("fortress_log".to_string(), serde_json::Value::String("info".to_string())),
        ("fortress_config_get".to_string(), serde_json::Value::String("test".to_string())),
    ...
    let invalid_config = HashMap::from([
        ("fortress_file_access".to_string(), serde_json::Value::String("enabled".to_string())),
        ("fortress_network_call".to_string(), serde_json::Value::String("allowed".to_string())),
    ]);
    ...
    let dangerous_config = HashMap::from([
        ("max_memory".to_string(), serde_json::Value::String("unlimited".to_string())),
        ("execution_timeout".to_string(), serde_json::Value::String("disabled".to_string())),
    ]);
    
    let result = plugin.validate_config(&dangerous_config);
    assert!(result.is_err(), "Dangerous configuration should fail validation");
}
...
async fn test_plugin_health_monitoring() {
    ...
    assert!(health.healthy, "Plugin should be healthy after initialization");
    ...
    for i in 1..=5 {
        ...
        let health = plugin.health_check().await.unwrap();
        assert!(health.healthy, "Plugin should remain healthy during operation");
    }
...
async fn test_attack_vector_prevention() {
    ...
    assert!(execution_time < Duration::from_secs(2), "Infinite loop should be terminated");
    assert!(result.is_err(), "Infinite loop should fail due to time limits");
    ...
    assert!(result.is_err(), "Memory exhaustion should fail due to limits");
...
async fn test_plugin_lifecycle_security() {
    ...
    let health = plugin.health_check().await.unwrap();
    assert!(health.healthy, "Plugin should be in a healthy state after lifecycle completion");
}
...
async fn test_performance_under_security_constraints() {
    ...
    assert!(total_time < Duration::from_secs(30), "Should complete within reasonable time");
...
async fn test_error_handling_and_recovery() {
    ...
    assert!(result.is_err(), "Malformed parameters should fail gracefully");
