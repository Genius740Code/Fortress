...
use fortress_core::{
    ...
    mpc::{MpcManager, ComputationConfig, ComputationStatus, SecretSharingScheme, PartyRole, MpcParty},
...
mod mpc_tests {
    use super::*;
    use fortress_core::mpc::{PartyRole, MpcMessage};
    ...
    async fn test_mpc_party_message_handling() {
        ...
        let message = MpcMessage {
            ...
            recipient: Some(party.party_id().clone()),
    ...
    async fn test_mpc_error_handling() {
        ...
        let nonexistent_session = "nonexistent-session".to_string();
        let result = manager.get_session_status(&nonexistent_session).await;
        assert!(result.is_err());
        
        // Try to add party to non-existent session
        let party = InMemoryMpcParty::new("party1".to_string(), PartyRole::Participant);
        let result = manager.join_session(&nonexistent_session, Box::new(party)).await;
        assert!(result.is_err());
    }
...
mod integration_tests {
    ...
    async fn test_system_performance_and_scalability() {
        ...
        for plugin in &plugins {
            total_executions += plugin.get_execution_count().await;
        }
