...
use fortress_core::query::{
    ExecutionPlan, InMemoryQueryEngine, PlanNode, PlanNodeType, QueryEngine,
};
...
impl QueryOptimizerTests {
    ...
    async fn test_projection_pruning(&self) -> Result<Vec<OptimizationTestResult>> {
        ...
        let start = Instant::now();
        let optimized_complex = self
            .optimizer
            .optimize_plan(complex_projection_plan)
            .await?;
        let optimization_time = start.elapsed();
    ...
    async fn test_plan_caching(&self) -> Result<Vec<CachingTestResult>> {
        ...
        for i in 0..1100 {
            // Exceed default cache limit of 1000
    ...
    async fn test_statistics_based_optimization(&self) -> Result<Vec<OptimizationTestResult>> {
        ...
        self.optimizer
            .update_table_statistics("users".to_string(), table_stats)
            .await?;
        ...
        table_stats
            .index_stats
            .insert("email_index".to_string(), index_stats);
        self.optimizer
            .update_table_statistics("users".to_string(), table_stats)
            .await?;
        ...
        println!(
            "  Statistics-based optimization: {} tests passed",
            results.len()
        );
        for result in &results {
            println!(
    ...
    async fn test_performance_under_load(&self) -> Result<LoadTestResult> {
        ...
        let avg_optimization_time = Duration::from_secs_f64(
            optimization_times.iter().sum::<Duration>().as_secs_f64() / num_plans as f64,
        );
        ...
        println!(
            "    Min/Max optimization time: {:?}/{:?}",
            min_optimization_time, max_optimization_time
        );
    ...
    async fn test_complex_query_optimization(&self) -> Result<Vec<OptimizationTestResult>> {
        ...
        println!(
            "  Complex query optimization: {} tests passed",
            results.len()
        );
        for result in &results {
            println!(
    ...
    async fn create_test_plan(&self) -> Result<ExecutionPlan> {
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                children: Vec::new(),
                parameters: HashMap::from([("table".to_string(), json!("users"))]),
            ...
            nodes: vec![
                ...
                PlanNode {
                    ...
                    children: vec![PlanNode {
                        node_type: PlanNodeType::TableScan,
                        children: Vec::new(),
                        parameters: HashMap::from([("table".to_string(), json!("orders"))]),
                        estimated_cost: 300.0,
                    }],
                    parameters: HashMap::from([(
                        "predicate".to_string(),
                        json!("orders.user_id = users.id"),
                    )]),
                ...
                PlanNode {
                    ...
                    children: Vec::new(),
                    parameters: HashMap::from([("join_type".to_string(), json!("inner"))]),
    ...
    async fn create_plan_with_filter(&self) -> Result<ExecutionPlan> {
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                parameters: HashMap::from([("predicate".to_string(), json!("users.age > 18"))]),
                estimated_cost: 100.0,
            }],
    ...
    async fn create_complex_projection_plan(&self) -> Result<ExecutionPlan> {
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                parameters: HashMap::from([(
                    "columns".to_string(),
                    json!(["users.id", "users.name", "orders.total"]),
                )]),
        ...
        Ok(ExecutionPlan {
            nodes: vec![
                PlanNode {
                    node_type: PlanNodeType::Join,
                    children: Vec::new(),
                    parameters: HashMap::from([("join_type".to_string(), json!("left"))]),
    ...
    async fn create_variable_complexity_plan(&self, complexity: usize) -> Result<ExecutionPlan> {
        ...
        for i in 0..(complexity % 10 + 1) {
            nodes.push(PlanNode {
                ...
                children: Vec::new(),
                parameters: HashMap::from([("table".to_string(), json!(format!("table_{}", i)))]),
        ...
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                children: vec![PlanNode {
                    ...
                    parameters: HashMap::from([("columns".to_string(), json!(["id", "name"]))]),
                    estimated_cost: 100.0,
                }],
                parameters: HashMap::from([(
                    "predicate".to_string(),
                    json!("id IN (SELECT user_id FROM orders WHERE total > 1000)"),
                )]),
                estimated_cost: 200.0,
            }],
            estimated_cost: 800.0,
        ...
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                children: vec![PlanNode {
                    node_type: PlanNodeType::TableScan,
                    children: Vec::new(),
                    parameters: HashMap::from([("table".to_string(), json!("sales"))]),
                    estimated_cost: 600.0,
                }],
                parameters: HashMap::from([(
                    "columns".to_string(),
    ...
    async fn create_cte_plan(&self) -> Result<ExecutionPlan> {
        Ok(ExecutionPlan {
            nodes: vec![PlanNode {
                ...
                parameters: HashMap::from([("columns".to_string(), json!(["cte.id", "cte.name"]))]),
                estimated_cost: 150.0,
            }],
            estimated_cost: 550.0,
            metadata: HashMap::new(),
    ...
    async fn verify_statistics_usage(&self, plan: &ExecutionPlan) -> Result<bool> {
        ...
        for node in &plan.nodes {
            if node.node_type == PlanNodeType::TableScan {
                ...
                if node.estimated_cost < 1000.0 {
                    // Original cost was 1000.0
                    return Ok(true);
                }
            }
        }
...
impl QueryOptimizationTestResults {
    ...
    pub fn print_summary(&self) {
        ...
        if self.performance_load.success {
            ...
            println!(
                "  Plans per second: {:.2}",
                self.performance_load.plans_per_second
            );
            println!(
                "  Average optimization time: {:?}",
                self.performance_load.avg_optimization_time
            );
        } else {
        ...
        let predicate_pushdown_refs: Vec<&OptimizationTestResult> =
            self.predicate_pushdown.iter().collect();
        let projection_pruning_refs: Vec<&OptimizationTestResult> =
            self.projection_pruning.iter().collect();
        let join_reordering_refs: Vec<&OptimizationTestResult> =
            self.join_reordering.iter().collect();
        let statistics_based_refs: Vec<&OptimizationTestResult> =
            self.statistics_based.iter().collect();
        let complex_queries_refs: Vec<&OptimizationTestResult> =
            self.complex_queries.iter().collect();

        let all_optimizations: Vec<&OptimizationTestResult> = [
            cost_based_refs,
            predicate_pushdown_refs,
            projection_pruning_refs,
            join_reordering_refs,
            statistics_based_refs,
            complex_queries_refs,
        ]
        .concat();

        if !all_optimizations.is_empty() {
            let avg_improvement = all_optimizations
                .iter()
                .map(|r| r.cost_improvement)
                .sum::<f64>()
                / all_optimizations.len() as f64;
...
mod tests {
    ...
    async fn test_cost_based_optimization() {
        ...
        assert!(
            !results.is_empty(),
            "Should generate cost-based optimization results"
        );
    }
    ...
    async fn test_comprehensive_optimization_tests() {
        ...
        assert!(
            !results.cost_based.is_empty(),
            "Should have cost-based results"
        );
        assert!(
            !results.predicate_pushdown.is_empty(),
            "Should have predicate pushdown results"
        );
        assert!(
            !results.projection_pruning.is_empty(),
            "Should have projection pruning results"
        );
        assert!(
            !results.join_reordering.is_empty(),
            "Should have join reordering results"
        );
        assert!(
            !results.plan_caching.is_empty(),
            "Should have plan caching results"
        );
        assert!(
            !results.statistics_based.is_empty(),
            "Should have statistics-based results"
        );
        assert!(
            !results.complex_queries.is_empty(),
            "Should have complex query results"
        );
    }
