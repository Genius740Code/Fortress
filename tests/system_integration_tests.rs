...
mod mpc_tests {
    ...
    async fn test_mpc_manager_initialization() {
        ...
        let manager = DefaultMpcManager::new(network, "test-protocol");
        
        assert_eq!(manager.default_protocol(), "test-protocol");
        ...
        assert!(protocols.contains(&"shamir-secret-sharing".to_string()));
    }
    ...
    async fn test_mpc_computation_lifecycle() {
        ...
        // Add test protocol
        let protocol = TestMpcProtocol::new("test-protocol");
        manager.add_protocol(Box::new(protocol)).await.unwrap();
        ...
        manager.add_party_to_session(&session_id, party1).await.unwrap();
        manager.add_party_to_session(&session_id, party2).await.unwrap();
        manager.add_party_to_session(&session_id, party3).await.unwrap();
        ...
        let result = manager.finalize_computation(&session_id).await.unwrap();
    ...
    async fn test_mpc_error_handling() {
        let network = Arc::new(InMemoryMpcNetwork::new());
        let manager = DefaultMpcManager::new(network, "nonexistent-protocol");
        
        // Try to initialize computation with non-existent protocol
        let config = ComputationConfig {
            session_id: None,
            protocol_id: "nonexistent-protocol".to_string(),
            parties: vec!["party1".to_string()],
            threshold: 1,
        ...
        let result = manager.initialize_computation(&config).await;
        assert!(result.is_err());
...
mod plugin_tests {
    ...
    async fn test_plugin_concurrent_execution() {
        let context = PluginContext {
            config: HashMap::new(),
            metadata: TestPlugin::new("concurrent-test", vec![PluginCapability::Hash]).metadata().clone(),
            encryption_access: false,
            storage_access: false,
        };
...
mod integration_tests {
    ...
    async fn test_cluster_mpc_integration() {
        ...
        for i in 0..3 {
            ...
            let node = ClusterNode {
                ...
                capabilities: NodeCapabilities {
                    ...
                    supports_computation: true,
                    max_connections: 1000,
                    supported_protocols: vec!["mpc".to_string()],
                },
                load_metrics: LoadMetrics {
                    cpu_usage: 0.1,
                    memory_usage: 0.2,
        ...
        // Start and finalize computation
        manager.start_computation(&session_id).await.unwrap();
        let result = manager.finalize_computation(&session_id).await.unwrap();
    ...
    async fn test_mpc_plugin_integration() {
        ...
        let network = Arc::new(InMemoryMpcNetwork::new());
        let manager = DefaultMpcManager::new(network, "plugin-protocol");
        
        // Add test protocol
        let protocol = TestMpcProtocol::new("plugin-protocol");
        manager.add_protocol(Box::new(protocol)).await.unwrap();
        
        // Create cryptographic plugin
        let mut crypto_plugin = TestPlugin::new("crypto-plugin", vec![
            PluginCapability::Encrypt,
            PluginCapability::Decrypt,
            PluginCapability::GenerateKey,
        ]);
        ...
        let config = ComputationConfig::new(
            ...
            SecretSharingScheme::Shamir { threshold: 2, total_shares: 3 },
        ).with_parameter("plugin_id".to_string(), serde_json::Value::String(crypto_plugin.metadata().id.clone()))
         .with_parameter("operation".to_string(), serde_json::Value::String("encrypt".to_string()));
    ...
    async fn test_cluster_plugin_integration() {
        ...
        let mut security_plugin = TestPlugin::new("security-plugin", vec![
            PluginCapability::SignTransaction,
            PluginCapability::VerifySignature,
            PluginCapability::SecretManagement,
        ]);
        ...
        let total_rps: f64 = nodes.iter().map(|n| n.ops_per_second).sum();
    ...
    async fn test_full_system_integration() {
        ...
        let mut integrated_plugin = TestPlugin::new("integrated-plugin", vec![
            PluginCapability::Encrypt,
            PluginCapability::SignTransaction,
            PluginCapability::ApiIntegration,
        ]);
        ...
        let config = ComputationConfig::new(
            ...
            SecretSharingScheme::Shamir { threshold: 2, total_shares: 3 },
        ).with_parameter("plugin_assisted".to_string(), serde_json::Value::Bool(true))
         .with_parameter("cluster_secure".to_string(), serde_json::Value::Bool(true))
