...
impl PerformanceOptimizationSuite {
    ...
    fn generate_comprehensive_report(
        results: &PerformanceOptimizationResults,
        ...
        if !results.benchmarks.encryption.is_empty() {
            ...
            println!(
                "  Combined throughput: {:.2} MB/s",
                avg_encrypt + avg_decrypt
            );
        ...
        if !results.benchmarks.key_management.key_generation.is_empty() {
            ...
            if let Some(key_gen) = best_key_gen {
                println!("\nKey Management Performance:");
                println!(
                    "  Best key generation: {:.2} keys/sec",
                    key_gen.keys_per_sec
                );
        ...
        let passed_query_tests = results
            .query_optimization
            .cost_based
            .iter()
            .filter(|r| r.success)
        ...
        println!(
            "  Success rate: {:.1}%",
            (passed_query_tests as f64 / total_query_tests as f64) * 100.0
        );
        println!(
            "  Performance under load: {:.2} plans/sec",
        ...
        println!(
            "  Security performance success rate: {:.1}%",
            security_success_rate * 100.0
        );
        ...
        if !results.benchmarks.encryption.is_empty() {
            let avg_throughput = results
                ...
                .encryption
                .iter()
                .map(|r| r.encrypt_throughput_mbps + r.decrypt_throughput_mbps)
                .sum::<f64>()
                / results.benchmarks.encryption.len() as f64;
        ...
        max_score += 25.0;
        println!(
            "  Security Performance: {:.1}/25",
            security_success_rate * 25.0
        );
        ...
        let scalability_score = if results.query_optimization.performance_load.success
            && results.security_performance.scalability.success
        {
            0.9
        } else {
            0.7
        ...
        if final_score >= 90.0 {
            ...
            println!(
                "Fortress demonstrates exceptional performance and optimization capabilities."
            );
    ...
    pub fn overall_success(&self) -> bool {
        ...
        let query_success = !self.query_optimization.cost_based.is_empty()
            && self
                ...
                .filter(|r| r.success)
                .count() as f64
                / self.query_optimization.cost_based.len() as f64
                >= 0.7;
...
mod tests {
    ...
    async fn test_performance_optimization_suite() {
        ...
        assert!(
            !results.benchmarks.encryption.is_empty(),
            "Should have benchmark results"
        );
        assert!(
            !results.query_optimization.cost_based.is_empty(),
            "Should have query optimization results"
        );
        assert!(
            results
                .security_performance
                ...
                > 0,
            "Should have security performance results"
        );
    }
    ...
    async fn test_benchmark_suite_only() {
        ...
        assert!(
            ...
            "Should have encryption benchmark results"
        );
        assert!(
            !results.storage.write_results.is_empty(),
            "Should have storage benchmark results"
        );
        assert!(
            !results.key_management.key_generation.is_empty(),
            "Should have key management results"
        );
    }
    ...
    async fn test_query_optimization_only() {
        ...
        assert!(
            ...
            "Should have cost-based optimization results"
        );
        assert!(
    ...
    async fn test_security_performance_only() {
        ...
        assert!(
            results.high_concurrency_auth.total_authentications > 0,
            "Should have high concurrency auth results"
        );
        assert!(
            !results.bulk_encryption.operations.is_empty(),
            "Should have bulk encryption results"
        );
        assert!(
            results.memory_stress.sessions_generated > 0,
            "Should have memory stress results"
        );
    }
}
