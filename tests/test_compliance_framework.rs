...
use fortress_core::audit::{AuditLogger, AuditEntry, AuditQuery, IntegrityReport, AuditStatistics};
use chrono::Utc;
...
async fn test_comprehensive_compliance_framework() {
    ...
    let request_id = compliance_manager.create_data_subject_request(
        "user_123".to_string(),
    ...
    // Process the request
    compliance_manager.process_data_subject_request(
        &request_id,
    ...
    let request = compliance_manager.get_data_subject_request(&request_id).unwrap();
...
async fn test_gdpr_data_subject_rights() {
    ...
    for (i, right) in rights.iter().enumerate() {
        ...
        let processed_request = compliance_manager.get_data_subject_request(&request_id).unwrap();
...
async fn test_compliance_requirements() {
    ...
    requirements.insert("data_encryption".to_string(), ComplianceRequirement {
