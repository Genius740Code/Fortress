...
use fortress_core::{
    key::{KeyManager, InMemoryKeyManager, RotationPolicy, RotationInterval, SmartKeyRotationScheduler},
...
async fn test_zero_downtime_rotation_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rotation_result = key_manager.rotate_key_with_zero_downtime(&key_id, algorithm.as_ref()).await;
    assert!(rotation_result.is_ok(), "Zero-downtime rotation should succeed");
    ...
    assert!(rotated_metadata.created_at > initial_metadata.created_at, "Creation time should be updated");
...
async fn test_rotation_rollback_on_failure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(rotation_result.is_err(), "Rotation should fail when key is missing");
    
    // Restore the key for continued testing
    key_manager.store_key(&key_id, &initial_key, &initial_metadata).await?;
    
    // Verify the key is still accessible with original version
    let (_, restored_metadata) = key_manager.retrieve_key(&key_id).await?;
    assert_eq!(restored_metadata.version, initial_version, "Version should be preserved after failure");
...
async fn test_smart_scheduler_zero_downtime_rotation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (key_id, _) in &keys {
        ...
        assert!(!key.is_empty(), "Key {} should be accessible after rotation", key_id);
        assert!(metadata.version >= 1, "Key {} should have valid version", key_id);
    }
    
    // Check scheduler metrics
    let metrics = scheduler.get_metrics().await;
    assert!(metrics.total_rotations > 0, "Should have rotation metrics");
    assert!(metrics.successful_rotations > 0, "Should have successful rotations");
...
async fn test_rotation_timeout_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(rotation_result.is_ok(), "Rotation should complete within timeout");
...
async fn test_rotation_with_different_algorithms() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (algo_name, algorithm) in algorithms {
        ...
        let rotation_result = key_manager.rotate_key_with_zero_downtime(&key_id, algorithm.as_ref()).await;
        assert!(rotation_result.is_ok(), "Rotation with {} should succeed", algo_name);
        
        // Verify rotation
        let (_, rotated_metadata) = key_manager.retrieve_key(&key_id).await?;
        assert_eq!(rotated_metadata.version, 2, "Version should be incremented for {}", algo_name);
        assert_eq!(rotated_metadata.algorithm, algorithm.name(), "Algorithm should be preserved for {}", algo_name);
    }
...
async fn test_rotation_metadata_integrity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Add custom metadata
    metadata = metadata.with_metadata("custom_field".to_string(), "custom_value".to_string())
    ...
    assert_eq!(rotated_metadata.purpose, metadata.purpose, "Purpose should be preserved");
    assert_eq!(rotated_metadata.performance_profile, metadata.performance_profile, "Performance profile should be preserved");
    
    // Check rotation-specific metadata
    assert!(rotated_metadata.get_metadata("rotation_id").is_some(), "Should have rotation ID");
    assert!(rotated_metadata.get_metadata("transition_status").is_some(), "Should have transition status");
...
async fn test_multiple_rotations_sequence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..num_rotations {
        ...
        assert_eq!(current_metadata.version, initial_version + i + 1, 
                  "Version should be {} after rotation {}", initial_version + i + 1, i + 1);
        
        // Verify key is still accessible
        let (accessed_key, _) = key_manager.retrieve_key(&key_id).await?;
        assert!(!accessed_key.is_empty(), "Key should be accessible after rotation {}", i + 1);
    }
    
    // Verify final state
    let (_, final_metadata) = key_manager.retrieve_key(&key_id).await?;
    assert_eq!(final_metadata.version, initial_version + num_rotations, 
              "Final version should be {}", initial_version + num_rotations);
