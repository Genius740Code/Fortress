...
use fortress_core::error::{EncryptionErrorCode, FortressError};
...
mod tests {
    ...
    async fn test_basic_encrypt_decrypt() {
        ...
        assert_eq!(
            ciphertext.len(),
            plaintext.len() + 12 + 16,
            "Ciphertext should include nonce (12) + tag (16)"
        );
    ...
    async fn test_encrypt_various_sizes() {
        ...
        for size in test_sizes {
            ...
            assert_eq!(
                ciphertext.len(),
                size + 12 + 16,
                "Ciphertext length incorrect for size {}",
                size
            );
    ...
    async fn test_key_generation_and_validation() {
        ...
        let valid_keys = vec![vec![0u8; 32], vec![255u8; 32], vec![1u8; 32], {
        ...
        for (i, key) in invalid_keys.iter().enumerate() {
            ...
            assert!(
                result.is_err(),
                "Should reject invalid key {} of length {}",
                i,
                key.len()
            );

            if let Err(FortressError::Encryption {
    ...
    async fn test_error_handling_invalid_inputs() {
        ...
        for (i, ciphertext) in short_ciphertexts.iter().enumerate() {
            ...
            assert!(
                result.is_err(),
                "Should reject short ciphertext {} of length {}",
                i,
                ciphertext.len()
            );
        }
    ...
    async fn test_performance_benchmarks() {
        ...
        for (name, size) in test_cases {
            ...
            assert_eq!(
                decrypted, plaintext,
                "Benchmark verification failed for {}",
                name
            );
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                ...
                let ciphertext = aes_clone
                    .encrypt(plaintext.as_bytes(), &key_clone)
                    .expect("Concurrent encryption should succeed");

                let decrypted = aes_clone
                    .decrypt(&ciphertext, &key_clone)
        ...
        for handle in handles {
            ...
            let decrypted = aes
                .decrypt(&ciphertext, &key)
                .expect("Re-decryption should succeed");
            assert_eq!(
                decrypted,
                original.as_bytes(),
                "Re-decryption verification failed"
            );
        }
    }
    ...
    async fn test_memory_safety_edge_cases() {
        ...
        let ciphertext = aes
            .encrypt(plaintext, &key)
            .expect("Empty encryption should succeed");
        assert_eq!(
            ciphertext.len(),
            28,
            "Empty ciphertext should be nonce + tag only"
        );

        let decrypted = aes
            .decrypt(&ciphertext, &key)
            .expect("Empty decryption should succeed");
        assert_eq!(
            decrypted, plaintext,
            "Empty decrypted should match original"
        );
        ...
        assert_eq!(
            ...
            "Large data decryption should match"
        );
    }
    ...
    async fn test_nonce_randomness_uniqueness() {
        ...
        for (i, ciphertext) in ciphertexts.iter().enumerate() {
            ...
            assert_eq!(
                decrypted, plaintext,
                "Ciphertext {} should decrypt to original",
                i
            );
        }
    }
    ...
    async fn test_algorithm_properties() {
        ...
        assert_eq!(
            aes.nonce_size(),
            12,
            "Nonce size should be 12 bytes (96 bits)"
        );
        assert_eq!(aes.tag_size(), 16, "Tag size should be 16 bytes (128 bits)");
        assert_eq!(
            aes.security_level(),
            256,
            "Security level should be 256 bits"
        );
    }
    ...
    async fn test_clone_default_functionality() {
        ...
        let ciphertext = aes_default
            .encrypt(plaintext, &key)
            .expect("Default encryption should succeed");
        let decrypted = aes_default
            .decrypt(&ciphertext, &key)
            .expect("Default decryption should succeed");
        ...
        let ciphertext1 = aes1
            .encrypt(plaintext, &key)
            .expect("Clone encryption 1 should succeed");
        let ciphertext2 = aes2
            .encrypt(plaintext, &key)
            .expect("Clone encryption 2 should succeed");

        // Should produce different ciphertexts due to random nonce
        assert_ne!(
            ciphertext1, ciphertext2,
            "Cloned instances should produce different ciphertexts"
        );

        // But both should decrypt correctly
        let decrypted1 = aes1
            .decrypt(&ciphertext1, &key)
            .expect("Clone decryption 1 should succeed");
        let decrypted2 = aes2
            .decrypt(&ciphertext2, &key)
            .expect("Clone decryption 2 should succeed");
    ...
    async fn test_data_integrity_authentication() {
        ...
        for i in 0..modified_ciphertext.len() {
            ...
            }) = result
            {
                ...
                panic!(
                    "Expected AuthenticationFailed error for tampered data at byte {}",
                    i
                );
            }
        ...
        let decrypted = aes
            .decrypt(&ciphertext, &key)
            .expect("Original should still decrypt");
        ...
        for trunc_len in 1..ciphertext.len() {
            ...
            assert!(
                result.is_err(),
                "Should detect truncation of length {}",
                trunc_len
            );
        }
    }
    ...
    async fn test_key_sensitivity_avalanche() {
        let aes = Aes256GcmWrapper::new();
        let plaintext =
            b"Avalanche test - small key changes should produce completely different ciphertexts";
        ...
        for byte_pos in 0..32 {
            for bit_pos in 0..8 {
                ...
                let ciphertext1 = aes
                    .encrypt(plaintext, &base_key)
                    .expect("Base encryption should succeed");
                let ciphertext2 = aes
                    .encrypt(plaintext, &modified_key)
                    .expect("Modified encryption should succeed");
    ...
    async fn test_stress_many_operations() {
        ...
        for i in 0..num_operations {
            ...
            match aes.encrypt(plaintext.as_bytes(), &key) {
                Ok(ciphertext) => match aes.decrypt(&ciphertext, &key) {
                    Ok(decrypted) => {
