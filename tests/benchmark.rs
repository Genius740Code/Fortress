...
impl BenchmarkSuite {
    ...
    pub async fn run_encryption_benchmarks(&self) -> Result<Vec<BenchmarkResults>> {
        ...
        for (algorithm, name) in algorithms.into_iter().zip(algorithm_names) {
            ...
            for (size, description) in &test_sizes {
                let result = self.benchmark_encryption_algorithm(&*algorithm, *size, description).await?;
    ...
    async fn print_encryption_comparison(&self, results: &[BenchmarkResults]) {
        ...
        for result in results {
            by_size.entry(result.data_size).or_insert_with(Vec::new).push(result);
        }
    ...
    async fn benchmark_storage_writes(&self) -> Result<Vec<StorageOperationResult>> {
        ...
        for size in write_sizes {
            ...
            let throughput_mbps = (size * iterations) as f64 / (1024.0 * 1024.0) / total_time.as_secs_f64();
            ...
            println!("    Write {} bytes: {:.2} ops/sec, {}", 
                size, ops_per_sec, BenchmarkResults::format_throughput(throughput_mbps));
        }
    ...
    async fn benchmark_concurrent_storage(&self) -> Result<ConcurrentBenchmarkResult> {
        ...
        println!("    Min/Max operation time: {:?}/{:?}", min_op_time, max_op_time);
    ...
    async fn benchmark_large_data_storage(&self) -> Result<Vec<StorageOperationResult>> {
        ...
        for size in large_sizes {
            ...
            let combined_throughput = size as f64 * 2.0 / (1024.0 * 1024.0) / total_time.as_secs_f64();
            ...
            println!("    Large data {} bytes: Write {}, Read {}, Combined {}", 
                size,
    ...
    async fn benchmark_key_generation(&self) -> Result<Vec<KeyOperationResult>> {
        ...
        for count in key_counts {
            ...
            println!("    Generate {} keys: {:.2} keys/sec, avg {:?}", 
    ...
    async fn benchmark_key_rotation(&self) -> Result<Vec<KeyOperationResult>> {
        ...
        for count in rotation_counts {
            ...
            println!("    Rotate {} keys: {:.2} rotations/sec, avg {:?}", 
    ...
    async fn benchmark_key_retrieval(&self) -> Result<Vec<KeyOperationResult>> {
        ...
        for count in retrieval_counts {
            ...
            println!("    Retrieve {} keys: {:.2} retrievals/sec, avg {:?}", 
...
impl StorageBenchmarkResults {
    ...
    pub fn print_summary(&self) {
        ...
        if let (Some(best_write), Some(best_read)) = (
            self.write_results.iter().max_by(|a, b| a.ops_per_sec.partial_cmp(&b.ops_per_sec).unwrap()),
            self.read_results.iter().max_by(|a, b| a.ops_per_sec.partial_cmp(&b.ops_per_sec).unwrap())
        ) {
            println!("Best write performance: {:.2} ops/sec ({} bytes)", 
                best_write.ops_per_sec, best_write.data_size);
            println!("Best read performance: {:.2} ops/sec ({} bytes)", 
        ...
        if let Some(best_large) = self.large_data_results.iter()
            .max_by(|a, b| a.throughput_mbps.partial_cmp(&b.throughput_mbps).unwrap()) {
            println!("Best large data throughput: {}", 
...
impl KeyManagementBenchmarkResults {
    ...
    pub fn print_summary(&self) {
        ...
        if let (Some(best_gen), Some(best_rot), Some(best_ret)) = (
            self.key_generation.iter().max_by(|a, b| a.keys_per_sec.partial_cmp(&b.keys_per_sec).unwrap()),
            self.key_rotation.iter().max_by(|a, b| a.keys_per_sec.partial_cmp(&b.keys_per_sec).unwrap()),
            self.key_retrieval.iter().max_by(|a, b| a.keys_per_sec.partial_cmp(&b.keys_per_sec).unwrap())
        ) {
            println!("Best key generation: {:.2} keys/sec", best_gen.keys_per_sec);
            println!("Best key rotation: {:.2} rotations/sec", best_rot.keys_per_sec);
            println!("Best key retrieval: {:.2} retrievals/sec", best_ret.keys_per_sec);
        }
    }
}
...
impl ComprehensiveBenchmarkResults {
    ...
    pub fn generate_report(&self) {
        ...
        if !self.encryption.is_empty() {
            let avg_encrypt: f64 = self.encryption.iter().map(|r| r.encrypt_throughput_mbps).sum::<f64>() / self.encryption.len() as f64;
            let avg_decrypt: f64 = self.encryption.iter().map(|r| r.decrypt_throughput_mbps).sum::<f64>() / self.encryption.len() as f64;
            
            println!("Encryption Performance:");
            println!("  Average encryption throughput: {}", 
                BenchmarkResults::format_throughput(avg_encrypt));
            println!("  Average decryption throughput: {}", 
                BenchmarkResults::format_throughput(avg_decrypt));
            println!("  Combined average throughput: {}", 
...
mod tests {
    ...
    async fn test_encryption_benchmarks() {
        ...
        assert!(!results.is_empty(), "Should generate encryption benchmark results");
    }
    ...
    async fn test_storage_benchmarks() {
        ...
        assert!(!results.write_results.is_empty(), "Should generate storage write results");
        assert!(!results.read_results.is_empty(), "Should generate storage read results");
    }
    ...
    async fn test_key_management_benchmarks() {
        ...
        assert!(!results.key_generation.is_empty(), "Should generate key generation results");
    }
    ...
    async fn test_comprehensive_benchmarks() {
        ...
        assert!(!results.encryption.is_empty(), "Should generate encryption results");
        assert!(!results.storage.write_results.is_empty(), "Should generate storage results");
        assert!(!results.key_management.key_generation.is_empty(), "Should generate key management results");
    }
