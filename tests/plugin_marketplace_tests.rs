...
async fn test_plugin_uninstallation_process() {
    ...
    fs::write(plugin_dir.join("metadata.json"), "{\"id\":\"test-plugin\"}")
        .await
        .unwrap();
    ...
    assert!(
        !plugin_dir.exists(),
        "Plugin directory should be removed after uninstallation"
    );
...
async fn test_installed_plugins_listing() {
    ...
    for i in 1..=3 {
        ...
        let package =
            create_mock_package(&format!("plugin-{}", i), &format!("Plugin {}", i), "1.0.0");
        let metadata_json = serde_json::to_string(&package).unwrap();
        fs::write(plugin_dir.join("metadata.json"), metadata_json)
            .await
            .unwrap();
    }
    ...
    assert_eq!(
        ...
        3,
        "Should find 3 installed plugins"
    );
...
async fn test_plugin_dependency_validation() {
    ...
    dependent_package.dependencies =
        vec!["base-auth-plugin".to_string(), "crypto-utils".to_string()];

    assert!(!dependent_package.dependencies.is_empty());
    assert_eq!(dependent_package.dependencies.len(), 2);
    assert!(dependent_package
        .dependencies
        .contains(&"base-auth-plugin".to_string()));
    assert!(dependent_package
        .dependencies
        .contains(&"crypto-utils".to_string()));
}
...
async fn test_plugin_configuration_schema() {
    ...
    assert!(
        package.config_schema.is_some(),
        "Plugin should have config schema"
    );
...
async fn test_plugin_download_tracking() {
    ...
    // Test download count
    assert_eq!(
        package.download_count, 100,
        "Download count should match fixture"
    );
...
async fn test_plugin_size_validation() {
    ...
    assert!(
        package.size_bytes <= max_size,
        "Plugin should not exceed size limit"
    );
...
async fn test_plugin_marketplace_error_handling() {
    let temp_dir = create_test_temp_dir().await.unwrap();
    let marketplace =
        PluginMarketplace::new(temp_dir.clone(), Some("invalid-url".to_string())).unwrap();
    ...
    assert!(
        uninstall_result.is_err(),
        "Should fail for non-installed plugin"
    );

    let update_result = marketplace.update("non-installed-plugin").await;
    assert!(
        update_result.is_err(),
        "Should fail for non-installed plugin update"
    );
...
async fn test_plugin_marketplace_performance() {
    ...
    assert!(
        ...
        "Plugin creation should be fast"
    );
    assert!(
        list_time < Duration::from_millis(1000),
        "Plugin listing should be fast (< 1s)"
    );
