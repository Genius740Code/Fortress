...
mod tests {
    ...
    async fn test_basic_cache_functionality() {
        ...
        cache
            .put(key_id.clone(), key.clone(), metadata.clone())
            .await
        ...
        assert_eq!(
            cached_key.to_vec(),
            key.to_vec(),
            "Cached key should match original"
        );
        assert_eq!(
            cached_metadata.algorithm, metadata.algorithm,
    ...
    async fn test_lru_eviction_strategy() {
        ...
        cache
            .put(new_key_id.clone(), new_key.clone(), new_metadata.clone())
            .await
    ...
    async fn test_cache_hit_miss_ratio() {
        ...
        assert!(
            (hit_ratio - expected_ratio).abs() < 0.001,
            "Hit ratio should be {:.3}, got {:.3}",
            expected_ratio,
            hit_ratio
        );
    }
    ...
    async fn test_concurrent_cache_access() {
        ...
        for _reader_id in 0..num_concurrent_readers {
        ...
        assert_eq!(
            total_successful_reads, expected_reads,
        ...
        for _writer_id in 0..num_concurrent_writers {
            let cache_clone = Arc::clone(&cache);
        ...
        assert_eq!(
            total_successful_writes, expected_writes,
        ...
        for _op_id in 0..num_mixed_operations {
            let cache_clone = Arc::clone(&cache);
            ...
            let handle = tokio::spawn(async move {
                ...
                for i in 0..5 {
                    ...
                    let _ = cache_clone
                        .put(new_key_id.clone(), new_key, new_metadata)
                        .await;
    ...
    async fn test_cache_invalidation_strategies() {
        ...
        assert!(
            !contains_after_remove,
            "Invalidated key should not be cached"
        );
        ...
        for key_id in keys_to_invalidate {
            ...
            assert!(
                !contains_after_bulk,
                "Bulk invalidated key should not be cached"
            );
        }
        ...
        assert_eq!(
            stats_after_clear.total_keys, 0,
    ...
    async fn test_cache_performance_many_operations() {
        ...
        println!(
            "Read {} cached keys in {:?}",
            successful_reads, read_duration
        );
        ...
        println!(
            "Checked contains for {} keys in {:?}",
            successful_contains, contains_duration
        );

        // Verify all operations succeeded
        assert_eq!(successful_reads, num_operations, "All reads should succeed");
        assert_eq!(
            successful_contains, num_operations,
            "All contains checks should succeed"
        );
        ...
        assert!(
            contains_duration.as_millis() < 500,
            "Contains operations should be very fast"
        );
    ...
    async fn test_cache_statistics_monitoring() {
        ...
        assert_eq!(
            initial_stats.cache_hits, 0,
            "Initial stats should show 0 hits"
        );
        assert_eq!(
            initial_stats.cache_misses, 0,
            "Initial stats should show 0 misses"
        );
        assert_eq!(
            initial_stats.total_requests, 0,
            "Initial stats should show 0 requests"
        );
        assert_eq!(
            initial_stats.memory_usage_bytes, 0,
            "Initial stats should show 0 memory usage"
        );
        ...
        assert!(
            stats_after_ops.total_requests > 0,
    ...
    async fn test_cache_configuration_variations() {
        ...
        time_cache
            .put(time_key_id.clone(), time_key, time_metadata)
            .await
    ...
    async fn test_cache_edge_cases_error_handling() {
        ...
        cache
            .put(duplicate_key_id.clone(), key1.clone(), metadata1.clone())
            .await
        ...
        cache
            .put(duplicate_key_id.clone(), key2.clone(), metadata2.clone())
            .await
            .expect("Second duplicate key storage should succeed");

        let duplicate_result = cache.get(&duplicate_key_id).await;
        assert!(
            duplicate_result.is_some(),
            "Duplicate key should be retrievable"
        );
