...
mod mpc_tests {
    ...
    async fn test_mpc_computation_lifecycle() {
        ...
        manager.join_session(&session_id, party1).await.unwrap();
        manager.join_session(&session_id, party2).await.unwrap();
        manager.join_session(&session_id, party3).await.unwrap();
    ...
    async fn test_mpc_error_handling() {
        ...
        let result = manager.join_session("nonexistent-session", party).await;
        assert!(result.is_err());
    }
...
mod integration_tests {
    ...
    async fn test_cluster_mpc_integration() {
        ...
        for node in &nodes {
            ...
            manager.join_session(&session_id, party).await.unwrap();
        }
    ...
    async fn test_full_system_integration() {
        ...
        assert!(mpc_result.unwrap().success);
        
        // Verify system integration
        assert_eq!(nodes.len(), 3);
        assert!(integrated_plugin.get_execution_count().await >= 1);
        assert!(mpc_result.unwrap().result_data.is_some());
    ...
    async fn test_system_security_validation() {
        ...
        assert!(secure_mpc_result.unwrap().success);
