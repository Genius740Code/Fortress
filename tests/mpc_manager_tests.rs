...
use fortress_core::mpc_manager::{MpcConfig, MpcManager, MpcProtocol, MpcSession};
...
mod tests {
    ...
    async fn test_mpc_manager_initialization() {
        ...
        assert!(
            ...
            "MPC manager should initialize successfully"
        );
    ...
    async fn test_mpc_session_creation() {
        ...
        assert_eq!(
            session.protocol_name, "test_protocol",
            "Protocol name should match"
        );
        assert_eq!(session.required_parties, 3, "Required parties should match");
        assert_eq!(
            session.current_parties, 0,
            "Current parties should be 0 initially"
        );
    }
    ...
    async fn test_mpc_party_addition() {
        ...
        let party_id = manager
            .add_party(&session_id, party_metadata)
            .await
            .unwrap();
        ...
        assert!(
            session.parties.contains_key(&party_id),
            "Party should be in session"
        );
    }
    ...
    async fn test_mpc_message_handling() {
        ...
        // Receive message
        let received_message = manager
            .receive_message(&session_id, "party_1")
            .await
            .unwrap();
        assert_eq!(
            received_message.get("type"),
            Some(&"share_data".to_string())
        );
        assert_eq!(
            received_message.get("data"),
            Some(&"test_share_data".to_string())
        );
    }
    ...
    async fn test_mpc_computation_execution() {
        ...
        let session_id = manager
            .create_session("addition_protocol", 2)
            .await
            .unwrap();
        ...
        let computation_result = manager
            .execute_computation(&session_id, &computation_input)
            .await
            .unwrap();
        assert!(
    ...
    async fn test_mpc_session_termination() {
        ...
        assert!(
            ...
            "Session should terminate successfully"
        );
    ...
    async fn test_mpc_concurrent_sessions() {
        ...
        for i in 0..3 {
            ...
            let handle = tokio::spawn(async move {
                let session_id = manager_clone
                    .create_session(&format!("protocol_{}", i), 2)
                    .await
                    .unwrap();
        ...
        for (session_id, party_id) in results {
            ...
            let session = manager.get_session(&session_id).await.unwrap();
            assert_eq!(
                session.current_parties, 1,
                "Each session should have 1 party"
            );
        }
    }
    ...
    async fn test_mpc_error_handling() {
        ...
        let add_party_result = manager
            .add_party(non_existent_session, party_metadata)
            .await;
        assert!(
            add_party_result.is_err(),
            "Adding party to non-existent session should fail"
        );
        ...
        assert!(
            start_result.is_err(),
            "Starting non-existent session should fail"
        );
    }
    ...
    async fn test_mpc_performance_metrics() {
        ...
        for i in 0..5 {
            let session_id = manager
                .create_session(&format!("perf_protocol_{}", i), 2)
                .await
                .unwrap();
        ...
        assert!(
            ...
            "5 sessions should be created within 5 seconds"
        );
        ...
        assert!(
            ...
            "Should have at least 5 active sessions"
        );
        assert!(
            metrics.total_sessions_created >= 5,
            "Should have created at least 5 sessions"
        );
        assert!(
            metrics.average_session_creation_time_ms > 0.0,
            "Should have average creation time"
        );
    }
    ...
    async fn test_mpc_security_validation() {
        ...
        assert!(
            add_result.is_err(),
            "Party with invalid public key should be rejected"
        );
        ...
        assert!(
            add_result.is_ok(),
            "Party with valid public key should be accepted"
        );
    }
    ...
    async fn test_mpc_message_integrity() {
        ...
        assert!(
            send_result.is_ok(),
            "Secure message should be sent successfully"
        );
        ...
        assert_eq!(
            received_message.get("type"),
            Some(&"secure_data".to_string())
        );
        assert_eq!(
            received_message.get("data"),
            Some(&"sensitive_computation_data".to_string())
        );
        assert_eq!(
            received_message.get("timestamp"),
