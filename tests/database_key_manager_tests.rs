...
mod tests {
    ...
    async fn test_key_lifecycle_management() {
        ...
        assert_eq!(
            metadata_only.unwrap().algorithm,
            metadata.algorithm,
            "Retrieved metadata should match"
        );
        ...
        db.store_key(&key_id, &updated_key, &updated_metadata)
            .await
        ...
        assert!(
            updated_retrieved.is_some(),
            "Updated key should be retrievable"
        );

        let (retrieved_updated_key, retrieved_updated_metadata) = updated_retrieved.unwrap();
        assert_eq!(
            retrieved_updated_key.to_vec(),
            updated_key.to_vec(),
            "Updated key should match new data"
        );
        assert_eq!(
            retrieved_updated_metadata.version, updated_metadata.version,
        ...
        let deleted_retrieved = db
            .retrieve_key(&key_id)
            .await
            .expect("Deleted key retrieval should succeed");
        assert!(
            deleted_retrieved.is_none(),
    ...
    async fn test_key_metadata_management() {
        ...
        metadata.metadata.insert(
            ...
            "Test key for metadata validation".to_string(),
        );
        metadata
            .metadata
        ...
        metadata
            .metadata
            .insert("rotation_policy".to_string(), "90-days".to_string());
        metadata
            .metadata
            .insert("compliance".to_string(), "gdpr".to_string());
        ...
        updated_metadata
            .metadata
            .insert("status".to_string(), "rotated".to_string());
        updated_metadata
            .metadata
            .insert("last_rotation".to_string(), Utc::now().to_rfc3339());

        db.store_key(&key_id, &key, &updated_metadata)
            .await
        ...
        assert!(
            updated_retrieved.is_some(),
            "Updated metadata should be retrievable"
        );
    ...
    async fn test_key_rotation_operations() {
        ...
        db.store_key(&key_id, &initial_key, &initial_metadata)
            .await
        ...
        rotated_metadata
            .metadata
            .insert("rotated".to_string(), "true".to_string());
        rotated_metadata
            .metadata
            .insert("rotation_reason".to_string(), "scheduled".to_string());

        // Store rotated key
        db.store_key(&key_id, &rotated_key, &rotated_metadata)
            .await
        ...
        assert_eq!(
            retrieved_key.to_vec(),
            rotated_key.to_vec(),
            "Retrieved key should be rotated key"
        );
        assert_eq!(retrieved_metadata.version, 2, "Version should be updated");
        assert_eq!(
            retrieved_metadata.metadata.get("rotated"),
            Some(&"true".to_string())
        );
        ...
        for rotation_num in 3..=5 {
            ...
            db.store_key(&key_id, &new_key, &new_metadata)
                .await
                .expect(&format!("Rotation {} should succeed", rotation_num));

            let retrieved = db.retrieve_key(&key_id).await.expect(&format!(
                "Retrieval after rotation {} should succeed",
                rotation_num
            ));
            assert!(
                retrieved.is_some(),
                "Key should be retrievable after rotation {}",
                rotation_num
            );
            ...
            assert_eq!(
                retrieved_key.to_vec(),
                new_key.to_vec(),
                "Key should match after rotation {}",
                rotation_num
            );
            assert_eq!(
                retrieved_metadata.version, rotation_num,
        ...
        assert_eq!(
            final_key.to_vec(),
            vec![5u8; 32],
            "Final key should match last rotation"
        );
    }
    ...
    async fn test_database_transaction_handling() {
        ...
        for (i, key_id) in key_ids.iter().enumerate() {
            ...
            assert_eq!(
                retrieved_metadata.algorithm, metadata_list[i].algorithm,
        ...
        // This should succeed
        db.store_key(&invalid_key_id, &valid_key, &valid_metadata)
            .await
        ...
        assert_eq!(
            stats.total_keys, 11,
            "Should have all 11 keys (10 valid + 1 test)"
        );
    ...
    async fn test_error_recovery_and_rollback() {
        ...
        assert!(
            ...
            "Retrieval should not error for non-existent key"
        );
        assert!(
            retrieval_result.unwrap().is_none(),
        ...
        assert!(
            ...
            "Metadata retrieval should not error for non-existent key"
        );
        assert!(
            metadata_result.unwrap().is_none(),
        ...
        assert!(
            exists_result.is_ok(),
            "Existence check should not error for non-existent key"
        );
    ...
    async fn test_performance_many_keys() {
        ...
        assert!(
            ...
            "Key creation should complete in reasonable time"
        );
        assert!(
            retrieval_duration.as_millis() < 5000,
            "Key retrieval should complete in reasonable time"
        );
        assert!(
            listing_duration.as_millis() < 1000,
            "Key listing should be fast"
        );
    ...
    async fn test_encryption_at_rest() {
        ...
        assert_eq!(
            retrieved_metadata.algorithm, metadata.algorithm,
            "Decrypted metadata should match"
        );
        ...
        let wrong_db = SqliteKeyDatabase::new(wrong_config)
            .await
            .expect("Database creation should succeed");
        wrong_db
            .initialize()
        ...
        let no_encrypt_db = SqliteKeyDatabase::new(no_encrypt_config)
            .await
            .expect("Database creation should succeed");
        no_encrypt_db
            .initialize()
        ...
        no_encrypt_db
            .store_key(&no_encrypt_key_id, &no_encrypt_key, &no_encrypt_metadata)
            .await
            .expect("Non-encrypted key storage should succeed");

        let no_encrypt_retrieved = no_encrypt_db
            .retrieve_key(&no_encrypt_key_id)
            .await
            .expect("Non-encrypted key retrieval should succeed");
        assert!(
            no_encrypt_retrieved.is_some(),
            "Non-encrypted key should be retrievable"
        );

        let (no_encrypt_retrieved_key, _) = no_encrypt_retrieved.unwrap();
        assert_eq!(
            no_encrypt_retrieved_key.to_vec(),
            no_encrypt_key.to_vec(),
            "Non-encrypted key should match"
        );
    }
    ...
    async fn test_key_preloading() {
        ...
        println!(
            "Individual retrieval of {} keys took {:?}",
            individual_retrievals, individual_duration
        );
        ...
        assert_eq!(
            individual_retrievals, num_keys,
            "All keys should be retrievable individually"
        );
    ...
    async fn test_database_statistics_monitoring() {
        ...
        let after_storage_stats = db
            .get_stats()
            .await
            .expect("Stats after storage should succeed");
        assert_eq!(
            after_storage_stats.total_keys, num_keys,
            "Stats should reflect stored keys"
        );
        assert!(
            after_storage_stats.database_size_bytes > initial_stats.database_size_bytes,
        ...
        assert_eq!(
            ...
            "Stats should show 0 keys after cleanup"
        );
    ...
    async fn test_concurrent_database_operations() {
        ...
        assert_eq!(
            final_stats.total_keys, 0,
            "All mixed operation keys should be cleaned up"
        );
    }
}
