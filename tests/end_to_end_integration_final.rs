//! End-to-End Integration Tests for Fortress (Final Working Version)
//!
...
async fn test_concurrent_encryption_performance() -> Result<()> {
    ...
    let performance_event = AuditEvent {
        ...
        data: HashMap::from([
            ...
            ("duration_ms".to_string(), Value::Number(serde_json::Number::from(elapsed.as_millis()))),
            ("ops_per_second".to_string(), Value::Number(serde_json::Number::from_f64(ops_per_second))),
            ("algorithm".to_string(), Value::String("AEGIS-256".to_string())),
        ]),
    };
...
async fn test_cache_performance_under_load() -> Result<()> {
    ...
    let performance_event = AuditEvent {
        ...
        data: HashMap::from([
            ...
            ("hit_rate_percent".to_string(), Value::Number(serde_json::Number::from_f64(hit_rate))),
