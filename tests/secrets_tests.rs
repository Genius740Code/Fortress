...
mod tests {
    ...
    async fn test_secret_creation_and_storage() {
        ...
        let secret = engine.write("secret/app1", &test_data).await
        ...
        assert!(secret.lease.is_some(), "Secret should have lease information");
        ...
        assert!(lease.expires_at > lease.created_at, "Expiry should be after creation");
    }
    ...
    async fn test_secret_retrieval_and_access_control() {
        ...
        engine.write("secret/api", &test_data).await
            .expect("Secret storage should succeed");
        
        // Retrieve the secret
        let retrieved = engine.read("secret/api").await
            .expect("Secret retrieval should succeed");
        
        assert!(retrieved.is_some(), "Secret should be found");
        let secret = retrieved.unwrap();
        assert_eq!(secret.data, test_data, "Retrieved data should match stored data");
        assert_eq!(secret.metadata.name, "secret/api");
        
        // Test retrieval of non-existent secret
        let not_found = engine.read("secret/nonexistent").await
            .expect("Retrieval should not error");
        assert!(not_found.is_none(), "Non-existent secret should return None");
        
        // Test listing secrets
        let secrets = engine.list("secret/").await
            .expect("Listing should succeed");
        assert!(secrets.contains(&"secret/api".to_string()), "Stored secret should be in list");
    }
    ...
    async fn test_secret_rotation_and_versioning() {
        ...
        let secret_v1 = engine.write(secret_path, &v1_data).await
        ...
        let secret_v2 = engine.write(secret_path, &v2_data).await
        ...
        let retrieved_v1 = engine.read_version(secret_path, v1_version).await
            .expect("Version retrieval should succeed");
        assert!(retrieved_v1.is_some(), "Version 1 should be found");
        assert_eq!(retrieved_v1.unwrap().data, v1_data, "Version 1 data should match");
        
        let retrieved_v2 = engine.read_version(secret_path, v2_version).await
            .expect("Version retrieval should succeed");
        assert!(retrieved_v2.is_some(), "Version 2 should be found");
        assert_eq!(retrieved_v2.unwrap().data, v2_data, "Version 2 data should match");
        
        // List all versions
        let versions = engine.list_versions(secret_path).await
    ...
    async fn test_secret_expiration_and_cleanup() {
        ...
        let secret = engine.write("secret/temp", &test_data).await
            .expect("Secret storage should succeed");
        ...
        assert!(renew_after_revoke.is_err(), "Renewal after revocation should fail");
    }
    ...
    async fn test_key_management_integration() {
        ...
        let retrieved = engine.read("secret/keys").await
            .expect("Sensitive data retrieval should succeed");
        
        assert!(retrieved.is_some(), "Encrypted secret should be retrievable");
        let retrieved_secret = retrieved.unwrap();
        assert_eq!(retrieved_secret.data, sensitive_data, "Retrieved data should match original");
        
        // Verify the secret is actually encrypted at rest by checking metadata
        assert!(retrieved_secret.metadata.custom.is_empty(), "Custom metadata should be empty for basic test");
        ...
        engine.write("secret/complex", &complex_data).await
            .expect("Complex data storage should succeed");
        
        let retrieved_complex = engine.read("secret/complex").await
            .expect("Complex data retrieval should succeed");
        
        assert!(retrieved_complex.is_some(), "Complex secret should be retrievable");
        assert_eq!(retrieved_complex.unwrap().data, complex_data, "Complex data should match");
    }
    ...
    async fn test_secrets_engine_manager() {
        ...
        assert!(registration_result.is_ok(), "Engine registration should succeed");
        
        // List engines
        let engines = manager.list_engines();
        assert!(engines.contains(&"kv".to_string()), "Registered engine should be in list");
        
        // Get engine by name
        let engine = manager.get_engine("kv");
        assert!(engine.is_some(), "Should be able to retrieve registered engine");
        
        // Use the engine through manager
        let engine = engine.unwrap();
        let test_data = json!({"managed": "secret"});
        engine.write("secret/managed", &test_data).await
            .expect("Managed engine write should succeed");
        
        let retrieved = engine.read("secret/managed").await
    ...
    async fn test_lease_management_operations() {
        ...
        let secret = engine.write("secret/leased", &test_data).await
            .expect("Secret storage should succeed");
        ...
        // Test lease renewal
        let renewed_lease = engine.renew(&lease.lease_id, Some(7200)).await
            .expect("Lease renewal should succeed");
        
        assert!(renewed_lease.expires_at > lease.expires_at, "Renewed lease should expire later");
        assert_eq!(renewed_lease.renewal_count, original_renewal_count + 1, "Renewal count should increment");
        assert_eq!(renewed_lease.lease_id, lease.lease_id, "Lease ID should remain same");
        
        // Test multiple renewals
        let mut current_lease = renewed_lease;
        for i in 1..5 {
            current_lease = engine.renew(&current_lease.lease_id, Some(3600)).await
                .expect(&format!("Renewal {} should succeed", i));
            assert_eq!(current_lease.renewal_count, original_renewal_count + 1 + i, 
                     "Renewal count should increment correctly");
        }
        
        // Test lease revocation
        engine.revoke(&current_lease.lease_id).await
        ...
        assert!(renewal_after_revoke.is_err(), "Renewal after revocation should fail");
    }
    ...
    async fn test_concurrent_secrets_operations() {
        ...
        for i in 0..num_concurrent {
            ...
            let handle = tokio::spawn(async move {
                ...
                engine_clone.write(&path, &data).await
        ...
        assert_eq!(secrets.len(), num_concurrent, "All concurrent writes should succeed");
        ...
        assert_eq!(retrieved_count, num_concurrent, "All concurrent reads should succeed");
    }
    ...
    async fn test_version_management_and_cleanup() {
        ...
        for i in 1..=5 {
            ...
            let secret = engine.write(secret_path, &data).await
        ...
        let version_list = engine.list_versions(secret_path).await
            .expect("Version listing should succeed");
        assert_eq!(version_list.len(), 3, "Should have only 3 versions after cleanup");
        ...
        for version in &latest_versions {
            assert!(version_list.contains(version), "Should contain version {}", version);
        }
        
        // Test version deletion
        let latest_version = *version_list.iter().max().unwrap();
        engine.delete_version(secret_path, latest_version).await
            .expect("Version deletion should succeed");
        
        // Verify version is deleted (soft-deleted)
        let deleted_version = engine.read_version(secret_path, latest_version).await
            .expect("Version retrieval should succeed");
        assert!(deleted_version.is_none(), "Deleted version should not be accessible");
    }
    ...
    async fn test_engine_configuration_customization() {
        ...
        let secret = engine.write("secret/ConfigTest", &test_data).await
            .expect("Secret storage should succeed");
        ...
        let retrieved = engine.read(case_sensitive_path).await
        ...
        let different_case = engine.read("secret/configtest").await
            .expect("Different case retrieval should succeed");
        assert!(different_case.is_none(), "Different case should be different secret");
    }
    ...
    async fn test_error_handling_edge_cases() {
        ...
        match result {
            Ok(_) => {
                ...
                let retrieved = engine.read("secret/large").await
                    .expect("Large secret retrieval should succeed");
                assert!(retrieved.is_some(), "Large secret should be retrievable");
                assert_eq!(retrieved.unwrap().data, large_data, "Large data should match");
            }
            Err(_) => {
        ...
        if result.is_ok() {
            let retrieved = engine.read(special_path).await
                .expect("Special chars retrieval should succeed");
            assert!(retrieved.is_some(), "Special chars secret should be retrievable");
            assert_eq!(retrieved.unwrap().data, special_data, "Special chars data should match");
        }
    }
    ...
    async fn test_performance_many_secrets() {
        ...
        for i in 0..num_secrets {
            let path = format!("secret/performance/{}", i);
            let result = engine.read(&path).await
        ...
        assert_eq!(successful_reads, num_secrets, "All secrets should be readable");
        
        // Performance assertions (these are rough guidelines)
        assert!(write_duration.as_millis() < 5000, "Writing should complete in reasonable time");
        assert!(read_duration.as_millis() < 2000, "Reading should complete in reasonable time");
        
        // Test list performance
        let start_time = std::time::Instant::now();
        let listed = engine.list("secret/performance/").await
            .expect("Listing should succeed");
