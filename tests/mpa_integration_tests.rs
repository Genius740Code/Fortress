...
async fn setup_comprehensive_mpa_test() -> (MpaService, Vec<UserId>) {
    ...
    let user_configs = vec![
        ...
        ("security_analyst1", "Analyst1_Password123", "Security Analyst"),
        ("security_analyst2", "Analyst2_Password123", "Security Analyst"),
...
async fn test_complete_critical_operations_workflow() {
    ...
    let approvers = vec![security_lead_id.clone(), compliance_officer_id.clone(), auditor_id.clone()];
    ...
    mpa_service.submit_approval_decision(
        ...
        Decision::Approve,
        Some("Key generation parameters reviewed and approved. Security measures are adequate.".to_string()),
    ...
    mpa_service.submit_approval_decision(
        ...
        Decision::Approve,
        Some("Compliance requirements verified. Key generation follows established policies.".to_string()),
...
async fn test_complex_rejection_scenarios() {
    ...
    let request1_id = mpa_service.create_approval_request(
        group_id.clone(),
    ...
    mpa_service.submit_approval_decision(
        ...
        approvers[1].clone(),
        Decision::Reject,
        Some("Insufficient documentation for key deletion. Need detailed audit trail.".to_string()),
        None,
        None,
    ).await.unwrap();
    
    // Verify request is rejected
    let request1 = mpa_service.get_approval_request(&request1_id, requester_id.clone()).await.unwrap().unwrap();
    assert_eq!(request1.status, ApprovalStatus::Rejected);
    
    // Scenario 2: Multiple rejections
    let request2_id = mpa_service.create_approval_request(
        group_id.clone(),
        MultiPersonOperationType::KeyDeletion,
    ...
    // Verify request is rejected
    let request2 = mpa_service.get_approval_request(&request2_id, requester_id.clone()).await.unwrap().unwrap();
    assert_eq!(request2.status, ApprovalStatus::Rejected);
    assert_eq!(request2.decisions.len(), 2);
    
    // Verify no operation is approved
    let is_approved1 = mpa_service.is_operation_approved(&request1_id).await.unwrap();
    let is_approved2 = mpa_service.is_operation_approved(&request2_id).await.unwrap();
    assert!(!is_approved1);
    assert!(!is_approved2);
}
...
async fn test_concurrent_approval_requests() {
    ...
    for (i, request_id) in request_ids.iter().enumerate() {
        ...
        let task = tokio::spawn(async move {
            mpa_service_clone.submit_approval_decision(
                &request_id_clone,
                approver_id,
                Decision::Approve,
                Some(format!("Certificate signing approved for {}", request_id_clone)),
                Some("192.168.1.100".to_string()),
...
async fn test_role_based_permissions() {
    ...
    let result = mpa_service.add_control_group_member(
        &group_id,
...
async fn test_custom_operation_types() {
    ...
    assert_eq!(request1.operation_type, MultiPersonOperationType::UserManagement);
    assert_eq!(request2.operation_type, MultiPersonOperationType::UserManagement);
...
async fn test_performance_under_load() {
    ...
    for i in 1..=100 {
        ...
        let task = tokio::spawn(async move {
            mpa_service_clone.create_approval_request(
                group_id_clone,
                MultiPersonOperationType::HsmOperation,
    ...
    for task in create_tasks {
        ...
        assert!(request_id.is_ok(), "Request creation should succeed under load");
    ...
    assert!(creation_time < Duration::from_secs(10), "Request creation should be fast under load");
    assert!(approval_time < Duration::from_secs(10), "Approval should be fast under load");
    assert!(total_time < Duration::from_secs(20), "Total operation should complete quickly");
...
async fn test_error_handling_and_recovery() {
    ...
    assert!(result.is_err(), "Should fail for non-existent control group");
...
async fn test_multi_group_operations() {
    ...
    let group1_id = mpa_service.create_control_group(
    ...
    let group2_id = mpa_service.create_control_group(
